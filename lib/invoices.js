...
import { markUnitsSold, reactivateUnit } from './catalog-sync';
...
// (reactivateUnit handles both), then marks the invoice 'refunded'. Idempotent:
...
export async function refundInvoice(invoiceId) {
  ...
  const { rows } = await query(
    `UPDATE invoices SET status = 'refunded', refunded_at = now()
      WHERE id = $1 AND status = 'paid' RETURNING id, number, total`,
    [invoiceId]
  );
  if (!rows.length) {
    const { rows: ex } = await query('SELECT id, number, status FROM invoices WHERE id = $1', [invoiceId]);
    if (!ex.length) throw new Error('Invoice not found.');
    if (ex[0].status === 'refunded') return { id: ex[0].id, number: ex[0].number, status: 'refunded', relisted: 0 };
    throw new Error('Only a paid invoice can be refunded.');
  }

  // Put the unit(s) back on the storefront + cancel the bridged order. Best-effort
  // per SKU so one bad row never blocks the rest of the refund.
  let relisted = 0;
  try {
    ...
    for (const it of its) {
      try { await reactivateUnit(it.sku); relisted++; }
      catch (e) { console.error('refund relist failed for', it.sku, e.message); }
    }
  } catch (e) {
    console.error('refundInvoice relist lookup failed', e.message);
  }
  return { id: rows[0].id, number: rows[0].number, status: 'refunded', relisted };
}
...
export async function backfillAllInvoiceOrders() {
  if (!hasDb()) throw new Error('Database not configured.');
  await ensureInvoiceSchema();
  const { rows } = await query("SELECT id FROM invoices WHERE status = 'paid' AND order_id IS NULL ORDER BY id");
  const created = [];
  for (const r of rows) {
    try {
  ...
  return { fixed: created.length, created };
}
