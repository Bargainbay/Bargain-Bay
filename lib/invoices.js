...
function normalizeLines(items, { addHst = false, taxInclusive = false } = {}) {
  const lines = (items || [])
    .map((it) => {
  ...
  // here, once, on the server — the form only ever previews this. The subtotal
  // comes off the quoted TOTAL rather than from dividing each line separately,
  // because the total is the number the customer was actually told. See lib/tax.js.
  if (taxInclusive && addHst) {
    const split = splitTaxInclusive(lines.map((li) => li.amount));
    return lines.map((li, i) => ({ ...li, amount: split.lines[i] }));
  }
  return lines;
}
...
export async function createAndSendInvoice({ name, email, items, addHst, taxInclusive = false, daysUntilDue = 14, memo, deliveryMethod, address, city, postal, phone, sendEmail = true, invoiceDate, createdBy, channel = 'manual', attachToOrderId = null, brand = 'bargain_bay' }) {
  ...
  const lineItems = normalizeLines(items, { addHst, taxInclusive: taxIn });
