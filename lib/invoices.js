...
export async function listInvoices(opts = {}) {
  ...
  const { rows } = await query(
    `SELECT i.id, i.number, i.email, i.name, i.status, i.total, i.refund_total, i.payment_method,
      ...
      LIMIT $4 OFFSET $5`,
    [like, phoneLike, status, limit + 1, offset]
  );
...
export async function updateInvoice(invoiceId, { items, addHst, memo, invoiceDate }) {
  ...
  const orderId = ex[0].order_id || null;
  // SKUs the invoice holds right now, so the re-hold below can move only what
  // actually changed rather than dropping every reservation and racing to
  // reclaim it (a passing checkout could win that race and take the unit).
  const heldBefore = new Set(
    (await query("SELECT sku FROM invoice_items WHERE invoice_id = $1 AND sku IS NOT NULL AND COALESCE(kind,'unit') <> 'service'", [invoiceId])
  ...
  await withTransaction(async (client) => {
    ...
    // Keep the fulfilment order in step with the edit. The order is what the
    // revenue dashboard counts and what holds the units off the storefront, so
    // an edited invoice whose order still carried the old lines and old total
    // would report the wrong money and hold the wrong stock.
    if (orderId) {
      ...
      // Revenue follows the invoice date, so a re-date moves the sale with it.
      if (backdate) {
  ...
  // Move the holds to match the edited lines: release only what came OFF the
  // invoice, claim only what went ON. Untouched lines keep the reservation they
  // already have, so editing an unrelated line can't hand a held unit to a
  // passing checkout. Outside the transaction and best-effort — a unit somebody
  // else has grabbed is reported, never a reason to lose the edit.
  let contested = [];
  if (orderId) {
    const nowHeld = new Set(lineItems.filter((li) => li.kind !== 'service' && li.sku).map((li) => li.sku));
    const dropped = [...heldBefore].filter((sku) => !nowHeld.has(sku));
    const added = [...nowHeld].filter((sku) => !heldBefore.has(sku));
    if (dropped.length) {
      await query('DELETE FROM reservations WHERE order_id = $1 AND sku = ANY($2)', [orderId, dropped]).catch(() => {});
    }
    if (added.length) contested = await holdInvoiceSkus(orderId, added);
  }

  return { id: invoiceId, number: ex[0].number, subtotal, hst, total, contested };
}
