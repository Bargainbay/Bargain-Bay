...
export async function createAndSendInvoice({ name, email, items, addHst, taxInclusive = false, daysUntilDue = 14, memo, deliveryMethod, address, city, postal, phone, sendEmail = true, invoiceDate, createdBy, channel = 'manual', attachToOrderId = null, brand = 'bargain_bay', leadSource = null, leadBy = null }) {
  ...
  const invoice = await withTransaction(async (client) => {
    ...
    for (const li of lineItems) {
      await client.query(
        'INSERT INTO invoice_items (invoice_id, description, sku, amount, kind, warranty_months, cost, typed_amount) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)',
        [id, li.description, li.sku, li.amount, li.kind, li.warrantyMonths, li.cost, li.typedAmount ?? null]
      );
    }
...
export async function getInvoiceByNumber(number) {
  ...
  const { rows: items } = await query(
    'SELECT id, description, sku, amount, kind, warranty_months, refunded_at, typed_amount FROM invoice_items WHERE invoice_id = $1 ORDER BY id',
    [inv.id]
  );
...
export async function updateInvoice(invoiceId, {
  ...
  await withTransaction(async (client) => {
    ...
    for (const li of lineItems) {
      await client.query(
        'INSERT INTO invoice_items (invoice_id, description, sku, amount, kind, warranty_months, cost, typed_amount) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)',
        [invoiceId, li.description, li.sku, li.amount, li.kind, li.warrantyMonths, li.cost, li.typedAmount ?? null]
      );
    }
    await client.query(
