...
export async function createAndSendInvoice({ name, email, items, addHst, daysUntilDue = 14, memo, deliveryMethod, address, city, postal, phone, sendEmail = true, invoiceDate, createdBy }) {
  ...
  const invoice = await withTransaction(async (client) => {
    ...
    const { rows } = await client.query(
      ...
      [email, name || null, subtotal, hst, total, memo || null, String(days),
       ...
       backdate, author.email, author.name]
    );
    const id = rows[0].id;
  ...
  try {
    ...
    if (repLabel) {
      ...
      await query('UPDATE orders SET sales_rep = $2 WHERE id = $1', [order.id, repLabel]).catch(() => {});
    }
  } catch (e) {
...
export async function listInvoices(opts = {}) {
  ...
  const { rows } = await query(
    `SELECT i.id, i.number, i.email, i.name, i.status, i.total, i.refund_total, i.payment_method,
            ...
            i.created_by, i.created_by_name,
