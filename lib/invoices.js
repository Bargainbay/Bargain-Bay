...
export async function updateInvoice(invoiceId, { items, addHst, memo, invoiceDate }) {
  ...
  // Re-hold exactly the SKUs the invoice now carries: drop the old reservations
  // and claim the current set. Outside the transaction (and best-effort) so a
  // unit someone else has grabbed is reported, never a reason to lose the edit.
  let contested = [];
  if (orderId) {
    await query('DELETE FROM reservations WHERE order_id = $1', [orderId]).catch(() => {});
    contested = await holdInvoiceSkus(orderId, lineItems.map((li) => li.sku));
  }
