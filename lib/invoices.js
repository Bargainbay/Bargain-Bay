...
export async function createAndSendInvoice({ name, email, items, addHst, daysUntilDue = 14, memo, deliveryMethod, address, city, postal, phone, sendEmail = true, invoiceDate, createdBy, channel = 'manual', attachToOrderId = null, brand = 'bargain_bay' }) {
  ...
  const lineItems = (items || [])
    ...
    .filter((li) => li.description && li.amount !== 0 && Number.isFinite(li.amount));
  // A SERVICE line may be negative — that's a discount / credit (a promo code
  // at checkout, a goodwill knock-off on a manual invoice). A unit line can't: a
  // negative appliance is a typo, and it would confuse the order bridge. Rejected
  // rather than dropped, or the typo silently deletes the line.
  const badNegative = lineItems.find((li) => li.amount < 0 && li.kind !== 'service');
  if (badNegative) {
    throw new Error(`“${badNegative.description}” has a negative price. Only a service or credit line can be negative.`);
  }
  if (!lineItems.some((li) => li.amount > 0)) throw new Error('Add at least one line item with a description and a positive amount.');
