...
function ensureInvoiceSchema() {
  ...
  if (!_invSchema) {
    _invSchema = query(`
      ...
      -- The matching pair on ORDERS, provisioned here as well as in
      -- ensureAttributionColumns (which the checkout path uses). Not belt and
      -- braces: an invoice edit mirrors the corrected lead onto its order in the
      -- SAME transaction as the line items, so a missing column there would fail
      -- the whole edit and lose the correction. Idempotent DDL, run once per
      -- process — the cost of having it in both places is nothing.
...
export async function listLeadSenders() {
  ...
  try {
    const { rows } = await query(
      `SELECT lead_by AS name, COUNT(*) AS n
         FROM invoices
        WHERE COALESCE(lead_by,'') <> ''
        GROUP BY lead_by ORDER BY n DESC, name LIMIT 100`
    );
