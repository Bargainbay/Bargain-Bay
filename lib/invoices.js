...
export async function updateInvoice(invoiceId, {
  ...
  try {
    if (nextStatus === 'paid') {
      ...
      if (orderId) await query('DELETE FROM reservations WHERE order_id = $1', [orderId]).catch(() => {});
    } else {
      ...
      if (removed.length) {
        if (orderId && !wasSettled) {
          await query('DELETE FROM reservations WHERE order_id = $1 AND sku = ANY($2)', [orderId, removed]).catch(() => {});
        }
        relisted = removed;
      }
      if (orderId) {
        ...
        if (need.length) contested = await holdInvoiceSkus(orderId, need);
      }
    }
  } catch (e) {
