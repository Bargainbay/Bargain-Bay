...
import { round2, HST_RATE, money, MAX_RESTOCKING_FEE_PCT, isUnitLine, isCreditLine, normalizeLineKind, normalizeLeadSource, torontoToday } from './constants';
...
import { ensureAttributionColumns } from './attribution';

// Self-provision the fulfilment columns added for the invoice→order bridge, so
// it works on deploy without a manual migration (mirrors db/migrations/0001_baseline.sql). Cached.
let _invSchema = null;
...
const WARRANTY_TERMS = new Set([3, 6, 12, 24]);

// Clean a caller's line items into what the invoice actually stores. The single
// place that decides what each kind is allowed to carry, so the create and edit
// paths can't drift apart (they were duplicated, and did).
//
// Sign is enforced, not trusted: a discount or a trade-in is money coming OFF
// the sale, so it is always stored negative however it arrives — the whole rest
// of the system can then treat a line total as plain SUM(amount). A negative
// UNIT line is rejected rather than dropped: it's a typo, and silently deleting
// the appliance somebody just typed is the worse failure.
function normalizeLines(items) {
  const lines = (items || [])
    .map((it) => {
      const kind = normalizeLineKind(it.kind);
      const unit = isUnitLine(kind);
      ...
      return {
        ...
        amount: isCreditLine(kind) ? -Math.abs(amount) : amount,
        sku: unit ? (String(it.sku || '').trim() || null) : null,
        kind,
        warrantyMonths: unit ? (WARRANTY_TERMS.has(wm) ? wm : null) : null,
        // Captured unit cost (for a unit not cost-linked in the catalog) so margin
        // is right. null for everything that isn't an appliance.
        cost: unit ? (Number.isFinite(cost) && cost >= 0 ? round2(cost) : null) : null,
        // Only meaningful on an appliance line with no SKU — a picked unit needs no reason.
        offStockReason: unit && !String(it.sku || '').trim()
          ? (String(it.offStockReason || '').trim().slice(0, 200) || null) : null
      };
    })
    .filter((li) => li.description && Number.isFinite(li.amount) && li.amount !== 0);

  const badNegative = lines.find((li) => li.amount < 0 && !isCreditLine(li.kind) && li.kind !== 'service');
  if (badNegative) {
    throw new Error(`“${badNegative.description}” has a negative price. Use a Discount or Trade-in line to take money off.`);
  }
  if (!lines.some((li) => li.amount > 0)) {
    throw new Error('Add at least one line item with a description and a positive amount.');
  }

  return lines;
}

// The line items AND the money, from what a caller sent. Both the create and the
// edit path go through here: they used to each normalize the lines and then
// repeat the same three lines of totals arithmetic, which is exactly the drift
// normalizeLines exists to prevent.
//
// Prices quoted WITH the tax in them ("twelve hundred out the door") are split
// here, once, on the server — the form only ever previews this. Two things make
// that case different from the ordinary one, and both matter:
//
//   * the subtotal comes off the quoted TOTAL, not from dividing each line
//     separately and adding up (that drifts), and
//   * the HST is the REMAINDER of the quote, not 13% of the subtotal recomputed.
//     Recomputing is what made a $800 out-the-door sale invoice for $799.99.
//
// See lib/tax.js for why the cent has to land in the tax line instead.
// Exported so a caller that must reason about PRE-TAX money before saving (the
// consignment floor check) sees exactly the figures that will be stored, rather
// than doing its own division by 1.13 and drifting from them.
export function priceInvoice(items, { addHst = false, taxInclusive = false } = {}) {
  const lines = normalizeLines(items);
  const taxIn = !!taxInclusive && !!addHst;

  if (taxIn) {
    const split = splitTaxInclusive(lines.map((li) => li.amount));
    if (!(split.subtotal > 0)) throw new Error('The credit lines cancel out the invoice — its total has to be more than zero.');
    // typedAmount is what came in, before the split — the number the rep keyed
    // and the customer was told. Kept so reopening the invoice shows it back
    // exactly rather than a gross-up that lands a cent off it.
    return {
      lineItems: lines.map((li, i) => ({ ...li, amount: split.lines[i], typedAmount: li.amount })),
      subtotal: split.subtotal, hst: split.hst, total: split.total
    };
  }

  const subtotal = round2(lines.reduce((a, li) => a + li.amount, 0));
  if (!(subtotal > 0)) throw new Error('The credit lines cancel out the invoice — its total has to be more than zero.');
  const hst = addHst ? round2(subtotal * HST_RATE) : 0;
  return { lineItems: lines, subtotal, hst, total: round2(subtotal + hst) };
}
...
export async function createAndSendInvoice({ name, email, items, addHst, taxInclusive = false, daysUntilDue = 14, memo, deliveryMethod, address, city, postal, phone, sendEmail = true, invoiceDate, createdBy, channel = 'manual', attachToOrderId = null, brand = 'bargain_bay', leadSource = null, leadBy = null }) {
  ...
  const taxIn = !!taxInclusive && !!addHst;
  const { lineItems, subtotal, hst, total } = priceInvoice(items, { addHst, taxInclusive: taxIn });
  ...
  // Where the lead came from, and who sent it. An unrecognised source is stored
  // as NULL rather than as itself: the whole value of the report is that the
  // buckets are countable, and one free-text answer spelled four ways is four
  // buckets. NULL reads as "nobody said", which the report shows on purpose.
  const lead = {
    source: normalizeLeadSource(leadSource),
    by: String(leadBy || '').trim().slice(0, 120) || null
  };
  ...
  const invoice = await withTransaction(async (client) => {
    ...
    const { rows } = await client.query(
      `INSERT INTO invoices (email, name, status, subtotal, hst, total, memo, due_date,
                             ...
                             created_by, created_by_name, channel, brand, tax_inclusive,
                             lead_source, lead_by)
      ...
      [email, name || null, subtotal, hst, total, memo || null, String(days),
       ...
       brand === 'rs_solutions' ? 'rs_solutions' : 'bargain_bay', taxIn,
       lead.source, lead.by]
    );
    const id = rows[0].id;
    ...
    for (const li of lineItems) {
      await client.query(
        'INSERT INTO invoice_items (invoice_id, description, sku, amount, kind, warranty_months, cost, typed_amount, off_stock_reason) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)',
        [id, li.description, li.sku, li.amount, li.kind, li.warrantyMonths, li.cost, li.typedAmount ?? null, li.offStockReason ?? null]
      );
    }
  ...
  // The same push for where the lead came from. Its own function rather than a
  // branch inside stampRep: a sale can have a lead source with no rep on it
  // (Sarah's phone orders), and stampRep returns early when there is no rep.
  //
  // The report reads ORDERS, not invoices — that is where revenue lives, and a
  // storefront sale has no manual invoice to read from — so the answer has to
  // reach the order or it is invisible. Best-effort for the same reason as the
  // rep stamp: losing the invoice over a reporting column would be absurd.
  const stampLead = async (orderId, keepExisting) => {
    if (!lead.source && !lead.by) return;
    // The one place this path needs the columns, so this is where it pays for
    // them — cached per process, and never on a read.
    await ensureAttributionColumns().catch(() => {});
    await query(
      keepExisting
        ? 'UPDATE orders SET lead_source = COALESCE(lead_source, $2), lead_by = COALESCE(lead_by, $3) WHERE id = $1'
        : 'UPDATE orders SET lead_source = $2, lead_by = $3 WHERE id = $1',
      [orderId, lead.source, lead.by]
    ).catch(() => {});
  };
  ...
  if (attachToOrderId) {
    ...
    try {
      ...
      // keepExisting: the order was raised by the storefront and already says
      // where it came from. An invoice attaching to it must not overwrite that.
      await stampLead(attachToOrderId, true);
    } catch (e) {
    ...
    try {
      const order = await createOrderFromInvoice(
        { invoiceNumber: invoice.number, email, name, phone,
          ...
          items: lineItems.map((li) => ({ sku: li.sku, title: li.description, price: li.amount, cost: li.cost, kind: li.kind })) },
      ...
      await stampLead(order.id, false);
    } catch (e) {
...
// Names that have already been typed into "sent by", most-used first. Offered
// as a datalist on both invoice screens for one reason: the report groups on
// this string, so "Dave", "dave" and "Dave S." are three referrers who are one
// person. Suggesting what was typed before is the only thing that keeps the
// buckets from fragmenting — a free-text field with no memory always does.
export async function listLeadSenders() {
  if (!hasDb()) return [];
  await ensureInvoiceSchema();
  try {
    // Folded the same way the report groups them (lib/analytics.js leadReport),
    // and offered as the most-used spelling. Suggesting both "Dave" and "dave"
    // would invite the rep to pick the one the report is about to merge away,
    // and would make the list look like two people.
    const { rows } = await query(
      `SELECT mode() WITHIN GROUP (ORDER BY lead_by) AS name, COUNT(*) AS n
         FROM invoices
        WHERE COALESCE(lead_by,'') <> ''
        GROUP BY lower(btrim(lead_by)) ORDER BY n DESC, name LIMIT 100`
    );
    return rows.map((r) => r.name);
  } catch (e) {
    console.error('listLeadSenders failed', e.message);
    return [];
  }
}
...
export async function listInvoices(opts = {}) {
  ...
  const where = `
    ($1 = '' OR (
      ...
      -- Searchable by whoever SENT the lead, so a name on the referrals panel
      -- can be typed straight into the search box to see their actual sales.
      OR lower(coalesce(i.lead_by,'')) LIKE $1
...
export async function getInvoiceByNumber(number) {
  ...
  const { rows: items } = await query(
    'SELECT id, description, sku, amount, kind, warranty_months, refunded_at, typed_amount, off_stock_reason FROM invoice_items WHERE invoice_id = $1 ORDER BY id',
    [inv.id]
  );
...
//
// With a `restockingPct` this hands off to the per-line path over everything
// still un-refunded, and that path closes the invoice out. The two must not be
// merged: cancelling the order (what a plain full refund does) would erase the
// fee, and per-line arithmetic on the LINES is the only thing that keeps a fee
// kept on an earlier partial return from being refunded back on a later one.
export async function refundInvoice(invoiceId, { restockingPct = 0, reason = '', by = null } = {}) {
  ...
  const pct = clampRestockingPct(restockingPct);
  if (pct > 0) {
    const { rows: left } = await query(
      'SELECT id FROM invoice_items WHERE invoice_id = $1 AND refunded_at IS NULL', [invoiceId]
    );
    if (left.length) {
      // Selecting every remaining line IS the full refund, and refundInvoiceItems
      // closes the invoice out when a fee is being kept. No recursion: it only
      // calls back here when there's no fee.
      return refundInvoiceItems(invoiceId, { itemIds: left.map((r) => r.id), restockingPct: pct, reason, by });
    }
  }

  // What is still refundable = everything charged that hasn't already come back.
  // Derived from the totals rather than the lines, because at this point there
  // are none left to derive it from (a service-only invoice, or one whose lines
  // were all stamped by an earlier refund).
  ...
  const split = splitRestocking(owedBase, pct, hasHst);
  ...
  if (inv.order_id) {
    try {
      if (!split.feeKept) {
        ...
        // Shrink the order by what went back and put the fee on as its own line.
        // Additive, never a rewrite: an order can already carry fees kept on an
        // earlier partial return, and those are earned revenue.
        await withTransaction(async (client) => {
          const { rows: ord } = await client.query('SELECT id FROM orders WHERE id = $1', [inv.order_id]);
          if (!ord.length) return;
          // One adjustment line for the money going back; what stays on the
          // order IS the fee, so it needs no line of its own here.
          await client.query(
            'INSERT INTO order_items (order_id, sku, title, price) VALUES ($1, null, $2, $3)',
            [inv.order_id, `Refund ${torontoToday()} — ${inv.number} returned, ${split.pct}% restocking fee kept`.slice(0, 200), -split.refundBase]
          );
          await client.query(
            `UPDATE orders SET
  ...
  try {
    const { rows: its } = await query(
      "SELECT sku FROM invoice_items WHERE invoice_id = $1 AND sku IS NOT NULL AND COALESCE(kind,'unit') = 'unit' AND refunded_at IS NULL",
      [invoiceId]
    );
...
export async function refundInvoiceItems(invoiceId, { itemIds = [], skus = [], restockingPct = 0, reason = '', by = null } = {}) {
  ...
  const { rows: everyLine } = await query(
    'SELECT id, description, sku, amount, kind, refunded_at FROM invoice_items WHERE invoice_id = $1 ORDER BY id',
    [invoiceId]
  );
  // Only charged lines can come back. You can't "return" a discount or a
  // trade-in credit — refunding one would hand the customer money they never
  // paid — so they're excluded from what's refundable in the first place, and
  // therefore from the "was that the last line?" test as well.
  const all = everyLine.filter((it) => !isCreditLine(it.kind));
  ...
  // What the customer actually paid for these lines. An invoice carrying a
  // discount or a trade-in credit charged LESS than its lines add up to, so a
  // line refunded at face value hands back more than was ever taken. The credit
  // is therefore spread across the charged lines in proportion: refunding a
  // $1,000 unit off a $1,000 sale that had $100 off returns $900, and the
  // reclaimed $100 goes back on the order as its own line so the order's items
  // still add up to its subtotal.
  const chargedBase = round2(everyLine.filter((it) => !isCreditLine(it.kind))
    .reduce((a, it) => a + (Number(it.amount) || 0), 0));
  const creditBase = round2(Math.abs(everyLine.filter((it) => isCreditLine(it.kind))
    .reduce((a, it) => a + (Number(it.amount) || 0), 0)));
  const creditFactor = chargedBase > 0
    ? Math.max(0, Math.min(1, round2((chargedBase - creditBase) / chargedBase * 10000) / 10000))
    : 1;

  const remaining = all.filter((it) => !it.refunded_at);
  const isFull = targets.length === remaining.length;
  const pct = clampRestockingPct(restockingPct);
  // Every remaining line, and nothing kept → the sale is off entirely, so reuse
  // the full-refund path: it cancels the order outright. With a fee kept the
  // order has to survive carrying it, so the per-line arithmetic below runs and
  // simply closes the invoice out at the end. (refundInvoice only calls back
  // into here when a fee IS being kept, so these two can't recurse.)
  if (isFull && !pct) {
    const full = await refundInvoice(invoiceId, { restockingPct: 0, reason, by });
    return { ...full, refundedItems: targets.length, fullyRefunded: true };
  }

  const hasHst = Number(inv.hst) > 0;
  const faceValue = round2(targets.reduce((a, it) => a + (Number(it.amount) || 0), 0));
  const base = round2(faceValue * creditFactor);
  // The share of the invoice's discount / trade-in credit that belonged to the
  // lines coming back. It stops being a credit the moment they do.
  const creditReclaimed = round2(faceValue - base);
  const split = splitRestocking(base, pct, hasHst);
  ...
  await withTransaction(async (client) => {
    ...
    await client.query(
      `UPDATE invoices SET
       ...
       WHERE id = $1`,
      [invoiceId, split.refundAmount, isFull]
    );
    ...
    if (inv.order_id) {
      ...
      if (ord.length && ord[0].status !== 'cancelled') {
        ...
        if (creditReclaimed > 0) {
          await client.query(
            'INSERT INTO order_items (order_id, sku, title, price, kind) VALUES ($1, null, $2, $3, $4)',
            [inv.order_id, `Credit reclaimed on refund of ${targets.length} line(s)`.slice(0, 200), creditReclaimed, 'service']
          );
        }
        ...
        await client.query(
          `UPDATE orders SET
           ...
           WHERE id = $1`,
          [inv.order_id, split.refundBase, split.refundHst, split.refundAmount,
           `${isFull ? 'Returned' : 'Partial refund'} ${torontoToday()}: ${targets.map((t) => t.description).join(', ')} (−$${split.refundAmount.toFixed(2)}` +
           (split.feeKept ? `, kept ${split.pct}% restocking fee $${split.feeKept.toFixed(2)}` : '') + ')']
        );
        // Nothing is owed on this order any more, so nothing should still be
        // held off the storefront in its name.
        if (isFull) await client.query('DELETE FROM reservations WHERE order_id = $1', [inv.order_id]);
      }
    }
  });
  ...
  let relisted = 0;
  try {
    const refundSkus = [...new Set(targets.filter((it) => it.sku && isUnitLine(it.kind)).map((it) => it.sku))];
  ...
  await logRefund(invoiceId, { amount: split.refundAmount, fee: split.feeKept, pct: split.pct, kind: isFull ? 'full' : 'items', reason, by });
  return {
    id: inv.id, number: inv.number, status: isFull ? 'refunded' : 'paid', fullyRefunded: isFull,
    refundedItems: targets.length, refundAmount: split.refundAmount, relisted,
    feeKept: split.feeKept, restockingPct: split.pct, orderCancelled: false,
...
export async function updateInvoice(invoiceId, {
  items, addHst, taxInclusive = false, memo, invoiceDate,
  name, email, phone, deliveryMethod, address, city, postal,
  leadSource, leadBy
}) {
  if (!hasDb()) throw new Error('Database not configured.');
  await ensureInvoiceSchema();
  // The lead columns ride onto the ORDER in the same transaction as the line
  // items below, so they have to exist before it opens — a missing column there
  // fails the whole edit and loses the correction. Awaited here rather than from
  // ensureInvoiceSchema so the lock lands on an edit, not on every invoice read.
  if (leadSource !== undefined || leadBy !== undefined) {
    await ensureAttributionColumns().catch(() => {});
  }
  const backdate = normalizeBackdate(invoiceDate, 'invoice date');
  const { rows: ex } = await query(
  ...
  const { rows: before } = await query(
    "SELECT sku, amount FROM invoice_items WHERE invoice_id = $1 AND sku IS NOT NULL AND COALESCE(kind,'unit') = 'unit'",
    [invoiceId]
  );
  const skusBefore = new Map(before.map((r) => [r.sku, round2(Number(r.amount) || 0)]));
  const skusAfter = new Map(
    lineItems.filter((li) => isUnitLine(li.kind) && li.sku).map((li) => [li.sku, li.amount])
  );
  ...
  const contact = {
    ...
    // Correcting where the sale came from is an ordinary fix — the answer is
    // often given after the fact ("that was Dave's customer"). Normalised on the
    // way in exactly as it is on creation, so an edit can't introduce a bucket
    // that creation would have refused.
    lead_source: leadSource === undefined ? undefined : normalizeLeadSource(leadSource),
    lead_by: leadBy === undefined ? undefined : (String(leadBy || '').trim().slice(0, 120) || null)
  };
  ...
  await withTransaction(async (client) => {
    ...
    for (const li of lineItems) {
      await client.query(
        'INSERT INTO invoice_items (invoice_id, description, sku, amount, kind, warranty_months, cost, typed_amount, off_stock_reason) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)',
        [invoiceId, li.description, li.sku, li.amount, li.kind, li.warrantyMonths, li.cost, li.typedAmount ?? null, li.offStockReason ?? null]
      );
    }
    await client.query(
      'UPDATE invoices SET subtotal = $2, hst = $3, total = $4, memo = $5, status = $6, tax_inclusive = $7 WHERE id = $1',
      [invoiceId, subtotal, hst, total, memo != null ? String(memo).trim() : null, nextStatus, taxIn]
    );
    ...
    if (orderId) {
      ...
      for (const li of lineItems) {
        await client.query(
          'INSERT INTO order_items (order_id, sku, title, price, cost, kind) VALUES ($1,$2,$3,$4,$5,$6)',
          [orderId, li.sku || null, li.description, li.amount, li.cost, li.kind]
        );
      }
      // The order now carries the invoice's own lines verbatim — including a
      // storefront promo, which arrives as a negative "Promo code" line. So the
      // separate orders.discount field is cleared: keeping both would count the
      // same discount twice on every screen that reads it. The code stays for
      // the record; the affiliate's numbers live in coupon_redemptions and are
      // untouched by an edit.
      await client.query(
        'UPDATE orders SET subtotal = $2, hst = $3, total = $4, discount = 0 WHERE id = $1',
...
export async function backfillInvoiceOrder(invoiceId, { holdSkus = true } = {}) {
  ...
  // Carry where the sale came from onto the order this just built. Without it
  // every sale that reaches its order through the backfill — an invoice whose
  // up-front bridge threw, one paid before the bridge existed, anything the
  // nightly sweep or the Sync button catches — lands with a NULL lead_source and
  // is counted as "Not recorded" forever, because leadReport reads ORDERS. The
  // invoice would still plainly say "Referral · Dave Mitchell" and nobody would
  // be able to explain the gap. Best-effort: the order is the point, not this.
  if (iv.lead_source || iv.lead_by) {
    await ensureAttributionColumns().catch(() => {});
    await query(
      'UPDATE orders SET lead_source = $2, lead_by = $3 WHERE id = $1',
      [order.id, iv.lead_source || null, iv.lead_by || null]
    ).catch((e) => console.error('backfill lead stamp failed', e.message));
  }
