...
export async function backfillInvoiceOrder(invoiceId) {
  ...
  const { rows: lines } = await query('SELECT description, sku, amount, cost FROM invoice_items WHERE invoice_id = $1 ORDER BY id', [invoiceId]);
  const order = await createOrderFromInvoice({
    ...
    items: lines.map((l) => ({ sku: l.sku || null, title: l.description, price: l.amount, cost: l.cost }))
  });
