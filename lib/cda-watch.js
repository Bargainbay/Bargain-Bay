...
export async function watchCdaSheet({ dryRun = false, max = 200 } = {}) {
  ...
  try {
    ...
    return failed(e.message, dryRun);
  }
  ...
  try {
    ...
    return failed(`could not read the workbook: ${e.message}`, dryRun);
  }
  ...
  try {
    ...
    return failed(`could not read what has already been seen: ${e.message}`, dryRun);
  }
...
// Every way this watcher gives up, in one place, because each of them used to
// return quietly to a cron nobody reads. CDA edit their workbook in place and
// there is no event — so a watcher that has silently stopped looking is
// indistinguishable from a week where they added no rows. That is the whole
// failure this feature was written to end, and it applied to the watcher itself.
//
// Throttled on the message: this runs eight times a day, and an expired Microsoft
// grant would otherwise mail the desk every three hours until somebody reconnects.
const ALERT_KEY = 'cda_alert';
const QUIET_HOURS = 12;

async function failed(reason, dryRun) {
  if (!dryRun) {
    try {
      const now = Date.now();
      ...
      if (!last || last.reason !== reason || now - new Date(last.at).getTime() >= QUIET_HOURS * 3600e3) {
        ...
        await sendEmail({
          to: dispatchDesk(),
          subject: '[Dispatch] The CDA workbook is not being read',
          brand: 'rs_solutions',
          html: `<p><b>The watcher on CDA's shared workbook has stopped reading it.</b></p>
            <p>${reason}</p>
            <p>Nothing is being staged from it, so any rows they have added since are not
            on the Import tab. The commonest cause is the Microsoft sign-in having expired —
            reconnect it under <b>People &amp; access</b> on dispatch.</p>
            <p style="color:#777;font-size:12px">You will not get this again for ${QUIET_HOURS} hours
            unless the problem changes.</p>`
        });
      }
    } catch (e) {
      console.error('cda alert failed', e.message);
    }
  }
