...
// Decorate units in place-style: when RS Ops has photos for a SKU, the real
// Front photo becomes `image` (imageFor() honors an explicit image field, so
// cards, the buy panel, the Meta feed, and OG metadata all pick it up), and
// the full gallery rides along as `rsopsPhotos`.
export async function withRsopsPhotos(units) {
  ...
  return units.map((u) => {
    ...
    return { ...u, image: photos[0].url, rsopsPhotos: photos };
  });
}
