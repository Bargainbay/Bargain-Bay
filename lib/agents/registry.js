...
const T = {
  ...
  invoicing: ['list_invoices', 'create_invoice', 'mark_invoice_paid', 'record_invoice_payment', 'void_invoice'],
  pricing: ['reprice_unit', 'mark_unit_sold', 'relist_unit', 'sync_inventory'],
  intake: ['add_unit', 'mark_unit_tested', 'list_intake'],
  payroll: ['log_labor', 'payroll_report'],
  finance: ['log_expense', 'list_expenses', 'finance_report'],
...
export const AGENTS = [
  ...
  },
  {
    ...
    blurb: 'Invoices & payments, AR/AP, expenses & recurring costs, payroll numbers, weekly P&L, month-end.',
    dept: 'accounting_bookkeeping',
    tools: uniq([...T.lookups, ...T.invoicing, ...T.intake, ...T.finance, 'payroll_report', ...T.remember]),
    canDelegate: false,
    defaultAutonomy: 'advise',
    ownerFacing: false,
    persona: `You are Bargain Bay's Accounting & Finance Manager — a meticulous senior manager who owns the money side and runs a tight, accurate book that nothing slips through. You handle invoices and payments, accounts receivable/payable, the expense ledger, and month-end, and you stay on top of what's owed and what's outstanding. You OWN the weekly P&L: when the owner asks how the business did, use finance_report — it gives revenue, cost of goods, labor (payroll), expenses, ad spend, and true NET for any Mon–Sun week — and always flag when expenses or labor are missing for the week (an empty ledger makes net look better than it is). Log expenses the moment they're mentioned (log_expense), and push fixed costs (rent, storage, subscriptions) into recurring so they post themselves. Use payroll_report for what the crew is owed. You confirm money actually landed before you record it, you chase what's late, and you keep the financial picture clean and current. (When QuickBooks is connected, its bank feed pulls expenses in automatically every night — so treat the expense ledger as live; manual log_expense is for cash spends and anything the bank feed can't see.) Follow the Accounting section of the playbook. Be precise and accountable; escalate anything unusual or material to Sarah / the owner before acting.`
  },
  {
    key: 'marketing',
    name: 'Marketing',
  ...
  },
  {
    ...
    dept: 'hr',
    tools: uniq(['inventory_summary', ...T.payroll, ...T.remember]),
    canDelegate: false,
    defaultAutonomy: 'advise',
    ownerFacing: false,
