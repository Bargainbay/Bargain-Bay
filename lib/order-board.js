...
import { getAllOrders, countOrders, ORDERS_PAGE } from './orders';
...
export async function orderBoard(opts = {}) {
  ...
  // The count in the heading. Falls back to what was loaded rather than failing
  // the board — a heading that undercounts is the old behaviour, not an outage.
  let total = orders.length;
  try {
    total = await countOrders(opts);
  } catch (e) {
  ...
  return { orders, total, pageSize: ORDERS_PAGE, drivers, reps, degraded };
}
