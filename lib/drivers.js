...
export async function mergeDrivers(keepId, dropId) {
  ...
  await withTransaction(async (client) => {
    // Which stops this actually touches, before anything moves — so the history
    // written afterwards lands on those and not on every stop the keeper has
    // ever had.
    const { rows: touched } = await client.query(
      'SELECT id FROM jobs WHERE driver_id = $1 OR driver2_id = $1', [drop]
    );
    const ids = touched.map((r) => r.id);

    // The order of these three is load-bearing, and `jobs_crew_distinct` is what
    // makes getting it wrong an error rather than a silent duplicate.
    //
    // 1. A stop that already carried BOTH accounts is one person once they are
    //    merged, so the second seat is emptied first. Doing this after the moves
    //    below would mean writing the same id into both seats on the way past.
    await client.query(
      `UPDATE jobs SET driver2_id = NULL
        WHERE (driver2_id = $1 AND driver_id = $2) OR (driver2_id = $2 AND driver_id = $1)`,
      [drop, keep]
    );
    // 2. then the remaining second seats, and 3. the primary seats. Nothing can
    //    collide by this point: step 1 removed every pairing of the two.
    await client.query('UPDATE jobs SET driver2_id = $1 WHERE driver2_id = $2', [keep, drop]);
    await client.query('UPDATE jobs SET driver_id = $1 WHERE driver_id = $2', [keep, drop]);
    moved.jobs = ids.length;

    // A merge changes who is on a stop, so it goes in that stop's history like
    // any other crew change — otherwise the stop's History reads as a driver who
    // vanished off it with nothing saying where they went.
    if (ids.length) {
      await client.query(
        `INSERT INTO job_events (job_id, event, detail, by_name)
         SELECT id, 'assigned',
                'driver #' || driver_id || COALESCE(' + #' || driver2_id, '') || ' · accounts merged',
                'Merge'
...
export async function listDriversForOffice() {
  if (!hasDb()) return [];
  await ensureDriverSchema();
  // hourly_rate lives with the shift schema; the roster reads it, so make sure
  // the column is there before selecting it on a database that predates it.
  await query('ALTER TABLE users ADD COLUMN IF NOT EXISTS hourly_rate numeric(8,2)').catch(() => {});
  const { rows } = await query(
    `SELECT u.id, u.name, u.email, u.phone, u.driver_last_seen, u.hourly_rate,
