  ...
  },
  {
    ...
    input_schema: {
      ...
      properties: {
        ...
        taxInclusive: { type: 'boolean', description: 'Set true when the amounts you are giving ALREADY include the 13% HST — a price quoted "out the door". The tax is backed out for you: the line amounts become the pre-tax figures and the invoice total lands on the price you were told. Default false (amounts are before tax).' },
...
async function create_invoice(input) {
  ...
  const inv = await createAndSendInvoice({
    ...
    // Phone orders are raised by the agent, not a person at a keyboard. Credit
    // them explicitly so they don't read as unattributed on the invoice list.
    createdBy: { email: 'sarah@bargainbay.ca', name: 'Sarah (phone)' },
    // A sale Sarah raises came in over the phone — that is what she is. Stated,
    // not inferred, so these don't land in the lead report's "not recorded"
    // pile. Where the caller HEARD of us is a different question she isn't
    // asked, and leadBy is deliberately left empty rather than guessed.
    leadSource: 'phone'
  });
  return {
    ok: true, invoiceId: inv.id, invoiceNumber: inv.number,
    orderNumber: inv.orderNumber || null,
    // Units that could not be held because something else already has them.
    alreadySpokenFor: inv.contested?.length ? inv.contested : undefined,
    total: money(inv.total), emailedTo: inv.email,
    message: `Invoice ${inv.number} for ${money(inv.total)} emailed to ${inv.email}`
      + (inv.orderNumber ? `, order ${inv.orderNumber} raised and the unit held.` : '.')
  };
}
...
async function void_invoice(input) {
  ...
  return {
    ok: true, invoiceNumber: v.number, orderCancelled: !!v.orderCancelled,
    message: `Invoice ${v.number} voided${v.orderCancelled ? ' — its order is cancelled, the sale is off the dashboard and the unit is relisted.' : '.'}`
  };
}
