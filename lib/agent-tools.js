...
export const TOOLS = [
  ...
  },
  {
    name: 'add_unit',
    description:
      'WRITE — Add newly-acquired unit(s) to inventory as PENDING (held off the storefront) until an owner confirms they are tested-working. Use for vendor purchases (e.g. a SecondShop invoice) or haul-away units to be fixed and resold — no spreadsheet needed. After adding, ASK an owner whether the unit(s) are tested working; once they confirm, call mark_unit_tested to publish at a price. Set source to the vendor name, or "Haulaway" for units acquired free on one of our deliveries (cost 0).',
    input_schema: {
      type: 'object',
      properties: {
        ...
        condition: { type: 'string', description: 'e.g. New Open Box, Scratch & Dent, Refurbished.' },
        cost: { type: 'number', description: 'What we paid per unit in CAD (0 for haul-away).' },
        source: { type: 'string', description: 'Vendor name (e.g. "SecondShop") or "Haulaway".' },
  ...
  },
  {
    ...
    description:
      'WRITE — Confirm a PENDING intake unit is tested-working and PUBLISH it live at a sale price (it becomes invoiceable immediately). Use only after an owner confirms it tested working. Identify the unit by SKU (from add_unit or list_intake).',
    input_schema: {
      type: 'object',
      properties: {
        sku: { type: 'string', description: 'The unit SKU, e.g. "IN-10042".' },
        price: { type: 'number', description: 'Sale price in CAD dollars.' },
        condition: { type: 'string', description: 'Optional updated condition.' }
      },
      required: ['sku', 'price']
    }
  },
  {
    name: 'list_intake',
    description: 'List intake units still PENDING a tested-working decision (SKU, item, cost, source). Read-only.',
    input_schema: { type: 'object', properties: {} }
  },
  {
    name: 'remember',
...
async function add_unit(input) {
  const r = await addIntakeUnits({
    make: input.make, model: input.model, category: input.category,
    condition: input.condition, cost: input.cost, source: input.source, qty: input.qty
  });
  return {
    ok: true, added: r.count, skus: r.created,
    message: `Added ${r.count} pending unit${r.count === 1 ? '' : 's'} (${r.created.join(', ')}) from ${input.source || 'intake'}. They're held off the storefront — confirm they're tested working and I'll publish them with mark_unit_tested.`
  };
}

async function mark_unit_tested(input) {
  const r = await markIntakeTested(input.sku, { price: input.price, condition: input.condition });
  return { ok: true, sku: r.sku, message: `${r.sku} is tested-working, live at ${money(r.price)}, and ready to invoice.` };
}

async function list_intake() {
  const rows = await listIntakePending();
  return { count: rows.length, units: rows.map((u) => ({ sku: u.sku, item: u.title, cost: money(u.cost), source: u.source })) };
}
