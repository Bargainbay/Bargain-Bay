...
import {
  createAndSendInvoice, listInvoices, markInvoicePaid, voidInvoice, refundInvoice, deleteInvoice,
...
export const TOOLS = [
  ...
  {
    name: 'refund_invoice',
    description: 'WRITE — Refund a PAID invoice (customer returned the item / sale reversed). Relists the unit(s) on the storefront and cancels the linked fulfilment order, then marks the invoice refunded. Confirm with the owner first.',
    input_schema: {
      type: 'object',
      properties: {
        invoiceNumber: { type: 'string', description: 'Invoice number like "INV-1042".' },
...
async function create_invoice(input) {
  ...
  for (const it of (Array.isArray(input.items) ? input.items : [])) {
    const line = { description: String(it.description || '').trim(), amount: Number(it.amount), sku: it.sku ? String(it.sku).trim() : undefined };
    if (line.sku) {
      const r = resolve(line.sku, line.description);
      if (r.ok) line.sku = r.sku || line.sku;
      else { unresolved.push({ sku: line.sku, description: line.description, ambiguous: r.ambiguous }); continue; }
    }
    items.push(line);
  }
  if (unresolved.length) {
    return {
      ...
      error: `I couldn't match ${unresolved.map((u) => `"${u.sku}"`).join(', ')} to a unit in inventory — a manufacturer model number is NOT a SKU. ` +
        ...
        `If it's a unit just purchased and not in the tracker yet, ask the owner for the purchase invoice # and the cost, add it with add_unit, then invoice it. I did NOT create this invoice.`
    };
  }
...
const EXECUTORS = {
  ...
  mark_invoice_paid, void_invoice, refund_invoice, delete_invoice, fix_invoice_dashboard, reprice_unit, mark_unit_sold, relist_unit, sync_inventory, remember,
