...
async function refund_invoice(input) {
  ...
  try {
    ...
    return {
      ok: true, invoiceNumber: r.number, status: r.status, unitsRelisted: r.relisted,
      message: `Invoice ${r.number} refunded.` + (r.relisted ? ` ${r.relisted} unit(s) relisted and the linked order cancelled.` : '')
    };
  } catch (e) {
