...
export async function watchFreightcom({ max = 15, dryRun = false } = {}) {
  ...
  for (const em of emails) {
    ...
    try {
      ...
      if (!doc) {
        // NOT silent, and not a failure either: plenty of these are a covering
        // note or a reply. It goes in the report so the desk can see what came
        // in that nothing was read from, and decide whether it mattered.
        attention.push({ bol, subject: em.subject,
          why: atts.length ? `nothing readable attached (${atts.map((a) => a.mimeType || '?').join(', ')})`
                           : 'no attachment at all' });
        continue;
      }

      const { base64 } = await getAttachment(inbox, em.id, doc.attachmentId);
      const read = await extractStopsFromPdf({ base64, mediaType: doc.mimeType });
      ...
      if (!read.rows?.length) { failed.push({ bol, subject: em.subject, why: 'no stop found in the BOL' }); continue; }
...
async function report({ staged, failed, attention }) {
  if (!staged.length && !failed.length && !attention.length) return; // a genuinely quiet run
  const problems = [...failed, ...attention];
  ...
  try {
    await sendEmail({
      ...
      html: `<p>These came in from Parallel and have been read and staged. <b>They are not on the board yet.</b></p>
        <ul>${li(staged, (s2) => `${s2.bol || s2.subject} — ${s2.rows} stop${s2.rows === 1 ? '' : 's'}`)}</ul>
