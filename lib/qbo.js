...
import { upsertExpense, countExpensesBySource, getLedgerStart, loadRules, matchRule } from './finance';
...
export async function qboStatus() {
  ...
  const company = await getSetting('qbo_company', null).catch(() => null);
  return {
    configured: qboConfigured(),
    connected: !!t?.refreshToken,
    company,
    // Which Intuit world these keys point at. Sandbox keys reach a DEMO company
    // full of invented transactions, and importing those into the real expense
    // ledger corrupts the P&L and every input tax credit derived from it — so
    // this is surfaced rather than left as an env var nobody looks at.
    env: process.env.QBO_ENV === 'sandbox' ? 'sandbox' : 'production',
    // Intuit's demo companies are all named this way. Belt and braces: the env
    // var can be changed without the connection being redone, and then the
    // stored tokens still point at the sandbox.
    sandboxCompany: /^sandbox company/i.test(String(company || '')),
    imported: await countExpensesBySource('qbo').catch(() => 0),
...
function mapTxn(entity, t, rules = []) {
  ...
  // An uncategorised QuickBooks gives us a payee and nothing else, which is
  // exactly what the rules are for. QBO's own account name wins when it has one:
  // if the books say Fuel, the books are more specific than a substring match.
  const vendor = t.EntityRef?.name || t.VendorRef?.name || null;
  const hit = matchRule(rules, vendor, category);
  const known = category && !/^uncategori[sz]ed/i.test(category);
  ...
  return {
    ...
    category: String((known ? category : hit?.category) || category || 'Other').slice(0, 80),
    vendor,
...
export async function syncQboExpenses({ days = 35 } = {}) {
  ...
  // Never reach back past the day the books start here — see getLedgerStart.
  // Asking QBO for less is also cheaper than fetching a year and dropping it.
  const start = await getLedgerStart();
  const since = ymd(days) < start ? start : ymd(days);
  ...
  for (const entity of ['Purchase', 'Bill']) {
    ...
    for (;;) {
      ...
      for (const t of rows) {
        const m = mapTxn(entity, t, rules);
        if (!m || !m.incurredOn) continue;
        if (m.incurredOn < start) continue; // belt and braces on the query above
  ...
  return { configured: true, synced, since, ledgerStart: start, errors: errors.slice(0, 5) };
}
