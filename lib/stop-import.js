...
//
// A quote only OPENS a field when it is the field's first character. Anywhere
// else it is a literal — which on an appliance sheet it almost always is:
// `Whirlpool 36" Fridge` is inches. Treating that one as an opening quote
// swallowed every delimiter to the end of the line, so one product description
// silently ate the window, the order type and the notes and shifted the whole
// row left. Same rule the spreadsheets themselves follow.
...
export const IMPORT_FIELDS = [
  // WHOSE work this is — the company we bill, not the person at the door.
  // Sheets that carry it per row are the reason a day's stops used to be filed
  // under the wrong company one card at a time. Kept distinct from "Client
  // Name", which on a consignee sheet means the customer.
  { key: 'clientName', label: 'Client (who we bill)', aliases: ['client', 'account', 'account name', 'bill to', 'billed to', 'billing', 'customer account', 'dealer', 'store', 'on behalf of', 'broker'] },
  ...
  // The business we collect from, and separately the person to ask for. A BOL
  // names both and they are not the same thing.
  { key: 'pickupCompany', label: 'Shipper (company)', aliases: ['shipper', 'shipper name', 'pickup company', 'from company', 'origin name', 'consignor', 'vendor'] },
  { key: 'pickupName', label: 'Pickup contact', aliases: ['pickup contact', 'shipper contact', 'pickup name', 'from name', 'origin contact', 'contact at pickup'] },
...
export function guessMapping(headers) {
  ...
  // Score every field against every column, then hand out the strongest pairs
  // first. Walking the fields in order instead let an EARLIER field take a
  // column on a loose match that a LATER field named exactly — which is how a
  // "Client" field would quietly swallow the "Client Name" column that means
  // the customer. Strength decides, not declaration order.
  const pairs = [];
  IMPORT_FIELDS.forEach((field) => {
    cols.forEach((n, i) => {
      const sc = score(field, n);
      if (sc < Infinity) pairs.push({ key: field.key, col: i, sc });
    });
  });
  pairs.sort((a, b) => a.sc - b.sc);

  for (const p of pairs) {
    if (map[p.key] != null || takenCol.has(p.col)) continue;
    map[p.key] = p.col;
    takenCol.add(p.col);
  }
  return map;
...
// The shape of a sheet, as a stable key.
//
// A client's export looks the same every week — same columns, same order, same
// spelling. Fingerprinting the heading row is what lets the second import of
// that sheet map itself: the profile stored against this key already knows
// which column is which and who it's for. Sorted, so a client who moves a
// column still lands on the same profile; normalized, so capitals and
// punctuation drift doesn't mint a new one.
export function fingerprintHeaders(headers = []) {
  const parts = headers.map((h) => norm(h)).filter(Boolean).sort();
  if (parts.length < 3) return null;          // too thin to identify anything
  const joined = parts.join('|');
  // A short non-cryptographic digest — this is a cache key, not a secret.
  let h1 = 0x811c9dc5;
  let h2 = 0x01000193;
  for (let i = 0; i < joined.length; i++) {
    h1 = Math.imul(h1 ^ joined.charCodeAt(i), 0x01000193) >>> 0;
    h2 = Math.imul(h2 + joined.charCodeAt(i) * (i + 1), 0x85ebca6b) >>> 0;
  }
  return `${h1.toString(36)}${h2.toString(36)}-${parts.length}`;
}
...
export function toJobs(rows, mapping, defaults = {}) {
  ...
  const aliases = defaults.aliasesByClient || {};
  return rows.map((row) => {
    ...
    // Whose work this row is. A client named ON the row beats the one chosen
    // for the whole import, because a sheet that bothers to say it per row is a
    // sheet with more than one company in it. An unconfident read never wins —
    // it becomes a question, and the batch default stands until it's answered.
    const sheetClient = at(row, 'clientName');
    const matched = sheetClient ? matchClient(sheetClient, clients, aliases) : null;
    const rowClientId = matched?.confident ? matched.clientId : null;

    const job = {
      type: 'delivery',
      clientId: rowClientId || defaults.clientId || null,
      ...
      notes: notes || null,
      pickupCompany: pickupAddress ? at(row, 'pickupCompany') : null,
    ...
    const problems = [];
    // Say who it landed under whenever the sheet had an opinion — a silent
    // match is the same experience as a silent mistake.
    if (matched?.confident) {
      problems.push({ kind: 'client_matched', text: `“${sheetClient}” → ${matched.client.name} (${matched.why})`, info: true });
    } else if (sheetClient && matched?.clientId) {
      problems.push({
        kind: 'client_unsure',
        text: `“${sheetClient}” — ${matched.why}. Left as ${defaults.clientName || 'no client'} until you confirm.`,
        suggest: { clientId: matched.clientId, name: matched.client.name, alternatives: matched.alternatives }
      });
    } else if (sheetClient) {
      problems.push({
        kind: 'client_unknown',
        text: `“${sheetClient}” isn’t a client I know.`,
        suggest: { clientId: null, name: sheetClient, alternatives: matched?.alternatives || [] }
      });
    }
    if (qc) {
      problems.push({ kind: 'quebec', text: `Quebec load → pickup only, dropping at ${QUEBEC_DROP.city}`, info: true });
      // Redirected to a cross-dock with nowhere to collect from is not a job.
      if (!job.pickupAddress) problems.push({ kind: 'no_pickup', text: 'no pickup address on the BOL', blocking: true, asks: 'pickupAddress' });
    }
    // The one thing a stop cannot be without.
    if (!job.address) problems.push({ kind: 'no_address', text: 'no address', blocking: true, asks: 'address' });
    if (!job.jobDate) problems.push({ kind: 'no_date', text: 'no date — it will wait in “To assign”', asks: 'jobDate' });
    if (job.windowStart && job.windowEnd && job.windowEnd <= job.windowStart) {
      problems.push({ kind: 'bad_window', text: 'window ends before it starts', asks: 'window' });
      job.windowStart = null; job.windowEnd = null;
    }
    return {
      job,
      problems,
      // What the sheet said, kept alongside what we made of it — the voice
      // agent has to be able to read the row back in the client's own words.
      sheetClient: sheetClient || null,
      blocking: !job.address || (qc && !job.pickupAddress),
      // Anything that isn't purely informational still wants a human's eye.
      needsReview: problems.some((p) => !p.info)
    };
  });
}

// Problems are objects so the voice agent can act on them; every surface that
// only wants to print them goes through here.
export const problemText = (p) => (typeof p === 'string' ? p : p?.text || '');
