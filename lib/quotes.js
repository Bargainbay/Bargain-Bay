...
import { sendQuoteEmail, sendBundleRequestAck } from './email';
...
export async function createAndSendQuote({ name, email, items, bundlePct = 0, cashDeal = null, freeDelivery = false, addHst = true, daysValid = 14, memo, sourceQuoteId = null }) {
  ...
  return { id: quote.id, number: quote.number, total: totals.total, status: 'open', email, hostedUrl: hostedPath(quote.number) };
}
...
export async function convertQuoteToInvoice(quoteId) {
  ...
  const invoice = await createAndSendInvoice({
    name: q.name, email: q.email, items: scaled, addHst: Number(q.hst) > 0, daysUntilDue: 14, memo: memoParts.join(' ')
  });
