...
export const PDF_COLUMNS = [
  ...
  'Province', 'Pickup contact', 'Pickup phone', 'Pickup postal'
];
...
export async function extractStopsFromPdf({ base64, mediaType }) {
  ...
  const instruction =
    ...
    + `{"stops": [[date, customer, phone, address, city, postal, items, notes, pickupAddress, pickupCity, reference, province, pickupContact, pickupPhone, pickupPostal], ...], `
    + `"sender": string|null, "pages": number}\n`
    + `Each stop is a compact 15-element array in EXACTLY that order.\n`
    + `Rules:\n`
    ...
    + `- On a bill of lading the SHIPPER / EXPEDITEUR block is the pickup and the CONSIGNEE / DESTINATAIRE block is the delivery. Fill pickupAddress / pickupCity / pickupPostal / pickupContact / pickupPhone from the shipper block, and address / city / postal / customer / phone from the consignee block. "date" is the pickup date if that is the only date shown.\n`
  ...
  const rows = parsed.stops
    .map((st) => (Array.isArray(st)
      ...
      : PDF_COLUMNS.map((_, i) => [st?.date, st?.customer, st?.phone, st?.address, st?.city,
        ...
        st?.province, st?.pickupContact, st?.pickupPhone, st?.pickupPostal][i])))
