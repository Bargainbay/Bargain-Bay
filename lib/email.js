...
import { money, PICKUP_ADDRESS, SALES_EMAIL, CUSTOMER_SERVICE_EMAIL, ETRANSFER_EMAIL, REVIEW_URL, warrantyLabel,
         BUSINESS_NAME, BUSINESS_LEGAL, BUSINESS_ADDRESS, HST_NUMBER, DISPATCH_EMAIL, SERVICE_EMAIL, RETURN_POLICY_SUMMARY, isUnitLine } from './constants';
...
// A send that didn't land used to be logged once and forgotten, and the office
// found out by NOT getting an email — which is unfalsifiable: nobody can tell
// "the driver never tapped Done" apart from "the mail was dropped". Two answers
// are worth another go rather than a shrug:
//
//  • 429. Resend allows two requests a second. One finished stop can fire three
//    emails (office, client, customer) and the offline queue drains several
//    stops back to back, so the third one over the line is refused — and it is
//    refused for a reason that will be gone in half a second.
//  • 5xx / a timeout. Somebody else's outage, not ours.
//
// A 4xx that isn't 429 is a decision (bad address, unverified domain) and is
// returned as it stands.
const RETRYABLE = (status) => status === 429 || status === 408 || status >= 500;
const pause = (ms) => new Promise((r) => setTimeout(r, ms));

// Retrying costs time, and some of these sends now happen while a driver stands
// in a doorway waiting for Done to go green — the completion route has sixty
// seconds for everything it does. So the retries live inside a hard budget:
// three quick attempts, and the whole thing gives up well before it could become
// the reason a stop failed to close. Normal is one attempt in under a second.
...
// `headers` is additive and almost always absent. It exists for the
// List-Unsubscribe pair on marketing mail (RFC 2369 / RFC 8058), which is what
// renders the native Unsubscribe button in Gmail and Outlook. Transactional
// mail passes nothing and is unaffected — and must NOT carry those headers: an
// order confirmation is not something anybody can unsubscribe from.
export async function sendEmail({ to, subject, html, brand = DEFAULT_BRAND, headers = null }) {
  if (!emailConfigured()) {
    console.log('[email skipped — RESEND_API_KEY not set]', subject);
    return { ok: false, skipped: true, reason: 'RESEND_API_KEY not set' };
  }

  // A staging deployment shares RESEND_API_KEY with production unless somebody
  // remembers otherwise, so without this it emails real customers about orders
  // they did not place. Redirected rather than refused: you still want to see
  // that mail works, and the original recipient is put in the subject so a
  // redirected message is never mistaken for a real one.
  const gate = outbound('email');
  if (!gate.allowed) {
    console.log(`[email not sent — ${gate.reason}]`, subject, '->', to);
    return { ok: false, skipped: true, reason: gate.reason };
  }
  if (gate.redirectTo) {
    subject = `${envBanner()} (to: ${to || NOTIFY()}) ${subject}`;
    to = gate.redirectTo;
  }
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    try {
      const r = await fetch('https://api.resend.com/emails', {
        ...
        // Without this a hung connection holds the whole request open until the
        // function is killed, and the driver watches a spinner for it.
      ...
      console.error('resend send failed', r.status, text.slice(0, 300), '—', subject);
      last = { ok: false, status: r.status, error: text.slice(0, 300) };
      if (!RETRYABLE(r.status)) return last;
    } catch (e) {
      console.error('resend error', e.message, '—', subject);
      last = { ok: false, error: e.message };
    }
  }
  // Three goes and it still didn't leave. Say so plainly — this line is the only
  // trace an email that was owed and never sent leaves behind.
  console.error('resend GAVE UP —', subject, last.status || last.error, `${Date.now() - startedAt}ms`);
  return last;
}
...
// postal, subtotal, hst, total, discount, couponCode. `items` is [{ title, price }, ...].
...
function orderEmailHtml(order, items, { forOwner }) {
  ...
  return `<div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;color:#2e2d2b">
    ...
    <table style="width:100%;border-collapse:collapse;margin:12px 0">${itemRows(items)}
      ...
      ${Number(order.discount) > 0 ? `<tr><td style="padding:6px 0;text-align:right">Promo code ${esc(order.couponCode || '')}</td><td style="padding:6px 0;text-align:right">−${money(order.discount)}</td></tr>` : ''}
...
export function packingSlipRows(items) {
  return (items || [])
    .map((it, i) => {
      const serial = isUnitLine(it.kind) ? (it.serial || '(no serial on file)') : '—';
...
export function packingSlipHtml(slip) {
  ...
  // What we're taking AWAY. The emailed slip is what the warehouse loads from,
  // so a trade-in has to be on it or the van goes out with no room for it.
  const tradeIns = (slip.items || []).filter((it) => it.kind === 'trade_in');
  const tradeInBox = tradeIns.length ? `<div style="margin:14px 0;padding:10px 12px;border:2px solid #2e2d2b;border-radius:6px">
      <div style="font-size:11px;text-transform:uppercase;letter-spacing:.06em;font-weight:bold">Bring back to the warehouse — trade-in</div>
      ${tradeIns.map((t) => `<div style="font-size:14px;margin-top:4px">${esc(t.description)} &nbsp;·&nbsp; Collected ☐</div>`).join('')}
    </div>` : '';
