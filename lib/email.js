...
export async function sendOrderStatusEmail(order, items) {
  ...
  const copy = {
    ...
    ready: {
      ...
      body: pickup
        ? `Good news, ${esc(first)} — order <b>${esc(order.order_number)}</b> is ready to collect at ${esc(PICKUP_ADDRESS)}. <a href="${track}">Book your pickup time here</a> (Mon–Fri, 10am–8pm). Bring photo ID and a vehicle suited to the appliance.`
