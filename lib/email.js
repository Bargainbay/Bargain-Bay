...
export async function sendInvoiceEmail(invoice) {
  ...
  const html = `<div style="font-family:Arial,Helvetica,sans-serif;max-width:580px;margin:0 auto;color:#2e2d2b">
    <table style="width:100%;border-collapse:collapse;border-bottom:2px solid #2e2d2b;padding-bottom:8px">
      <tr>
        <td style="vertical-align:top">
          <div style="font-size:20px;font-weight:bold;text-transform:uppercase;letter-spacing:.06em">${esc(BUSINESS_NAME)}</div>
          <div style="font-size:12px;color:#666">${esc(BUSINESS_LEGAL)} · ${esc(BUSINESS_ADDRESS)}</div>
          <div style="font-size:12px;color:#666">${esc(SALES_EMAIL)} · HST# ${esc(HST_NUMBER)}</div>
        </td>
    ...
    <p style="margin-top:14px">Hi ${esc(first)}, here's your invoice from ${esc(BUSINESS_NAME)}${due ? ` — due <b>${esc(due)}</b>` : ''}.</p>
    ${billToBlock(invoice)}
    ...
    <p style="font-size:13px;color:#666">View this invoice any time: <a href="${SITE_URL}/invoice/${encodeURIComponent(invoice.number)}?t=${linkToken('invoice', invoice.number)}">${SITE_URL}/invoice/${esc(invoice.number)}</a></p>
...
export async function sendInvoicePaidEmail(invoice) {
  ...
  const customer = await sendEmail({ to: invoice.email, subject: `Payment received — invoice ${invoice.number}`, html });
...
export async function sendInvoicePartialPaymentEmail({ number, email, name, amount, method, amountPaid, balance, total }) {
  const first = (name || '').split(' ')[0] || 'there';
  const html = `<div style="font-family:Arial,Helvetica,sans-serif;max-width:580px;margin:0 auto;color:#2e2d2b">
    ...
    <p style="font-size:13px;color:#666">View this invoice any time: <a href="${SITE_URL}/invoice/${encodeURIComponent(number)}?t=${linkToken('invoice', number)}">${SITE_URL}/invoice/${esc(number)}</a></p>
    <p style="font-size:13px;color:#666">Questions? Email ${esc(SALES_EMAIL)} with your invoice number.</p>
  </div>`;
  const customer = await sendEmail({ to: email, subject: `Payment received — ${money(amount)} toward invoice ${number} (${money(balance)} owing)`, html });
