...
export async function sendOrderStatusEmail(order, items) {
  ...
  const copy = {
    confirmed: {
      ...
      body: `Thanks, ${esc(first)} — we've received your payment for order <b>${esc(order.order_number)}</b> and we're preparing it. We'll email again when it's ready for ${pickup ? 'pickup' : 'delivery'}.`
    },
    ready: {
      subject: `Your order ${order.order_number} is ready for ${pickup ? 'pickup' : 'delivery'}`,
      heading: pickup ? 'Ready for pickup' : 'Ready for delivery',
      body: pickup
        ? `Good news, ${esc(first)} — order <b>${esc(order.order_number)}</b> is paid and ready to collect at ${esc(PICKUP_ADDRESS)}. Reply to this email to book a pickup time (bring photo ID and a suitable vehicle).`
        : `Good news, ${esc(first)} — order <b>${esc(order.order_number)}</b> is paid and ready. We'll reach out to ${esc(order.phone || 'you')} to arrange a delivery window.`
    },
    out_for_delivery: {
      subject: `Order ${order.order_number} — ${pickup ? 'pickup scheduled' : 'out for delivery'}`,
      heading: pickup ? 'Pickup scheduled' : 'Out for delivery',
      body: pickup
        ? `Your pickup for order <b>${esc(order.order_number)}</b> is scheduled. See you soon!`
        : `Your order <b>${esc(order.order_number)}</b> is out for delivery today.`
    },
    delivered: {
      subject: `Order ${order.order_number} — ${pickup ? 'picked up' : 'delivered'}`,
      heading: pickup ? 'Picked up' : 'Delivered',
      body: `That's a wrap on order <b>${esc(order.order_number)}</b> — thanks for choosing Bargain Bay! Your unit is covered by our one-year warranty; just reply here if anything comes up.`
    },
    cancelled: {
      subject: `Order ${order.order_number} cancelled`,
      heading: 'Order cancelled',
      body: `Order <b>${esc(order.order_number)}</b> has been cancelled. If you've already sent payment or this is unexpected, reply to this email and we'll sort it out right away.`
    }
  };
  ...
  const html = `<div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;color:#2e2d2b">
    ...
    <p style="font-size:13px;color:#666">Track this order any time: <a href="${track}">${track}</a></p>
    <p style="font-size:13px;color:#666">Questions? Email ${esc(SALES_EMAIL)} with your order number.</p>
  </div>`;
  try {
