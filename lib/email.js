...
import { money, PICKUP_ADDRESS, SALES_EMAIL, ETRANSFER_EMAIL, REVIEW_URL } from './constants';
...
export async function sendOrderStatusEmail(order, items) {
  ...
  const copy = {
    ...
    ready: {
      ...
      body: pickup
        ? `Good news, ${esc(first)} — order <b>${esc(order.order_number)}</b> is ready to collect at ${esc(PICKUP_ADDRESS)}. Reply to this email to book your pick-up time, and bring photo ID and a vehicle suited to the appliance.`
    ...
    delivered: {
      ...
      body: `Thank you for shopping with Bargain Bay! Order <b>${esc(order.order_number)}</b> is ${pickup ? 'all yours' : 'delivered'} and covered by our one-year warranty — just reply here if anything comes up.`,
      extra: reviewCta
    },
    cancelled: {
  ...
  const html = `<div style="font-family:Arial,Helvetica,sans-serif;max-width:560px;margin:0 auto;color:#2e2d2b">
    ...
    ${order.status === 'cancelled' ? '' : `<p style="font-size:13px;color:#666">Track this order any time: <a href="${track}">${track}</a></p>`}
    <p style="font-size:13px;color:#666">Questions? Email ${esc(SALES_EMAIL)} with your order number.</p>
  </div>`;
  try {
