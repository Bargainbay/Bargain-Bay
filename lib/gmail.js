...
// The mailboxes this app may act on. The original list is marked Sensitive on
// Vercel, which means its value cannot be read back — so adding one address to it
// means retyping the whole thing blind and betting Sarah's inboxes on the guess.
// GMAIL_EXTRA_INBOXES exists to add one WITHOUT touching it. Same format, same
// meaning, simply concatenated; deduped so an address in both is harmless.
export function inboxes() {
  const parse = (v) => (v || '').split(',').map((s) => s.trim().toLowerCase()).filter(Boolean);
  return [...new Set([...parse(process.env.SARAH_EMAIL_INBOXES), ...parse(process.env.GMAIL_EXTRA_INBOXES)])];
}
...
// `includeEmpty` returns the matching emails that carry NOTHING readable as well
// as the ones that do, with an empty `attachments`. Off by default, so the
// existing callers (invoice intake) are unchanged: they want documents and an
// email without one is nothing to them.
//
// The Freightcom watcher wants them, and the reason is the whole point of this
// change: an email it never sees is an email it cannot count, report, or tell
// anybody about — which is exactly how "some of them have no PDF" stayed
// invisible while looking, from the board, like no mail had arrived.
export async function listAttachmentEmails(inbox, { query, max = 15, includeEmpty = false } = {}) {
  const mailbox = resolveInbox(inbox);
  const gmail = await gmailFor(mailbox);
  ...
  for (const m of data.messages || []) {
    ...
    walk(msg.payload);
    if (attachments.length || includeEmpty) {
      out.push({ id: m.id, from: h.from || '', subject: h.subject || '', date: h.date || '', attachments });
    }
  }
