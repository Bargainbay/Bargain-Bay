...
import {
  ...
  proposeAction, openPending, takeAction, finishAction, cancelAction
} from './store';
...
function stableSystem(person, playbooks) {
...
export async function runAssistant({ person, text, lang = null, threadId = null, via = 'text' }) {
  ...
  const [history, pending] = await Promise.all([loadHistory(thread.id), openPending(person, thread.id)]);

  // Stable first (cached), per-turn facts after it.
  const system = [
    { type: 'text', text: stableSystem(person, [{ title: 'Company', text: general }, ...books]), cache_control: { type: 'ephemeral' } },
