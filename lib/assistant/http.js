...
import { ensureAssistantSchema, recentMessages, openPending, getBanter, setBanter } from './store';
...
export async function handleTurn(person, turn) {
  ...
  // Just flipping the banter switch — no question attached.
  if (turn.banter !== undefined && !turn.text && !turn.audio) {
    return NextResponse.json({ ok: true, banter: await setBanter(person, truthy(turn.banter)) });
  }
...
export async function handleHistory(person, threadId) {
  ...
  const [messages, pending, banter] = await Promise.all([
    id ? recentMessages(person, id) : [],
    id ? openPending(person, id) : [],
    getBanter(person).catch(() => false)
  ]);
  return NextResponse.json({
    ...
    // Only a driver has one; everyone else gets false and no switch.
    banter,
