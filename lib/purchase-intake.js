...
export async function extractPurchaseInvoice({ base64, mediaType }) {
  ...
  const instruction =
    ...
    `"description" must be a clear, searchable retail product title — brand + size/capacity + appliance TYPE + color + a standout feature — NOT just the model number. ` +
    `e.g. "Blomberg 24-inch Ventless Electric Washer/Dryer Combo, White" or "Whirlpool 30-inch 19.3 cu ft Top-Freezer Refrigerator, White". Always include the appliance type word(s) (washer, dryer, refrigerator, range, dishwasher, etc.).\n` +
