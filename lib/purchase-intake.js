...
import { INTAKE_CATEGORIES as CATEGORIES } from './constants';
...
export async function extractPurchaseInvoice({ base64, mediaType }) {
  ...
  const instruction =
    ...
    `"subtotal", "tax" and "total" come from the invoice's own SUMMARY/footer, not from adding the lines up. ` +
    `"tax" is the sales tax charged TO US (HST/GST/QST/PST — add them together if several are listed); it is a recoverable ` +
    `input tax credit and is the reason these three fields matter. Use 0 for an invoice that plainly charges no tax ` +
    `(a US supplier, an exempt sale), and null only when you genuinely cannot find a summary.\n` +
    ...
    `Ignore non-product lines (shipping, tax, fees, pallets) in "items" — the tax belongs in the "tax" field above, not as a line. If a field is unknown use null. Numbers are plain (no "$", no thousands separators).`;
  ...
  // What the invoice was totalled at, kept separate from the line items: this is
  // the tax half, and it is the biggest input tax credit the business has. A
  // truncated read means the footer may never have been seen, so the money
  // figures are dropped rather than reported as a partial total.
  const money = (v) => { const x = num(v); return x != null && x >= 0 ? Math.round(x * 100) / 100 : null; };
  const totals = truncated
    ? { subtotal: null, tax: null, total: null }
    : { subtotal: money(parsed.subtotal), tax: money(parsed.tax), total: money(parsed.total) };
