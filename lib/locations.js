...
//
// Three tables, provisioned here and mirrored in db/schema.sql:
...
export async function addSpots({ kind, area, code, levels, purpose, note }) {
  ...
  const areaKey = kind === 'zone' ? 'zone' : area;
  if (!AREAS.some((a) => a.key === areaKey)) throw new Error('Pick which part of the warehouse it is in.');
  let codes = [base];
  if (kind === 'rack') {
