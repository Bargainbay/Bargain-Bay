...
export async function runSarah({ messages, readOnly = false }) {
  ...
  const playbook = (await getPlaybook().catch(() => '')) || '';
  let system = SYSTEM_PROMPT + (readOnly ? READONLY_NOTE : '');
  if (playbook.trim()) {
    system += `\n\n=== COMPANY PLAYBOOK (the owner's own training — treat this as the source of truth and answer in line with it) ===\n${playbook.trim()}`;
  }
