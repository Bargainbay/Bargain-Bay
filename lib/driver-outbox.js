...
// with blobs). `photoField` renames the blob part for a route that calls it
...
export async function sendNow(row) {
  try {
    if (row.kind === 'patch') {
      // `url` lets a queued item go somewhere other than the stop list — a
      // shift, a fuel receipt — without a second queue. There is only ever one
      // outbox on this phone, because two would drain in an order nobody
      // controls and a fuel entry could land before the shift it belongs to.
      const res = await fetch(row.url || '/api/driver/jobs', {
        method: row.method || 'PATCH', headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ...row.body, ref: row.ref })
      });
      return res.ok ? 'sent' : verdict(res.status);
    }
    const fd = new FormData();
    if (row.jobId) fd.append('jobId', String(row.jobId));
    ...
    // The field NAME is the item's to choose. A stop's photos go up as `photos`
    // because that is what the jobs route reads; a fuel receipt has to go up as
    // `receipt` because that is what the fuel route reads. Hard-coding `photos`
    // here is how every fuel receipt so far was accepted, uploaded and dropped.
    const field = row.photoField || 'photos';
    (row.photos || []).forEach((b, i) =>
      fd.append(field, b, row.photoField ? `${field}.jpg` : `photo-${i}.jpg`));
    const res = await fetch(row.url || '/api/driver/jobs', { method: 'POST', body: fd });
    return res.ok ? 'sent' : verdict(res.status);
  } catch {
