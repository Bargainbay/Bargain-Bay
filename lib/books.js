...
// `read` is a lib/partial reader — the caller owns it, so one page load or one
// CSV knows which of its sections failed. Falling back to a fresh reader keeps
// the signature usable on its own.
async function rowsFor(section, from, to, read = reader('books')) {
  const safe = (sql, args = []) => read(section, sql, args);
  switch (section) {
    case 'sales':
      return safe(`
...
export async function booksSummary(period = 'month') {
  ...
  for (const key of Object.keys(BOOK_SECTIONS)) {
    ...
    sections[key] = {
      ...
      // PER SECTION, because "0 rows and £0.00" is a real answer for a quiet
      // month and an alarming one for a query that fell over, and the page has
      // to be able to tell the reader which it is looking at.
      failed: read.problems.length > before
    };
  }
...
export async function sectionCsv(section, period = 'month') {
  ...
  const read = reader('books-csv');
  const rows = await rowsFor(section, from, to, read);
  ...
  const csv = [
    // Row 1, where Excel opens. A download that is quietly missing its rows is
    // indistinguishable from a quiet month.
    ...(read.incomplete
      ? [cell(`*** INCOMPLETE — this section could not be read (${read.problems[0].message}). DO NOT FILE FROM THIS FILE. ***`), '']
      : []),
    headers.join(','),
  ...
  return {
    name: `bargain-bay-${section}-${range.f}-to-${range.t}${read.incomplete ? '-INCOMPLETE' : ''}.csv`,
    csv,
    count: rows.length,
    incomplete: read.incomplete
  };
