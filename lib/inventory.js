...
import { photosBySku, listUnitPhotos } from './unit-photos';
...
// Our own photos of these exact units (lib/unit-photos), for the product page's
// "Photos of this exact unit" gallery. ADDITIVE ONLY — exactly like the RS Ops
// gallery beside it. `imageFor` never looks at them, so the card, the buy panel,
// the OG tag and the Meta feed all still lead with the stock picture.
// One query for the whole page; soft-fails to no photos.
async function withOwnPhotos(units) {
  if (!units.length) return units;
  const map = await photosBySku(units.map((u) => u.id));
  if (!map.size) return units;
  return units.map((u) => (map.has(u.id) ? { ...u, photos: map.get(u.id) } : u));
}

export async function getAll(opts = {}) {
  const units = await withOwnPhotos(await loadUnits(opts));
  // RS Ops real unit photos attach as a gallery; stock image stays primary (soft-fails to no-op).
...
export async function getById(id) {
  if (hasDb()) {
    try {
      ...
      if (rows.length) {
        const own = await listUnitPhotos(id);
        const u = { ...num(rows[0]), ...(own.length ? { photos: own } : {}) };
        return withRsopsPhotosOne({ ...u, image: imageFor(u) });
      }
...
export async function getMany(ids) {
  if (hasDb()) {
    try {
      ...
      // No photo join here on purpose: getMany is the CHECKOUT read, it renders
      // no gallery, and the stock picture it does show never depended on them.
