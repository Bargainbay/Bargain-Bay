...
// Our own photos of these exact units (lib/unit-photos), attached BEFORE
// imageFor runs — imageFor prefers them over a manufacturer stock shot, and a
// vendor drop-off listed by the sales floor has no stock shot to fall back on.
...
export async function getMany(ids) {
  if (hasDb()) {
    try {
      ...
      if (rows.length || (await dbPopulated())) {
        const withPhotos = await withOwnPhotos(rows.map(num));
        return withPhotos.map((u) => ({ ...u, image: imageFor(u) }));
      }
    } catch (e) {
