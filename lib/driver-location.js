...
// each ping spends eight), so a backfill goes in slices rather than one statement.
...
export async function recordVehiclePings(vehicleId, pings = []) {
  ...
  for (let i = 0; i < unique.length; i += INSERT_CHUNK) {
    ...
    const { rowCount } = await query(
      `INSERT INTO driver_pings (vehicle_id, lat, lng, speed_kmh, heading, battery_pct, accuracy_m, at)
       VALUES ${tuples.join(',')}
       ON CONFLICT (vehicle_id, at) WHERE vehicle_id IS NOT NULL DO NOTHING`,
      chunk.flatMap((r) => [Number(vehicleId), r.lat, r.lng, r.speed, r.heading, r.battery, r.accuracy, r.at])
    );
    stored += rowCount || 0;
  }
...
export async function driverTrail(userId, { date } = {}) {
  ...
  const day = /^\d{4}-\d{2}-\d{2}$/.test(String(date || '')) ? String(date) : null;
  // The `::timestamp` before AT TIME ZONE is load-bearing — a bare date takes the
  // timestamptz overload and reads the day in the SESSION zone (UTC on Vercel),
  // sliding the trail four hours into the previous evening. See lib/orders.js.
  const { rows } = await query(
