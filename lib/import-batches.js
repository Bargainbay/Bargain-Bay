...
// ── Aliases ─────────────────────────────────────────────────────────────────
// Where a batch came from, when nobody typed it. Holds a stable key for the
// source — `bol:PSC10392` for a Freightcom notification — so the same shipment
// staged twice is refused rather than landing on the board twice. A partial
// unique index, because every hand-made batch has NULL here and NULLs must not
// collide with each other.
let _srcCol = null;
function ensureSourceColumn() {
  if (!hasDb()) return Promise.resolve();
  if (!_srcCol) {
    _srcCol = ensureImportSchema()
      .then(() => query('ALTER TABLE import_batches ADD COLUMN IF NOT EXISTS source_msg_id text'))
      .then(() => query(`CREATE UNIQUE INDEX IF NOT EXISTS idx_import_batches_src
                           ON import_batches (source_msg_id) WHERE source_msg_id IS NOT NULL`))
      .catch((e) => { _srcCol = null; throw e; });
  }
  return _srcCol;
}

// Has this shipment already been staged? Counts a CANCELLED batch too: somebody
// throwing one away is a decision, and re-staging it on the next poll would
// undo it every fifteen minutes.
export async function alreadyStaged(sourceMsgId) {
  if (!hasDb() || !sourceMsgId) return false;
  try {
    await ensureSourceColumn();
    const { rows } = await query(
      'SELECT 1 FROM import_batches WHERE source_msg_id = $1 LIMIT 1', [sourceMsgId]
    );
    return rows.length > 0;
  } catch (e) {
    // Fail CLOSED: if we cannot tell, do not stage. A duplicate stop on the
    // board is worse than a shipment that waits for the next run.
    console.error('alreadyStaged check failed', e.message);
    return true;
  }
}
...
export async function stageBatch({
  headers = [], rows = [], sourceName, readAs = 'paste', jobDate, clientId, createdBy, sourceMsgId
} = {}) {
  if (!hasDb()) throw new Error('Database not configured.');
  await ensureImportSchema();
  if (sourceMsgId) await ensureSourceColumn();
  ...
  const { rows: made } = await query(
    `INSERT INTO import_batches
       (source_name, read_as, client_id, job_date, quebec_rule, headers, rows, mapping, fingerprint,
        created_by, created_by_name, source_msg_id)
    ...
    [clean(sourceName, 200), readAs,
     ...
     fingerprint, author.email, author.name, sourceMsgId || null]
  );
  const id = made[0].id;
