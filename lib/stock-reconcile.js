...
import { recordConsignmentUnit, consignmentFloors } from './consignment';
...
export async function stockForInvoicing({ catalog = [] } = {}) {
  ...
  // Consigned units carry the least they may be sold for. It is a PRICE, not a
  // cost — the rep has to be able to see the line they must not go under — and
  // it is the only figure derived from cost that reaches a sales screen.
  const floors = hasDb() ? await consignmentFloors().catch(() => new Map()) : new Map();
  const out = rows
    ...
    .map((r) => ({
      ...
      model: r.model,
      minPrice: floors.get(r.sku)?.floor || 0,
      consignedTo: floors.get(r.sku)?.vendor || null,
...
export async function bookInForInvoice({ make, model, category, serial, description, rsopsSku, source, vendor, cost, by, byEmail } = {}) {
  ...
  // WHERE IT CAME FROM decides how the row is written, and it is asked rather
  // than guessed. A vendor DROP-OFF (today that is Abi) is stock we hold and do
  // not own: it is marked CONSIGNMENT and booked as money owed the day it sells,
  // exactly as the vendor drop-off screen does — miss that and the sale credits
  // inventory for a unit nobody paid for and the vendor is owed nothing on paper.
  // Anything else is a unit whose purchase invoice has not been uploaded, and is
  // marked NEEDS INVOICE so it stands on Stock gaps until it has a cost.
  const drop = source === 'consignment';
  const vend = String(vendor || '').trim();
  if (drop && !vend) throw Object.assign(new Error('Which vendor dropped it off? We pay them when it sells, so the name has to be on the row.'), { status: 400 });

  const rand = Math.random().toString(36).slice(2, 5).toUpperCase();
  const sku = `${drop ? 'VD' : 'BI'}-${Date.now().toString(36).toUpperCase()}${rand}`;
  const today = torontoDate(new Date());
  const unchecked = held.rsopsError ? ' (RS Ops could not be checked.)' : '';
  await appendTrackerUnits([{
    ...
    vendor: drop ? vend : null,
    invoice: drop ? 'CONSIGNMENT' : NEEDS_INVOICE,
    cost: drop && Number(cost) > 0 ? Number(cost) : null,
    // Only the drop-off carries a status: it is here, it works, it is being sold
    // right now. A unit waiting on its purchase invoice is RS Ops's to grade.
    status: drop ? 'Tested Working' : null,
    notes: drop
      ? `Vendor drop-off booked in from the invoice form by ${by || 'staff'} on ${today} — sold on this invoice. We pay ${vend} when it sells.${unchecked}`
      : `Booked in from the invoice form by ${by || 'staff'} on ${today} — it was on neither the tracker nor RS Ops. Cost unknown until its purchase invoice is uploaded.${unchecked}`
  }]);
  _stockCache = { at: 0, rows: null };

  // Best-effort, and REPORTED if it fails: the tracker row is the record of the
  // appliance and must not be lost over a bookkeeping write, but nothing else
  // would ever say the books don't know we owe for this one.
  let booked = true;
  if (drop) {
    try { await recordConsignmentUnit({ sku, vendor: vend, cost, createdBy: byEmail || null }); }
    catch (e) { booked = false; console.error('consignment row not written', sku, e?.message || e); }
  }
  const desc = String(description || '').trim() || [mk, md, category].filter(Boolean).join(' ');
  return { ok: true, sku, description: `${desc} (${sku})`, from: drop ? 'consignment' : 'new', booked, rsopsUnchecked: !!held.rsopsError };
}
