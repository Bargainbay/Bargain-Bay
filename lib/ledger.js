...
import { reader } from './partial';
import { round2, HST_RATE } from './constants';
import { getSetting, setSetting } from './settings';
import { consignedSkus, consignmentPayments } from './consignment';
...
export const ACCOUNTS = {
  ...
  // Stock we hold and did not buy. The obligation starts when the unit SELLS —
  // that is the arrangement — so it can never be an opening balance of ours and
  // it never touches Inventory (1200), which was never debited for it.
  2150: { name: 'Owed to consignment vendors', type: 'liability', derived: true },
...
// `read` is a lib/partial reader. Passing one in is how trialBalance finds out
// that a section of the journal is MISSING rather than empty — see lib/partial
// for why the trial balance itself cannot tell the difference.
export async function journal(from, to, read = reader('ledger')) {
  if (!hasDb()) return [];
  const out = [];
  ...
  for (const r of await read('invoices raised',
  ...
  for (const r of await read('payments received',
  ...
  for (const r of await read('refunds',
  ...
  //
  //    A CONSIGNED unit is the exception and it is not a rounding detail: its
  //    cost was never debited into Inventory, because we never bought it. Credit
  //    1200 for one and inventory drifts negative by that cost while the money
  //    we now owe its vendor appears nowhere at all. So it credits 2150 instead
  //    — the liability starts on the day the unit sells, which is the deal.
  //
  //    Read per LINE and split in JS rather than joining `consignment_units`:
  //    `safe()` swallows a failed query, so a join against a table that doesn't
  //    exist yet would silently delete COGS from the whole ledger. An unreadable
  //    consignment table degrades to "nothing is consigned", which is what it was
  //    before this existed.
  const consigned = await consignedSkus();
  const cogsByOrder = new Map(); // order_number -> { d, own, cons }
  for (const r of await read('cost of goods sold',
    `SELECT ${LT('o.created_at')}::date AS d, o.order_number, oi.sku,
    ...
    const c = n(r.cost);
    if (!c) continue;
    const key = `${r.d}|${r.order_number || ''}`;
    if (!cogsByOrder.has(key)) cogsByOrder.set(key, { d: r.d, number: r.order_number, own: 0, cons: 0 });
    const acc = cogsByOrder.get(key);
    if (consigned.has(r.sku)) acc.cons += c; else acc.own += c;
  }
  for (const acc of cogsByOrder.values()) {
    const own = round2(acc.own), cons = round2(acc.cons);
    if (own) {
      out.push(entry(acc.d, `Cost of goods — ${acc.number || 'sale'}`, acc.number, [
        { code: '5000', debit: own, credit: 0 },
        { code: '1200', debit: 0, credit: own }
      ]));
    }
    if (cons) {
      out.push(entry(acc.d, `Cost of goods (consignment) — ${acc.number || 'sale'}`, acc.number, [
        { code: '5000', debit: cons, credit: 0 },
        { code: '2150', debit: 0, credit: cons }
      ]));
    }
  }
  ...
  for (const r of await read('stock purchase invoices',
  ...
  for (const r of await read('supplier payments',
  ...
  // 5c. Settling a consignment vendor. Same shape as 5b and for the same reason:
  //     the obligation arose on the day the unit sold, the cash leaves whenever
  //     we actually pay them, and those are routinely different months.
  for (const r of await consignmentPayments(from, to)) {
    const amt = round2(n(r.amount));
    if (!amt) continue;
    out.push(entry(r.paidOn, `Paid consignment vendor — ${r.vendor || r.sku}`, r.sku, [
      { code: '2150', debit: amt, credit: 0 },
      { code: '1000', debit: 0, credit: amt }
    ]));
  }
  ...
  for (const r of await read('operating expenses',
  ...
  for (const r of await read('ad spend',
...
export async function trialBalance(asAt = null) {
  ...
  // One reader for the whole run, so `problems` below describes THIS trial
  // balance. Without it a dropped section is invisible: journal() emits
  // balanced pairs, so losing a whole query loses both sides and debits still
  // equal credits. See lib/partial.
  ...
  return {
    needsOpening: false,
    opening,
    // INCOMPLETE means a section of the journal could not be read. The figures
    // are wrong by an unknown amount and `outOfBalance` will still say 0.00.
