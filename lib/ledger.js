...
// a balance sheet.
//
// WHAT IT ASSUMES, and this is the honest limit: that everything was paid from
// the bank. There is no accounts-payable tracking here, so a supplier invoice
// bought on terms is treated as paid the day it was dated. The consequence is
// specific and checkable — the BANK figure below is derived from documents, not
// observed. Once the TD feed is live, the gap between this and the real balance
// is the measure of what the documents are missing. `bankDrift` exists for
...
export async function journal(from, to) {
  ...
  // 5. Stock bought: value into inventory, the tax on it recoverable.
  for (const r of await safe(
    `SELECT invoice_date AS d, vendor, invoice_number, subtotal, tax, total
    ...
    if (!tot) continue;
    out.push(entry(r.d, `Stock — ${r.vendor || 'supplier'} ${r.invoice_number || ''}`.trim(), r.invoice_number, [
  ...
  for (const r of await safe(
    ...
    if (!tot) continue;
    const code = /wage|payroll|subcontract/i.test(String(r.category || '')) ? '6200' : '6000';
