...
// left to lose by then -- the name it lost is read back out of job_events by
// crewLost), and the constraint stops it recurring whatever writes next.
//
...
export async function dispatchBoard(dateStr) {
  ...
  const everyJob = [...all, ...unscheduled];
...
export async function updateJob(jobId, patch = {}, by) {
  ...
  await withTransaction(async (client) => {
    ...
    // assignJob uses, so the stop's history reads as one story and `crewLost`
    // can find a name that went missing whichever screen took it off.
...
export async function assignJob(jobId, { driverId, driver2Id, jobDate, seq } = {}, by) {
  ...
  // mentioned them. `crewLost` reads exactly this to find the stops it already
  // happened to, so the note has to name the id.
...
// ── A name that went missing off a stop ──────────────────────────────────────
// Ardy's name disappeared off several of Ruban's stops and nobody could say how.
// The how is fixed in three places (see `normalizeCrew`, `resequence` and the
// `jobs_crew_distinct` constraint) — this is the part that finds the ones it
// already happened to.
//
// The columns lost the evidence; `job_events` did not. Every assignment writes a
// row naming the ids on the stop, so a driver who was on it and is not on it now
// is recoverable from the trail.
//
// **It walks the WHOLE trail, not the last entry.** The first cut compared the
// most recent 'assigned' event against the current crew, on the reasoning that a
// deliberate removal writes its own newer event and would mask itself. That test
// could never fire: the accidental drop happens INSIDE assignJob — the guard
// that empties a duplicate second seat runs during an ordinary assign call — so
// the accident writes an 'assigned' event too and looks exactly like a decision.
// Verified against the live board: RS-1023's trail reads "Ruban + Ardy" then
// "Ruban", and the last-entry test returned nothing at all.
//
// So the log cannot tell intent, and this does not pretend to. It reports every
// stop a name came off and leaves the judgement to somebody who knows who was in
// the van. It reports and offers; it never repairs by itself.
export async function crewLost({ from, to } = {}) {
  if (!hasDb()) return { from, to, rows: [] };
  await ensureJobSchema();
  const start = normalizeDate(from, 'start date') || torontoToday();
  const end = normalizeDate(to, 'end date') || start;

  const [jobs, trail] = await Promise.all([
    query(
      `SELECT j.id, j.job_number, j.job_date, j.customer_name, j.status,
              j.driver_id, j.driver2_id
         FROM jobs j
        WHERE j.status <> 'cancelled' AND j.job_date BETWEEN $1::date AND $2::date`,
      [start, end]
    ),
    query(
      `SELECT e.job_id, e.detail, e.at
         FROM job_events e
         JOIN jobs j ON j.id = e.job_id
        WHERE e.event = 'assigned'
          AND j.status <> 'cancelled' AND j.job_date BETWEEN $1::date AND $2::date
        ORDER BY e.job_id, e.at, e.id`,
      [start, end]
    )
  ]);

  const byJob = new Map();
  for (const e of trail.rows) {
    if (!byJob.has(e.job_id)) byJob.set(e.job_id, []);
    // 'driver #12 + #7 · 2026-08-27' — the ids are the durable part of that line.
    byJob.get(e.job_id).push({
      ids: [...String(e.detail || '').matchAll(/#(\d+)/g)].map((m) => Number(m[1])),
      at: e.at
    });
  }
  ...
  for (const j of jobs.rows) {
    const events = byJob.get(j.id) || [];
    if (!events.length) continue;
    const now = [j.driver_id, j.driver2_id].filter(Boolean).map(Number);
    const everWas = [...new Set(events.flatMap((e) => e.ids))];
    const missing = everWas.filter((n) => !now.includes(n));
    if (!missing.length) continue;
    // Restore the last crew that actually had the missing person on it — the
    // assignment before whichever one dropped them, not the oldest on record.
    const best = [...events].reverse().find((e) => missing.some((n) => e.ids.includes(n)));
    // ...but restoring is ADDITIVE. Putting that old crew back wholesale would
    // remove whoever is on the stop today, and most of what lands in this list
    // is an ordinary reassignment, not the bug — a stop that went from Ruban to
    // Saieasan reads here as "Ruban came off it", and a one-click "put Ruban
    // back" would quietly undo a decision somebody made on purpose. So the
    // missing name goes into a FREE seat beside whoever is there now, and when
    // both seats are taken there is nothing safe to offer: a stop is one van
    // with two names on it, and there is no third seat to put anybody in.
    const add = missing[0];
    const restore = !j.driver_id ? { driverId: add, driver2Id: null }
      : !j.driver2_id ? { driverId: j.driver_id, driver2Id: add }
      : null;
    out.push({
      restore,
      addBackId: restore ? add : null,
      id: j.id,
      ...
      driver2Id: j.driver2_id || null,
      wasDriverId: best?.ids[0] || null,
      wasDriver2Id: best?.ids[1] || null,
      bothSeatsTaken: !restore,
      missingIds: missing,
      lostAt: events[events.length - 1]?.at?.toISOString() || null
    });
    [...everWas, ...now].forEach((n) => wanted.add(n));
  }
  if (!out.length) return { from: start, to: end, rows: [] };

  const { rows: people } = await query(
    'SELECT id, COALESCE(NULLIF(name,\'\'), email) AS name FROM users WHERE id = ANY($1::int[])',
    [[...wanted]]
  );
  const nameOf = new Map(people.map((p) => [p.id, p.name]));
  const label = (id) => (id ? (nameOf.get(Number(id)) || `#${id}`) : null);
  ...
  return {
    ...
    rows: out.map((r) => ({
      ...r,
      driverName: label(r.driverId),
      driver2Name: label(r.driver2Id),
      wasDriverName: label(r.wasDriverId),
      wasDriver2Name: label(r.wasDriver2Id),
      addBackName: label(r.addBackId),
      missing: r.missingIds.map(label)
    }))
  };
}
