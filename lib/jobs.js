...
import { STATUS_LABELS as ORDER_STATUS_LABELS, round2, dispatchDesk, isUnitLine, isHaulAwayLine, torontoTime, torontoToday } from './constants';
import { sendEmail, esc } from './email';

// Self-provision, same pattern as the rest of the app: the tables are in
// db/migrations/0001_baseline.sql for a fresh database, and this covers a deploy where the
  ...
  if (!_schema) {
    _schema = query(`
      ...
      ALTER TABLE jobs ADD CONSTRAINT jobs_type_check
        CHECK (type IN (${Object.keys(JOB_TYPES).map((t) => "'" + t + "'").join(', ')})) NOT VALID;
...
export const JOB_TYPES = {
  ...
  pickup: 'Pickup',
  // The last thing on a run: parking the van. It is a stop so that the driver
  // taps Done on it, and that tap is a timestamp for when the day ACTUALLY
  // ended — which is the one thing missing whenever somebody forgets to clock
  // off. It earns nothing and is never billed; see NOT_WORK in dispatch-money.
  return_to_base: 'Return to base'
};

// Types that are not customer work. They ride on the board and on the driver's
// phone like anything else, but they must stay out of every number built from
// stops — a return to base is not a delivery that nobody charged for, and
// counting it as one puts the office looking for a price that will never exist.
export const NON_BILLABLE_TYPES = ['return_to_base'];
...
// Re-exported, not redefined: lib/constants is now the one definition. Seven
// modules and two pages import it from here, so the name stays.
export { torontoToday };
...
export async function dispatchBoard(dateStr) {
  const empty = { date: dateStr, jobs: [], unscheduled: [], drivers: [], clients: [], moneyToConfirm: [], staleCount: 0 };
  ...
  const [balances, tradeIns, pending, queue, stale] = await Promise.all([
    balancesForOrders(orderIds),
    tradeInsForOrders(orderIds),
    pendingCollectionsForJobs(everyJob.map((j) => j.id)),
    pendingCollectionQueue(),
    // Stops from before today that nobody ever closed. Only the number here —
    // the list itself is its own view — but the number has to be on the screen
    // the office already looks at, or nobody will ever go and find it.
    staleStopCount()
  ]);
  for (const j of everyJob) {
    ...
    // Reported by the driver, not yet counted by the office. The balance above
    // still says the money is owing — because until it is confirmed, it is.
    j.pendingCollections = pending.get(j.id) || [];
  }

  // The whole queue rides along with the board so the office sees yesterday's
  // unconfirmed money on today's screen.
  return { date, jobs, unscheduled, drivers: drivers.rows, clients, moneyToConfirm: queue, staleCount: stale };
}
...
// The way back for a stop that is off the board.
//
// Reopen lives on the card, and a cancelled stop has no card any more — so the
// button that undoes a cancellation was only ever reachable from the Cancelled
// column, and that column is gone. A cancelled Bargain Bay order already had a
// second door, typing its order number into Add order; a client job or a
// service call had none at all. This is that door for anything with a job
// number on it.
//
// It comes back on the day it was booked for, not the day being looked at. A
// stop cancelled last Tuesday belongs to last Tuesday until somebody decides
// otherwise, and quietly dragging it onto today would be a scheduling decision
// made by a text box.
export async function reopenJobByNumber(jobNumber, by) {
  if (!hasDb()) throw new Error('Database not configured.');
  await ensureJobSchema();
  // 'RS-1023', 'rs 1023' and a bare '1023' are the same stop to somebody
  // reading a number off a run sheet.
  const typed = String(jobNumber || '').trim();
  if (!typed) throw new Error('Which stop?');
  const digits = typed.replace(/\D/g, '');
  const num = digits ? `RS-${digits}` : typed;
  const { rows } = await query(
    'SELECT id, job_number, status FROM jobs WHERE upper(job_number) = upper($1)',
    [num]
  );
  const j = rows[0];
  if (!j) throw new Error(`No stop called ${num}. Check the number — it looks like RS-1023.`);
  // reopenJob refuses an open job too, but it does it without naming the stop,
  // and the number is the only thing the person typing has to go on.
  if (!['done', 'failed', 'cancelled'].includes(j.status)) {
    throw new Error(`${j.job_number} is already on the board.`);
  }
  const job = await reopenJob(j.id, by);
  return {
    id: job.id,
    jobNumber: job.job_number,
    status: job.status,
    was: j.status,
    jobDate: job.job_date ? job.job_date.toISOString().slice(0, 10) : null
  };
}

// Where the office copy of a dispatch email goes: the coordinator's desk, which
// is what "a shared inbox later" turned into. Still env-overridable, and still
// never a Bargain Bay address. Defined in lib/constants.js so the board, the
// letterhead and the reply-to can't drift apart.
const DISPATCH_INBOX = dispatchDesk;
...
// ── Loose ends ───────────────────────────────────────────────────────────────
// Every stop whose day has been and gone that nobody ever closed.
//
// These have always existed and there has never been anywhere to see them. The
// board is one day at a time, so a stop from last Tuesday is only visible by
// knowing to go back to Tuesday — and nobody goes looking for work they have
// forgotten about. The driver's phone was carrying them instead, every one of
// them, stacked on top of this morning's first delivery; taking them off that
// screen is only half an answer if the office still can't see them.
//
// A stale stop is not a bug to be swept. Each one is a real question with a
// different answer — it happened and nobody tapped Done, it never happened, it
// is still owed to a customer, or it should never have been raised — so this
// lists them with what is known about each and leaves the deciding to a person.
const STALE_LIMIT = 300;

export async function staleStops() {
  if (!hasDb()) return { today: null, rows: [], owing: 0, oldest: null, capped: false };
  await ensureJobSchema();
  const today = torontoToday();
  const { rows } = await query(
    `SELECT j.id, j.job_number, j.job_date, j.type, j.status, j.notes,
            j.customer_name, j.phone, j.address, j.city,
            j.started_at, j.arrived_at, j.time_in, j.time_out,
            j.signature_path IS NOT NULL AS has_signature,
            (SELECT count(*) FROM job_photos p WHERE p.job_id = j.id) AS photo_count,
            -- Money a driver reported taking on a stop nobody ever closed. It is
            -- the one thing on this list that is somebody's cash right now.
            (SELECT COALESCE(SUM(k.amount), 0) FROM job_collections k
              WHERE k.job_id = j.id AND k.status = 'pending') AS reported,
            j.order_id, o.order_number, c.name AS client_name,
            COALESCE(u.name, u.email)   AS driver_name,
      ...
      ORDER BY j.job_date, j.driver_id NULLS FIRST, j.seq NULLS LAST, j.id
      LIMIT ${STALE_LIMIT + 1}`,
    [today]
  );
  // Say so rather than quietly showing 300 of 340. A truncated list of things
  // nobody has dealt with reads exactly like a complete one.
  const capped = rows.length > STALE_LIMIT;
  const kept = capped ? rows.slice(0, STALE_LIMIT) : rows;

  const balances = await balancesForOrders(kept.map((r) => r.order_id));
  const day = (d) => (d ? d.toISOString().slice(0, 10) : null);
  const out = kept.map((r) => {
    const bal = r.order_id ? balances.get(r.order_id) : null;
    const on = day(r.job_date);
    return {
      id: r.id, jobNumber: r.job_number, date: on,
      daysAgo: on ? Math.round((new Date(`${today}T12:00:00Z`) - new Date(`${on}T12:00:00Z`)) / 86400000) : null,
      type: r.type, status: r.status,
      customerName: r.customer_name || null, phone: r.phone || null,
      ...
      // The clock, because it is the loudest evidence of what actually happened:
      // a stop clocked in and never out is one somebody DID and forgot to close.
      timeIn: r.time_in ? r.time_in.toISOString() : (r.arrived_at ? r.arrived_at.toISOString() : null),
      timeOut: r.time_out ? r.time_out.toISOString() : null,
      leftAt: r.started_at ? r.started_at.toISOString() : null,
      hasSignature: !!r.has_signature,
      photoCount: Number(r.photo_count) || 0,
      reported: round2(Number(r.reported) || 0),
  ...
  return {
    ...
    rows: out,
    capped,
    // What is still owing across the lot of it. A stale stop with a balance on it
    // is a delivery the customer may already have — and money nobody is chasing.
    owing: round2(out.reduce((a, r) => a + (r.balanceDue || 0), 0)),
    oldest: out.length ? out[0].date : null
  };
}

// Just the number, for the tab. Cheap enough to run with every board load, and
// a count nobody can see is a count nobody acts on.
export async function staleStopCount() {
  if (!hasDb()) return 0;
  try {
    const { rows } = await query(
      `SELECT count(*)::int AS n FROM jobs
        WHERE job_date IS NOT NULL AND job_date < $1::date
          AND status NOT IN ('done', 'failed', 'cancelled')`,
      [torontoToday()]
    );
    return rows[0]?.n || 0;
  } catch (e) {
    console.error('stale stop count failed', e.message);
    return 0;
  }
}

// What a driver SAYS they came back with, on stops that are on this board.
//
// Reads only — nothing here touches an invoice. The money becomes a payment in
// lib/door-money.js, when somebody in the office confirms it; until then this is
// what makes an unconfirmed $500 visible instead of it looking like a customer
// who simply hasn't paid.
export async function pendingCollectionsForJobs(jobIds = []) {
  const ids = [...new Set(jobIds.filter((n) => Number.isFinite(Number(n))).map(Number))];
  if (!hasDb() || !ids.length) return new Map();
  try {
    const { rows } = await query(
      `SELECT id, job_id, invoice_number, amount, method, note, collected_at, by_name
    ...
    for (const r of rows) {
      ...
      out.get(r.job_id).push(shapeCollection(r));
    }
    return out;
  } catch (e) {
    console.error('pending collection lookup failed', e.message);
    return new Map();
  }
}

// Everything waiting on the office, whatever day it was collected. Deliberately
// NOT scoped to the board's date: money reported at 7pm is confirmed the next
// morning, and a queue that empties itself at midnight is a queue that loses
// money.
export async function pendingCollectionQueue() {
  if (!hasDb()) return [];
  await ensureJobSchema();
  try {
    const { rows } = await query(
      `SELECT c.id, c.job_id, c.invoice_number, c.amount, c.method, c.note,
              c.collected_at, c.by_name,
              j.job_number, j.customer_name, j.job_date, j.status AS job_status,
              o.order_number
    ...
    console.error('pending collection queue failed', e.message);
    return [];
  }
}
...
async function jobFromOrder(o, by) {
  ...
  // A haul-away the customer PAID for is the other thing that comes back on the
  // van, and until now nothing told the crew about it: it is an ordinary service
  // line, so it is neither cargo nor a trade-in and the job came off the pull
  // with no tag at all. Recognised by its text (isHaulAwayLine) because there is
  // no line kind for it — the money goes the normal way, only the truck doesn't.
  const haulAways = its.filter((r) => isHaulAwayLine(r.kind, r.title));
  return createJob({
    ...
    services: [...(tradeIns.length ? ['trade_in'] : []), ...(haulAways.length ? ['haul_away'] : [])],
    notes: `Bargain Bay order ${o.order_number}.`
      + (tradeIns.length
        ? ` TRADE-IN TO COLLECT: ${tradeIns.map((r) => r.title).filter(Boolean).join('; ')}.`
        : '')
      + (haulAways.length
        ? ` HAUL-AWAY PAID FOR: ${haulAways.map((r) => r.title).filter(Boolean).join('; ')}.`
        : ''),
    createdBy: by
  });
}
...
export async function importOneBargainBayOrder(orderNumber, { by, address, city, postal } = {}) {
  ...
  const o = rows[0];
  // The box this comes back to takes stop numbers as well now, so a typo that
  // is neither should say what both look like.
  if (!o) throw new Error(`No order called ${num}. Check the number — an order looks like BB-1179, a stop looks like RS-1023.`);
...
// Whose work a stop is, on many stops at once.
//
// This exists because of one specific morning: a client's whole spreadsheet
// went onto the board under the wrong company, and the only way back was the
// edit form, one card at a time, for a day's worth of stops. The importer no
// longer lets that happen — but stops that are already on the board still have
// to be fixable, and "open thirty cards" is not a repair.
//
// An INVOICED job is refused rather than skipped quietly: its client is who was
// billed, and moving it would take a line off one company's invoice and put it
// on another's. The refusal names them so the rest can still go through.
export async function setJobsClient(jobIds, clientId, by) {
  if (!hasDb()) throw new Error('Database not configured.');
  await ensureJobSchema();
  const ids = [...new Set((Array.isArray(jobIds) ? jobIds : []).map((n) => Number(n)).filter(Number.isFinite))];
  if (!ids.length) throw new Error('No stops were selected.');
  const cid = clientId ? Number(clientId) : null;

  if (cid) {
    const { rows: c } = await query('SELECT name FROM clients WHERE id = $1', [cid]);
    if (!c.length) throw new Error('That client no longer exists.');
  }

  const { rows: current } = await query(
    'SELECT id, job_number, invoice_id FROM jobs WHERE id = ANY($1::int[])', [ids]
  );
  const invoiced = current.filter((r) => r.invoice_id);
  const movable = current.filter((r) => !r.invoice_id).map((r) => r.id);

  let changed = [];
  if (movable.length) {
    const { rows } = await query(
      'UPDATE jobs SET client_id = $2 WHERE id = ANY($1::int[]) RETURNING id, job_number',
      [movable, cid]
    );
    changed = rows;
    const label = cid
      ? (await query('SELECT name FROM clients WHERE id = $1', [cid])).rows[0]?.name
      : 'no client';
    await withTransaction(async (client) => {
      for (const r of changed) await logEvent(client, r.id, 'edited', `client set to ${label}`, by);
    });
  }

  return {
    changed: changed.length,
    jobNumbers: changed.map((r) => r.job_number),
    refused: invoiced.map((r) => ({ jobNumber: r.job_number, reason: 'already on an invoice' })),
    missing: ids.length - current.length
  };
}
...
export async function invoiceClientJobs(clientId, { from, to, addHst = true, sendEmail = false } = {}, by) {
  ...
  if (!client.contactEmail) {
    throw new Error(`${client.clientName} has no contact email — add one under People & access, then invoice them.`);
  }
