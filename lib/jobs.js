...
export async function updateJob(jobId, patch = {}, by) {
  ...
  const { rows: cur } = await query(
    'SELECT id, ticket_id, status, driver_id, driver2_id FROM jobs WHERE id = $1', [id]
  );
...
// The how is now fixed in three places (see `normalizeCrew`, `resequence` and
// the `jobs_crew_distinct` constraint) — this is the part that puts it back.
//
// The evidence survived even though the columns didn't: every assignment writes
// a `job_events` row naming the ids on the stop. So the test for "somebody came
// off this stop without anyone assigning them off it" is exact — the LAST
// 'assigned' event names a driver the job no longer carries. A deliberate
// removal writes its own newer 'assigned' event and is therefore invisible here,
// which is the whole point.
//
// It reports; it does not repair. Putting a name back is an assignment like any
// other and goes through `assignJob`, one stop at a time, by somebody who knows
// who was actually in the van.
export async function crewLost({ from, to } = {}) {
  ...
  const { rows } = await query(
    `SELECT j.id, j.job_number, j.job_date, j.customer_name, j.status,
            ...
            e.detail AS last_assigned, e.at AS assigned_at
       FROM jobs j
       JOIN LATERAL (
         SELECT detail, at FROM job_events
          WHERE job_id = j.id AND event = 'assigned'
  ...
  for (const r of rows) {
    ...
    const was = [...String(r.last_assigned || '').matchAll(/#(\d+)/g)].map((m) => Number(m[1]));
    if (!was.length) continue;
    const now = [r.driver_id, r.driver2_id].filter(Boolean).map(Number);
    const missing = was.filter((n) => !now.includes(n));
    if (!missing.length) continue;
    out.push({
      id: r.id,
      jobNumber: r.job_number,
      date: r.job_date ? r.job_date.toISOString().slice(0, 10) : null,
      customerName: r.customer_name,
      status: r.status,
      driverId: r.driver_id || null,
      driver2Id: r.driver2_id || null,
      // The crew as it was last actually assigned — what the Put it back button
      // restores, both seats, in the order they were in.
      wasDriverId: was[0] || null,
      wasDriver2Id: was[1] || null,
      missingIds: missing,
      assignedAt: r.assigned_at ? r.assigned_at.toISOString() : null
    });
    [...was, ...now].forEach((n) => wanted.add(n));
  }
