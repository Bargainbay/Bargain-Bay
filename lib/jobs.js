...
import { STATUS_LABELS as ORDER_STATUS_LABELS } from './constants';
...
// How far back the report looks when explaining what it skipped. Old finished
// business isn't a problem anyone needs listed every morning.
const SKIP_REPORT_DAYS = 60;
...
export async function importReadyBargainBayOrders({ by } = {}) {
  ...
  const { rows } = await query(
    `SELECT o.id, o.order_number, o.name, o.email, o.phone,
            ...
            o.created_at,
            EXISTS (SELECT 1 FROM jobs j WHERE j.order_id = o.id) AS has_job,
            (o.created_at > now() - make_interval(days => $1)) AS recent
       FROM orders o
  ...
  const skipped = [];
  let onBoard = 0;
  // Every eligible order is imported however old it is; only the EXPLANATION of
  // what was left behind is windowed, so the report stays about today's work.
  const skip = (o, reason) => { if (o.recent) skipped.push({ order: o.order_number, reason }); };
  ...
  for (const o of rows) {
    if (o.has_job) {
      ...
      if (o.recent) onBoard += 1;
      continue;
    }
    if (o.delivery_method !== 'delivery') {
      skip(o, 'marked Pickup — switch the order to Delivery if it needs a driver');
      continue;
    }
    if (!String(o.address || '').trim()) {
      skip(o, 'no address on the order — add one under Operations');
      continue;
    }
    if (!IMPORTABLE_ORDER_STATUSES.includes(o.status)) {
      ...
      skip(o, o.status === 'pending_payment' && !inv
        ...
        : `still ${ORDER_STATUS_LABELS[o.status] || o.status}${inv ? ` (deposit taken, ${inv.invoiceNumber || 'invoice'} still owing)` : ''}`
          + " — set it to Ready when it's good to go");
      continue;
    }
    try {
      ...
      skip(o, e.message || 'could not be added');
    }
  }
  return {
