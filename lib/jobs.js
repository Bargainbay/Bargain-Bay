...
export async function importReadyBargainBayOrders({ by } = {}) {
  ...
  const skip = (o, reason, canForce = true) =>
    skipped.push({ order: o.order_number, reason, canForce });
  ...
  for (const o of rows) {
    ...
    if (!String(o.address || '').trim()) {
      skip(o, 'no address on the order — add one under Operations first', false);
      continue;
    }
...
export async function importOneBargainBayOrder(orderNumber, { by } = {}) {
  if (!hasDb()) throw new Error('Database not configured.');
  await ensureJobSchema();
  ...
  const o = rows[0];
  if (!o) throw new Error(`No order called ${num}.`);
  if (o.live_job) throw new Error(`${o.order_number} is already on the board as ${o.live_job}.`);
  if (!String(o.address || '').trim()) throw new Error(`${o.order_number} has no address on it — add one under Operations first.`);
  const job = await jobFromOrder(o, by);
