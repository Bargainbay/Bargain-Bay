...
export async function dispatchBoard(dateStr) {
  const empty = { date: dateStr, jobs: [], unscheduled: [], drivers: [], clients: [], moneyToConfirm: [] };
  ...
  const [balances, tradeIns, pending, queue] = await Promise.all([
  ...
  return { date, jobs, unscheduled, drivers: drivers.rows, clients, moneyToConfirm: queue };
}
