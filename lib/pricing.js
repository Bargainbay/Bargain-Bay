...
function memberRegular(unit) {
  ...
  // Fallback for units missing from member-prices.json: a % of retail, but
  // never MORE than the public price — otherwise a unit already listed below
  // the fallback rate would charge approved members more than regular shoppers.
  const retail = unit.compareAt || unit.price || 0;
  const fallback = Math.round(retail * MEMBER_REGULAR_FALLBACK) || unit.price;
  return Math.min(fallback, unit.price);
}
