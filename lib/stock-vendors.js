...
import { consignmentFloor } from './constants';
...
export function groupRows(rows = []) {
  ...
  for (const r of rows) {
    ...
    const v = vendors.get(key) || {
      ...
      retail: 0, cost: 0, consigned: 0, costMissing: 0, belowFloor: 0, oldest: ''
    };
    ...
    // A dropped-off unit the tracker prices UNDER cost + 20%. The storefront
    // won't sell it that low (lib/pricing clamps it) and an invoice line under
    // the floor is refused — but the tracker row is where it gets fixed, by
    // moving Retail or the Condition that multiplies it.
    const consigned = isConsigned(r.invoice);
    const floor = consigned ? consignmentFloor(num(r.cost)) : 0;
    const under = floor > 0 && num(r.price) > 0 && num(r.price) + 0.005 < floor;
    if (under) v.belowFloor += 1;
    v.units.push({
      ...
      lot: r.lot, consigned, invoice: r.invoice, floor, belowFloor: under
    });
    vendors.set(key, v);
  }
  ...
  return {
    ...
    totals: {
      ...
      belowFloor: vendorList.reduce((a, v) => a + v.belowFloor, 0)
    }
  };
}
