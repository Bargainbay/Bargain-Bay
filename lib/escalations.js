...
// Fallback when the owner answers in the management group WITHOUT using Telegram's
// reply: return the open escalation only if there's exactly one (so a free-typed
// answer is unambiguous). Returns null when zero or multiple are open.
export async function openSingleEscalation() {
  if (!hasDb()) return null;
  try {
    await ensure();
    const { rows } = await query("SELECT * FROM escalations WHERE status = 'open' ORDER BY id DESC LIMIT 2");
