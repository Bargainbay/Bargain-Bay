...
import {
  balancesForOrders, tradeInsForOrders, pendingCollectionsForJobs, torontoToday, ensureJobSchema,
...
// A driver's working list for ONE DAY. Today by default; any earlier day the
// driver picks, because "what did we do Tuesday" is a real question a driver
// gets asked at a door and had no way to answer.
//
// Four buckets, and they are separate arrays on purpose — a screen that mixes
// them is the screen this replaced. `stops` is that day and nothing else: it
// used to carry every unfinished stop from every earlier day inline with the
// day's work, so a driver opened the app to last week's leftovers sitting above
// this morning's first delivery, and the "3 to go" count at the top was
// counting them.
//
//   · stops     — the day being looked at
//   · tomorrow  — the day after (only when that day is TODAY: you plan the night
//                 before, and nothing here can be started, finished or failed)
//   · earlier   — stops from before today that nobody ever closed (only when
//                 looking at today). Still reachable, because a stop that
//                 silently drops off at midnight is how a delivery is forgotten
//                 for a week — but held apart, and counted apart.
//   · closedBy  — who actually closed each one. Two drivers ride one stop; when
//                 the mate finishes it, the other man's phone had no way of
//                 saying so and it just looked like it hadn't gone anywhere.
export async function driverJobs(userId, { date } = {}) {
  if (!hasDb() || !userId) return { date: date || null, today: null, viewingToday: true, stops: [], tomorrow: [], earlier: [] };
  await ensureDriverPodSchema();
  const today = torontoToday();
  // A driver can look back, not forward: tomorrow's work is already its own
  // list, and a day that hasn't happened has nothing to report.
  const asked = /^\d{4}-\d{2}-\d{2}$/.test(String(date || '')) ? String(date) : today;
  ...
  const { rows } = await query(
    `SELECT j.id, j.job_number, j.type, j.status, j.seq, j.job_date,
            ...
            j.pickup_company, j.pickup_name, j.pickup_phone, j.driver_id, j.driver2_id,
            ...
            j.signature_path,
            -- Who tapped Done (or "couldn't complete"). One stop, two drivers,
            -- one of them closes it: the other has to be told which, or the
            -- card just looks stuck.
            (SELECT e.by_name FROM job_events e
              WHERE e.job_id = j.id AND e.event IN ('done', 'service_complete', 'failed')
              ORDER BY e.at DESC, e.id DESC LIMIT 1) AS closed_by,
            o.order_number,
      ...
      WHERE (j.driver_id = $1 OR j.driver2_id = $1)
        ...
        AND (j.job_date = $2
             -- Tomorrow to plan with, and the stops nobody ever closed — both
             -- of them belong to TODAY. Looking back at last Tuesday returns
             -- last Tuesday and nothing else.
             OR ($3 AND j.job_date = ($2::date + 1))
             OR ($3 AND j.job_date < $2 AND j.status NOT IN ('done','failed')))
      ORDER BY j.job_date, j.seq NULLS LAST, j.window_start NULLS LAST, j.id`,
    [Number(userId), day, day === today]
  );

  const orderIds = rows.map((r) => r.order_id);
  const [balances, tradeIns, reported] = await Promise.all([
    balancesForOrders(orderIds),
    tradeInsForOrders(orderIds),
    // What this driver has already said they took here. The balance behind it
    // stays owing until the office confirms the money — so without this the
    // phone would go on shouting COLLECT $500 at a stop they had just handed
    // five hundred dollars over for.
    pendingCollectionsForJobs(rows.map((r) => r.id))
  ]);
  const shape = (r) => {
    ...
    return {
      ...
      pickupCompany: r.pickup_company, pickupName: r.pickup_name, pickupPhone: r.pickup_phone,
      ...
      // Reported at the door, waiting on the office. Not deducted from the
      // balance: nothing has been paid off until somebody confirms it.
      reported: (reported.get(r.id) || []).reduce((a, c) => a + (Number(c.amount) || 0), 0),
  ...
  const on = (r) => (r.job_date ? r.job_date.toISOString().slice(0, 10) : null);
  // Looking at an earlier day is looking at the RECORD of it. Tomorrow's
  // planning list and the unfinished pile both belong to today, and showing
  // them stapled to last Tuesday would make that day's page a lie.
  ...
  return {
    ...
    stops: rows.filter((r) => on(r) === day).map(shape),
    tomorrow: isToday ? rows.filter((r) => on(r) === dayAfter).map(shape) : [],
    earlier: isToday ? rows.filter((r) => on(r) && on(r) < day).map(shape) : []
  };
}
...
//
// The status test is the whole point, and it was missing. `pod_ref` is stamped
// on the way IN, before the photos are uploaded and before the job is closed —
// so if anything after it failed (a blob upload, a timeout on a run of eight
// pictures over one bar of LTE), the phone's retry was answered "already done",
// the queue dropped it, and the stop was never completed and never emailed.
// Nobody found out: the driver's screen said Done and the board said nothing.
//
// A ref that is stamped on a stop that never reached done/failed is therefore
// NOT a replay — it is unfinished business, and the retry is allowed through to
// finish it. Stops already jammed this way heal on the next attempt.
export async function podAlreadyRecorded(jobId, ref) {
  if (!hasDb() || !ref) return false;
  await ensureDriverPodSchema();
  const { rows } = await query(
    "SELECT 1 FROM jobs WHERE id = $1 AND pod_ref = $2 AND status IN ('done','failed')",
    [Number(jobId), String(ref)]
  );
  return rows.length > 0;
}
...
// How many of this batch already landed. A completion that failed halfway
// through eight photos is retried with all eight, and without this the ones that
// did make it are stored a second time — the customer's proof of delivery ends
// up with the same picture three times over.
export async function jobPhotosWithRef(jobId, ref) {
  if (!hasDb() || !ref) return 0;
  await ensureDriverPodSchema();
  const { rows } = await query(
    'SELECT count(*)::int AS n FROM job_photos WHERE job_id = $1 AND ref = $2', [Number(jobId), String(ref)]
  );
  return rows[0]?.n || 0;
}
...
export async function markOrderDeliveredForJob(jobId) {
  ...
  try {
    ...
    if (updated) {
      // Awaited. This was the LAST thing before the route returned, so the send
      // was in a straight race with the response — and losing it meant a
      // customer who was told nothing about a delivery that had already
      // happened. Still best-effort: the catch keeps a mail problem out of the
      // driver's way.
      await sendOrderStatusEmail({ ...updated, status: 'delivered' }, items.map((r) => ({ title: r.title, price: Number(r.price) })))
