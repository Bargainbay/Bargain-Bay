...
export function isAdmin(session) {
  ...
  return emailList(process.env.ADMIN_EMAILS).includes(session.email.toLowerCase());
}

// Sales associates: SALES_EMAILS (comma-separated), same shape as ADMIN_EMAILS.
// They get the selling surfaces only — sales dashboard, quotes, invoices — and
// never see cost/profit, payroll, financial, marketing, or operations.
// An admin is implicitly sales too, so admin checks never regress.
export function isSales(session) {
  if (!session?.email) return false;
  if (isAdmin(session)) return true;
  return emailList(process.env.SALES_EMAILS).includes(session.email.toLowerCase());
}
