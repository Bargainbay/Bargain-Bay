...
const ORDER_MATCH = `(
  ...
  OR (c.email IS NULL AND c.phone_key IS NOT NULL
      AND '+1' || right(regexp_replace(coalesce(o.phone,''), '\\D', '', 'g'), 10) = c.phone_key)
)`;
...
export async function getCustomerProfile(id) {
  ...
  const BY = (t) => `(
    ...
    OR ($1::text IS NULL AND $2::text IS NOT NULL
        AND '+1' || right(regexp_replace(coalesce(${t}.phone,''), '\\D', '', 'g'), 10) = $2)
  )`;
  const args = [customer.email || null, customer.email ? null : (customer.phoneKey || null)];
  ...
  const [orders, invoices, quotes] = await Promise.all([
    ...
    query(
      ...
      `SELECT id, number, status, total, created_at
         FROM quotes WHERE $1::text IS NOT NULL AND lower(email) = $1
