...
import { readAvailableReport, sheetsConfigured, writebackEnabled, writeSoldRows, writeUnsoldRows } from './sheets';
...
export async function upsertProducts(units) {
  ...
  return withTransaction(async (client) => {
    ...
    // Deactivate units no longer in the tracker — BUT guard against a bad/partial
    // read silently dark-shipping the whole store. An empty read never wipes, and a
    // single sync won't deactivate a suspiciously large share of a non-trivial
    // catalog (almost always a truncated/failed read, not real sell-through).
    const { rows: ab } = await client.query('SELECT COUNT(*)::int AS c FROM products WHERE active = true');
    const activeBefore = ab[0].c;
    let deactivated = 0;
    let skippedDeactivation = false;
    if (clean.length === 0) {
      skippedDeactivation = true; // an empty read is an error, not "everything sold"
    } else {
      const { rows: td } = await client.query(
        'SELECT COUNT(*)::int AS c FROM products WHERE active = true AND NOT (sku = ANY($1))',
        [skus]
      );
      if (activeBefore >= 10 && td[0].c > activeBefore * 0.6) {
        skippedDeactivation = true; // would drop >60% in one sync — refuse, keep store up
      } else {
        const del = await client.query(
          'UPDATE products SET active=false, synced_at=now() WHERE active=true AND NOT (sku = ANY($1))',
          [skus]
        );
    ...
    if (skippedDeactivation) {
      console.warn(`sync guard: skipped deactivation (read ${clean.length} units, ${activeBefore} active) — likely a partial/failed tracker read`);
    }
    return { synced, deactivated, total: clean.length, skippedDeactivation };
  });
}
...
// `prices` (optional) is a { sku: unitPrice } map for the precise per-unit sold
// price (each invoice/order line carries its own price); falls back to `price`.
export async function markUnitsSold(skus, { channel = 'manual', ref = null, price = null, prices = null } = {}) {
  ...
  // Precise per-unit sold price where the caller supplied a line-item map.
  if (prices) {
    for (const sku of list) {
      const p = prices[sku];
      if (p != null) await query('UPDATE products SET sold_price = $2 WHERE sku = $1', [sku, p]).catch(() => {});
    }
  }
  ...
  // Write the sale back to the master tracker (Status=Sold + per-unit Sold Price
  // + Date Sold) when write-back is on. Best-effort — never block the sale.
  if (writebackEnabled()) {
    try {
      await writeSoldRows(list.map((sku) => ({ sku, price: prices?.[sku] ?? price ?? null })));
    } catch (e) { console.error('tracker Sold write-back failed', e.message); }
  }
  return { sold: res.rowCount };
}

// Reverse a sale on the master tracker after a refund/relist. The sale path
// writes Sold back to the sheet, but nothing wrote the reversal — so a refunded
// unit stayed "Sold" there while Postgres had it active, and the next tracker
// sync dutifully deactivated it again. Best-effort: a sheet failure must never
// block a refund.
export async function reverseTrackerSale(skus) {
  const list = [...new Set((skus || []).map((s) => String(s || '').trim()).filter(Boolean))];
  if (!list.length || !writebackEnabled()) return { written: 0, missing: [] };
  try {
    const r = await writeUnsoldRows(list);
    if (r.missing?.length) console.error('tracker un-sold write-back: no Item ID match for', r.missing.join(', '));
    return r;
  } catch (e) {
    console.error('tracker un-sold write-back failed', e.message);
    return { written: 0, missing: list, error: e.message };
  }
}
...
export async function reactivateUnit(sku) {
  ...
  if (!s) return { ok: false, relisted: 0, cancelledOrders: 0 };
  ...
  // ok reflects whether a product actually flipped back to active — a non-existent
  // SKU updates zero rows and must not report success.
  return { ok: upd.rowCount > 0, relisted: upd.rowCount, cancelledOrders: ords.rowCount };
}
...
export async function syncInventoryFromTracker() {
  ...
  // `report` rides along so the caller can say what was skipped and why. A sync
  // that silently imports less than the tracker holds is indistinguishable from
  // a sync that worked.
  const { units, report } = await readAvailableReport();
