...
export async function ensureAttributionColumns() {
  ...
  ensured = Promise.all([
    ...
    // How the customer reached us, as STATED — a walk-in, a referral, the
    // website — and who sent them. `source` above is the machine's answer and
    // only exists for a web visit; this is the human's, and it is the only one
    // that can describe the person who walked through the door. Separate
    // columns so neither can overwrite the other. See LEAD_SOURCES.
    query('ALTER TABLE orders ADD COLUMN IF NOT EXISTS lead_source text'),
    query('ALTER TABLE orders ADD COLUMN IF NOT EXISTS lead_by text'),
    query('CREATE INDEX IF NOT EXISTS idx_orders_lead_source ON orders(lead_source)')
