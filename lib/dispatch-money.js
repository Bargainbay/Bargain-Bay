...
import { round2, TZ, MAX_SHIFT_HOURS } from './constants';
...
export const EXPENSE_KINDS = {
  gas: 'Gas',
  // The carrier's fortnightly invoice for the 20ft box truck — the truck AND
  // its diesel in one bill. It is its own kind because it is the thing that
  // makes a carrier truck's fills non-costing: the money is here, once.
  carrier: 'Carrier bill (truck + fuel)',
  tolls: 'Tolls / 407',
  parking: 'Parking',
...
function shapeExpense(r) {
  return {
    ...
    // What a driver adds at the pump. The office reviews these rather than
    // re-typing them, which is the whole reason the driver enters them.
...
export async function listExpenses({ from, to } = {}) {
  ...
  const { rows } = await query(
    `SELECT e.*, COALESCE(u.name, u.email) AS driver_name, v.name AS vehicle_name
...
const ORDER_FEE = `
  ...
  END`;
const JOB_REVENUE = `
  CASE WHEN j.status = 'done' THEN COALESCE(j.charge_amount, ${ORDER_FEE}, 0) ELSE 0 END`;
// A stop is priced if somebody typed a charge, or if its order really did carry
// a delivery fee. **A fee of zero is not a price**, it is the arithmetic finding
// nothing — and on this database that is the common case: an invoice-bridged
// Bargain Bay order carries its delivery as a LINE INSIDE the subtotal, so
// `total − subtotal − hst` comes out at exactly 0. Counting that as "known" made
// every Bargain Bay delivery report revenue 0 with nothing flagged, which is the
// precise failure this file's second rule exists to prevent.
const REVENUE_KNOWN = `(j.charge_amount IS NOT NULL OR COALESCE(${ORDER_FEE}, 0) > 0)`;
...
const REVENUE_MISSING = `(j.status = 'done' AND j.type <> 'return_to_base' AND NOT ${REVENUE_KNOWN})`;
// Parking the van is not a delivery. It is on the board so the Done tap gives
// us a finish time, and it must not become a stop that "nobody charged for".
const IS_WORK = `j.type <> 'return_to_base'`;
const COUNTED = `j.status IN ('done','failed') AND ${IS_WORK}`;
// Only a duration that makes sense is summed. Live data had a stop clocked in at
// 20:28 and out at 17:30 — three hours of NEGATIVE time, enough on its own to
// drag the day's total hours below zero. A clock that runs backwards is a wrong
// number to be corrected (the Times tab flags it), never one to add up.
const HOURS = `EXTRACT(EPOCH FROM (j.time_out - j.time_in)) / 3600.0`;
const SANE = `j.time_in IS NOT NULL AND j.time_out IS NOT NULL AND j.time_out > j.time_in`;

// `shf` buckets a SHIFT by the Toronto day it started. A shift is a timestamptz,
// not a date, and a driver clocking on at 8pm must land on that evening's day
// and not on the UTC tomorrow it technically already is.
const SHIFT_DAY = `(s.started_at AT TIME ZONE 'America/Toronto')::date`;
const GROUPS = {
  day:   { sql: 'j.job_date', exp: 'e.expense_date', shf: SHIFT_DAY },
  week:  { sql: `date_trunc('week',  j.job_date)::date`, exp: `date_trunc('week',  e.expense_date)::date`,
           shf: `date_trunc('week',  ${SHIFT_DAY})::date` },
  month: { sql: `date_trunc('month', j.job_date)::date`, exp: `date_trunc('month', e.expense_date)::date`,
           shf: `date_trunc('month', ${SHIFT_DAY})::date` }
};

// What a shift is allowed to be worth paying for.
//
// A shift is only priceable if somebody clocked OFF it and the result is a
// believable day. Neither half is pedantry: over one fortnight this database
// holds a 191-hour shift and an 83-hour one — a driver who forgot the Done tap,
// not a driver who was working. Charging Ruban's $25/h against 191 hours
// invents $4,775 of cost, and the owner would be reading it as fact.
//
// The ceiling is MAX_SHIFT_HOURS (lib/constants.js), not 14. It was 14, and
// that refused real days: Ruban and Kowsi genuinely run past 14 hours on long
// drives (owner, 2026-09-14), so a properly clocked-off 12 September went
// uncosted and Crew came out short.
//
// So an unclosed or implausible shift is LEFT OUT and COUNTED, never estimated.
// The report says how many it dropped. That is the same rule mileageReport
// already follows for L/100km: name what is missing rather than invent it.
const SHIFT_PRICEABLE = `s.ended_at IS NOT NULL
  AND s.ended_at > s.started_at
  AND EXTRACT(EPOCH FROM (s.ended_at - s.started_at)) <= ${MAX_SHIFT_HOURS} * 3600`;
...
export async function profitReport({ from, to, group = 'day' } = {}) {
  ...
  await ensureExpenseSchema();
  // The labour and truck figures read driver_shifts, vehicles.day_rate and
  // users.hourly_rate, so the columns have to exist before the query runs.
  await ensureShiftSchema();
  const start = asDate(from, 'start date') || torontoToday();
  const end = asDate(to, 'end date') || start;

  const [work, spend, byDriver, lines, expenses, labour, trucks] = await Promise.all([
    query(
      `SELECT ${GROUPS[g].sql} AS bucket,
              ...
              COALESCE(SUM(${HOURS}) FILTER (WHERE ${SANE}), 0) AS hours
         FROM jobs j
    ...
    query(
      // **The double-count this guard exists to prevent.** The 20ft box truck
      // comes from a carrier who bills fortnightly for the truck and its diesel
      // together. A driver still logs the fills — that is how we know the
      // litres and therefore the mileage — but the money is already in the
      // carrier's invoice, so counting the fill as well charges us for the same
      // tank twice. Fuel on a `carrier` truck is reported SEPARATELY and is not
      // part of cost. Our own pickups are unaffected: the driver pumps, we
      // e-transfer them, and that fill is the only record of the money.
      `SELECT ${GROUPS[g].exp} AS bucket,
              COALESCE(SUM(e.amount) FILTER (
                WHERE e.kind = 'gas' AND COALESCE(v.fuel_paid_by,'us') <> 'carrier'), 0) AS gas,
              COALESCE(SUM(e.amount) FILTER (
                WHERE e.kind = 'gas' AND COALESCE(v.fuel_paid_by,'us') =  'carrier'), 0) AS carrier_fuel,
    ...
    // What the DAY cost in people and trucks, as opposed to what a stop cost.
    //
    // The carrier's invoice for the box truck is truck + diesel + Ruban's hours,
    // and it does not arrive for a fortnight — so the owner cannot tell on the
    // night whether the day made money. This builds the same figure from what we
    // already know: the driver's own clock at his own rate, the truck at its
    // day rate, and the diesel from the fills. When the invoice does come it
    // should land near this, and the difference is worth asking about.
    query(
      `SELECT ${GROUPS[g].shf} AS bucket,
              COALESCE(SUM(
                EXTRACT(EPOCH FROM (s.ended_at - s.started_at)) / 3600.0
                * COALESCE(u.hourly_rate, 0)
              ) FILTER (WHERE ${SHIFT_PRICEABLE}), 0) AS labour,
              COALESCE(SUM(
                EXTRACT(EPOCH FROM (s.ended_at - s.started_at)) / 3600.0
              ) FILTER (WHERE ${SHIFT_PRICEABLE}), 0) AS shift_hours,
              COUNT(*) FILTER (WHERE ${SHIFT_PRICEABLE})::int AS priced_shifts,
              -- Left out of the money on purpose, and counted so the report can
              -- say so. An unclosed shift is not a free one.
              COUNT(*) FILTER (WHERE NOT (${SHIFT_PRICEABLE}))::int AS unpriced_shifts,
              COUNT(*) FILTER (WHERE ${SHIFT_PRICEABLE} AND COALESCE(u.hourly_rate, 0) = 0)::int AS no_rate_shifts
         FROM driver_shifts s
         LEFT JOIN users u ON u.id = s.user_id
        WHERE ${SHIFT_DAY} BETWEEN $1::date AND $2::date
        GROUP BY 1`,
      [start, end]
    ),
    // The truck is a day rate, charged once per van per day it was driven —
    // not once per shift, or a two-man day on one truck pays for two trucks.
    query(
      `SELECT bucket, COALESCE(SUM(day_rate), 0) AS trucks, COUNT(*)::int AS truck_days FROM (
         SELECT DISTINCT ${GROUPS[g].shf} AS bucket, ${SHIFT_DAY} AS day,
                s.vehicle_id, COALESCE(v.day_rate, 0) AS day_rate
           FROM driver_shifts s
           JOIN vehicles v ON v.id = s.vehicle_id
          WHERE s.driving = true
            AND ${SHIFT_DAY} BETWEEN $1::date AND $2::date
       ) d GROUP BY bucket`,
      [start, end]
    )
  ]);
  ...
  // A day with a shift on it but no stops is still a day that cost money, so
  // the shift buckets join the key set rather than only decorating existing ones.
  const keys = new Set([
    ...work.rows.map((r) => iso(r.bucket)),
    ...spend.rows.map((r) => iso(r.bucket)),
    ...labour.rows.map((r) => iso(r.bucket))
  ]);
  const spendBy = new Map(spend.rows.map((r) => [iso(r.bucket), r]));
  const workBy = new Map(work.rows.map((r) => [iso(r.bucket), r]));
  const labourBy = new Map(labour.rows.map((r) => [iso(r.bucket), r]));
  const truckBy = new Map(trucks.rows.map((r) => [iso(r.bucket), r]));
  ...
  const buckets = [...keys].filter(Boolean).sort().reverse().map((key) => {
    ...
    // Deliberately NOT in `cost`: it is already inside the carrier's invoice,
    // which arrives as an `other` row of its own.
    const carrierFuel = round2(Number(s.carrier_fuel) || 0);
    const l = labourBy.get(key) || {};
    const tk = truckBy.get(key) || {};
    const labourCost = round2(Number(l.labour) || 0);
    const truckCost = round2(Number(tk.trucks) || 0);
    const cost = round2(driverPay + gas + other + labourCost + truckCost);
    return {
      key, label: bucketLabel(key, g),
      jobs: w.jobs || 0,
      failed: w.failed || 0,
      hours: Math.round((Number(w.hours) || 0) * 100) / 100,
      revenue, driverPay, gas, otherCost: other, carrierFuel,
      labour: labourCost,
      truck: truckCost,
      truckDays: tk.truck_days || 0,
      shiftHours: Math.round((Number(l.shift_hours) || 0) * 100) / 100,
      // The three ways this figure can be short, each named rather than folded
      // into the number: a shift nobody closed, a driver with no rate on file,
      // and a stop with no charge.
      unpricedShifts: l.unpriced_shifts || 0,
      noRateShifts: l.no_rate_shifts || 0,
      cost,
  ...
  const totals = {
    ...
    otherCost: sum('otherCost'), carrierFuel: sum('carrierFuel'),
    labour: sum('labour'), truck: sum('truck'),
    shiftHours: Math.round(buckets.reduce((a, b) => a + (b.shiftHours || 0), 0) * 100) / 100,
    unpricedShifts: buckets.reduce((a, b) => a + (b.unpricedShifts || 0), 0),
    noRateShifts: buckets.reduce((a, b) => a + (b.noRateShifts || 0), 0),
  ...
  return {
    ...
    lines: lines.rows.map((r) => ({
      ...
      revenueFrom: r.charge_amount != null ? 'charge' : (r.revenue > 0 ? 'order_fee' : null),
...
export async function stopTimes({ from, to, driverId } = {}) {
  ...
  const out = rows.map((r) => {
    ...
    const mins = timeIn && r.time_out
      ? Math.round((new Date(r.time_out) - new Date(timeIn)) / 60000) : null;
    return {
      ...
      // The clock runs backwards, or the stop took no time at all. Both are
      // real on this database: a close-out before the times were stamped by the
      // taps wrote time_in and time_out at the same instant, so a genuine
      // delivery reports zero minutes — and one stop is clocked in at 20:28 and
      // out at 17:30. Neither is a duration; both are a number to correct.
      badTimes: mins != null && mins <= 0,
      // Finished, but there is nothing usable on the clock. A stop with both
      // times missing and a stop with two identical times are the same problem
      // to whoever has to cost it, so they are flagged the same way.
      missingTimes: r.status === 'done' && !(mins > 0),
  ...
  return {
    ...
    bad: out.filter((r) => r.badTimes).length
  };
}
