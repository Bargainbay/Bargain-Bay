...
import { appendTrackerUnits, highestSkuNumber, setTrackerStatus, readByStatus, sheetsConfigured } from './sheets';
...
// The lot an invoice's units are filed under IS the supplier's order number, as
// typed (owner, 2026-09-17). It used to be a minted `IN-<clock>`, which meant the
// tracker, RS Ops and the supplier's paperwork each called the same truckload
// something different — S-ORD115612 arrived in the tracker as IN-MU4H8DGQ and had
// to be renamed by hand. A minted id is only used when there is no number at all.
//
// The SKU prefix is the same number with anything outside [A-Za-z0-9._-] turned
// into a dash: a SKU goes into product URLs, photo keys and warehouse stickers,
// and "INV 123/4" survives none of those. The Lot cell keeps the number verbatim.
export function lotForInvoice(invoice) {
  const lot = String(invoice || '').trim();
  const base = lot.replace(/[^A-Za-z0-9._-]+/g, '-').replace(/-{2,}/g, '-').replace(/^-+|-+$/g, '');
  if (!base) {
    const minted = `IN-${Date.now().toString(36).toUpperCase()}`;
    return { lot: minted, base: minted };
  }
  return { lot, base };
}
...
export async function addIntakeLines(lines, { vendor, invoice } = {}) {
  ...
  // A per-line invoice only names the lot when every line agrees on it.
  const lineInvoices = [...new Set(list.map((l) => String(l.invoice || '').trim()).filter(Boolean))];
  const { lot, base } = lotForInvoice(invoice || (lineInvoices.length === 1 ? lineInvoices[0] : ''));
  // Carry on from the last number already used under this order. Splitting a big
  // invoice into two commits is what the 300-unit limit below tells people to
  // do, and restarting at -001 would hand the second half the first half's SKUs.
  const start = await highestSkuNumber(base);
  const units = [];
  for (const l of list) {
    const qty = Math.max(1, Math.min(50, Math.round(Number(l.qty) || 1)));
    for (let i = 0; i < qty; i++) {
      units.push({
        lot, sku: `${base}-${String(start + units.length + 1).padStart(3, '0')}`,
  ...
  return { created: units.map((u) => u.sku), count: units.length, lot, units };
}
...
export async function addConsignmentUnit({ make, model, category, condition, cost, retail, vendor, serial, description, note, notes, createdBy }) {
  if (!sheetsConfigured()) throw notConfigured();
