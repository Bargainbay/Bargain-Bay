  ...
  const cancelled = await query(
    `UPDATE orders o SET status = 'cancelled'
      WHERE o.status = 'pending_payment' AND o.created_at < now() - interval '24 hours'
        ...
        -- ...and never an order raised by an invoice. Those sit in
    ...
    // Fresh database with no invoices table yet — fall back to the plain sweep.
    if (!/invoices/i.test(e.message || '')) throw e;
    return query(
