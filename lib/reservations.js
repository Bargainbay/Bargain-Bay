...
// Two different long holds, and collapsing them into one is a way to put an
// appliance somebody has already paid a deposit on back on sale.
//
// CHECKOUT_HOLD_MINUTES — a STOREFRONT offline order (e-transfer /
// pay-on-pickup). Not abandoned the way an unfinished card checkout is, so
// longer than 30 minutes. This was 60 days, which — with card payments off, so
// EVERY storefront order is offline — meant one junk order took a sellable
// one-of-a-kind unit off the site for two months. A week is long enough for a
// real e-transfer to land, after which the order self-cancels and the unit
// relists.
//
// OFFLINE_HOLD_MINUTES — a hold placed by an INVOICE (holdInvoiceSkus,
// updateOrderItems). Deliberately left long. The shop's flow is deposit now,
// balance on delivery, and a delivery is routinely booked weeks out: cutting
// this to a week would expire the reservation on a sale that is going perfectly
// well, and the unit would quietly go back on the storefront underneath the
// customer. Note the order itself is safe either way — the sweep below refuses
// to cancel anything carrying a manual invoice — but the ORDER and its
// RESERVATION are separate things, and only the reservation keeps the unit off
// the site (see `unavailableSkus`: pending_payment alone does not).
//
// This split only became necessary when invoices started raising their own
// orders (2026-08-26), three weeks after the shorter hold was written.
export const OFFLINE_HOLD_DAYS = Number(process.env.UNPAID_ORDER_DAYS || 7);
export const CHECKOUT_HOLD_MINUTES = OFFLINE_HOLD_DAYS * 24 * 60;
export const OFFLINE_HOLD_MINUTES = 60 * 24 * 60;
