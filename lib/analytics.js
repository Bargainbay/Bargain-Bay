...
import { round2, HST_RATE, LEAD_SOURCES, leadSourceLabel } from './constants';
import { loadUnits } from './inventory';
import { laborCostBetween } from './payroll';

// Orders that count as a real sale on the dashboards.
//
// Two kinds qualify:
//   * a settled order — confirmed / ready / out_for_delivery / delivered; and
//   * an order raised by a LIVE invoice that hasn't been settled yet.
//
// The second is what makes a deposit sale count. An invoice raises its
// fulfilment order the moment it's written, dated to the invoice, and parks it
// in 'pending_payment' until the money is in. Without this clause a unit sold on
// a deposit today with the balance due on delivery would be invisible on the
// dashboard for however long that took, and would then land on the wrong day.
// Cancelling, voiding or refunding the invoice cancels that order, so the money
// drops back out on its own.
//
// Scoped deliberately to invoice-bridged orders: a plain pending_payment row is
// an unfinished web checkout, which is NOT a sale until it settles. And scoped to
// this file — lib/finance-report.js keeps its own cash-basis SALE so the books
// still report money actually collected.
const SALE = (t = 'orders') => `(
...
export async function dashboardData() {
  ...
  const kpi = (await query(`
    SELECT
      (SELECT COALESCE(SUM(total),0) FROM orders WHERE ${SALE('orders')})                                   AS revenue,
      (SELECT COUNT(*)              FROM orders WHERE ${SALE('orders')})                                     AS orders,
      -- Appliances only. Services, discounts and trade-in credits are lines on
      -- an order, not units that left the building. Legacy rows have no kind
      -- and stay counted, so history doesn't move under anyone.
      (SELECT COUNT(*) FROM order_items oi JOIN orders o ON o.id=oi.order_id
        WHERE ${SALE('o')} AND COALESCE(oi.kind,'unit') = 'unit')                                    AS units_sold,
...
function dashWindow(period) {
  switch (period) {
    ...
    // Calendar weeks, Monday-based (date_trunc('week') is ISO Monday) — the same
    // week the payroll runs on, so labor and the P&L line up.
    case 'week': return {
      cur: [`date_trunc('week',${NOW})`, `date_trunc('week',${NOW}) + interval '7 days'`],
      prev: [`date_trunc('week',${NOW}) - interval '7 days'`, `date_trunc('week',${NOW})`],
      unit: 'day', fmt: 'Dy DD', prevLabel: 'last week' };
    case 'last_week': return {
      cur: [`date_trunc('week',${NOW}) - interval '7 days'`, `date_trunc('week',${NOW})`],
      prev: [`date_trunc('week',${NOW}) - interval '14 days'`, `date_trunc('week',${NOW}) - interval '7 days'`],
      unit: 'day', fmt: 'Dy DD', prevLabel: 'week before' };
    ...
    case 'all': return {
      cur: [`COALESCE((SELECT date_trunc('month', min(${LT('created_at')})) FROM orders WHERE ${SALE('orders')}), date_trunc('year',${NOW}))`,
...
export async function revenueDashboard(period = 'month') {
  ...
  const ro = (await query(`
    ...
    FROM (SELECT total, hst, ${LT('created_at')} AS lt FROM orders WHERE ${SALE('orders')}) o
  `)).rows[0] || {};
  ...
  const up = (await query(`
    ...
    FROM (
      SELECT oi.price, COALESCE(oi.cost, p.cost) AS cost, ${LT('o.created_at')} AS lt
  ...
  const byCategory = (await query(`
    SELECT COALESCE(p.category,'Other') AS category, COUNT(*) AS units,
           COALESCE(SUM(oi.price),0) AS revenue, COALESCE(SUM(COALESCE(oi.cost, p.cost)),0) AS cost
      FROM order_items oi JOIN orders o ON o.id = oi.order_id
      LEFT JOIN products p ON p.sku = oi.sku
     WHERE ${SALE('o')} AND ${inCur(LT('o.created_at'))}
  ...
  const topCustomers = (await query(`
    SELECT COALESCE(NULLIF(name,''), email) AS name, lower(email) AS email,
           COUNT(*) AS orders, COALESCE(SUM(total),0) AS spent
      FROM orders
     WHERE ${SALE('orders')} AND ${inCur(LT('created_at'))}
     GROUP BY COALESCE(NULLIF(name,''), email), lower(email)
     ORDER BY spent DESC LIMIT 8
  ...
  try {
    // Outstanding = unpaid balances: whole total for open invoices, the remaining
    // balance for partially-paid ones (deposits already received don't count as owed).
    const i = (await query(`
      SELECT COALESCE(SUM(i.total - pay.paid),0) AS total, COUNT(*) AS count,
             COALESCE(SUM(i.total - pay.paid) FILTER (WHERE i.due_date < now()),0) AS overdue_total,
             COUNT(*) FILTER (WHERE i.due_date < now()) AS overdue_count
        FROM invoices i
        LEFT JOIN LATERAL (SELECT COALESCE(SUM(amount),0) AS paid FROM invoice_payments p WHERE p.invoice_id = i.id) pay ON true
       WHERE i.status IN ('open','partial')`)).rows[0] || {};
  ...
  try {
    reps = (await query(`
      SELECT sales_rep AS rep, COUNT(*) AS orders, COALESCE(SUM(total),0) AS revenue
        FROM orders
       WHERE ${SALE('orders')} AND sales_rep IS NOT NULL AND sales_rep <> '' AND ${inCur(LT('created_at'))}
  ...
  // Of the revenue booked in this window, how much is still to be collected —
  // the unpaid balance of any live invoice behind it. Pro-rated onto the pre-tax
  // basis (× the order's ex-HST share) so it's comparable with `rev`, which is
  // pre-tax. Booked = collected + owing, so the dashboard can show what was sold
  // and what has actually landed without the two being read as separate money.
  let owing = 0;
  try {
    owing = num((await query(`
      SELECT COALESCE(SUM(
               (i.total - pay.paid) * (o.total - COALESCE(o.hst,0)) / NULLIF(o.total,0)
             ),0) AS owing
        FROM orders o
        JOIN invoices i ON i.order_id = o.id AND i.status IN ('open','partial')
        LEFT JOIN LATERAL (
          SELECT COALESCE(SUM(amount),0) AS paid FROM invoice_payments p WHERE p.invoice_id = i.id
        ) pay ON true
       WHERE ${SALE('o')} AND ${inCur(LT('o.created_at'))}
    `)).rows[0]?.owing);
  } catch { /* invoice_payments provisions on first write */ }
  ...
  return {
    ...
    kpis: {
      ...
      // Split of the booked figure: what's in the bank vs what's still owed on it.
      owing: Math.max(0, Math.min(owing, rev)),
      collected: Math.max(0, rev - Math.max(0, Math.min(owing, rev))),
...
export async function customersDashboard(period = 'month') {
  ...
  const bx = (await query(`
    WITH pb AS (SELECT DISTINCT lower(email) AS em FROM orders WHERE ${SALE('orders')} AND ${inCur(LT('created_at'))})
    SELECT COUNT(*) AS total,
      COUNT(*) FILTER (WHERE EXISTS (
        SELECT 1 FROM orders o2 WHERE lower(o2.email)=pb.em AND ${SALE('o2')} AND ${LT('o2.created_at')} < ${cs}
      )) AS returning
    FROM pb`)).rows[0] || {};
  ...
  const rep = (await query(`
    SELECT COUNT(*) AS buyers, COUNT(*) FILTER (WHERE cnt > 1) AS repeat
      FROM (SELECT lower(email) AS em, COUNT(*) AS cnt FROM orders WHERE ${SALE('orders')} GROUP BY 1) t`)).rows[0] || {};
  ...
  const clv = num((await query(`
    SELECT COALESCE(AVG(spent),0) AS clv FROM (
      SELECT lower(email) AS em, SUM(total) AS spent FROM orders WHERE ${SALE('orders')} GROUP BY 1) t`)).rows[0]?.clv);
  ...
  const geography = (await query(`
    SELECT COALESCE(NULLIF(city,''),'—') AS city, COUNT(*) AS orders, COALESCE(SUM(total),0) AS revenue
      FROM orders WHERE ${SALE('orders')} AND ${inCur(LT('created_at'))}
...
// Cost coverage on sold units: a full ledger of every sold SKU'd unit with its
// sale price vs. recorded cost, plus the subset that has NO meaningful cost
// (products.cost is NULL or <= 0). Service/fee lines (no SKU) are excluded — they
// legitimately have no cost. No customer PII — order #, unit, price, cost only.
export async function soldUnitsMissingCost() {
  if (!hasDb()) return { count: 0, uncostedValue: 0, byOrder: [], ledger: [], totals: { soldValue: 0, cost: 0 } };
  const { rows } = await query(`
    SELECT o.order_number, o.created_at, oi.sku, oi.title, oi.price,
           COALESCE(oi.cost, p.cost) AS cost,
           p.make, p.model, (p.sku IS NULL) AS not_in_catalog
      FROM order_items oi
      JOIN orders o ON o.id = oi.order_id
      LEFT JOIN products p ON p.sku = oi.sku
     WHERE ${SALE('o')} AND oi.sku IS NOT NULL
     ORDER BY o.created_at DESC, oi.id`);
  const r2 = (n) => Math.round(n * 100) / 100;
  const ledger = rows.map((r) => {
    const price = num(r.price), cost = r.cost == null ? null : num(r.cost);
    const noCost = cost == null || cost <= 0;
    return {
      order: r.order_number || '(no order #)',
      date: r.created_at ? r.created_at.toISOString().slice(0, 10) : null,
      sku: r.sku, title: r.title, price, cost,
      marginPct: !noCost ? r2((1 - cost / price) * 100) : null,
      noCost, notInCatalog: r.not_in_catalog
    };
  });
  const missing = ledger.filter((u) => u.noCost);
  const byOrderMap = new Map();
  for (const u of missing) {
    if (!byOrderMap.has(u.order)) byOrderMap.set(u.order, { order: u.order, date: u.date, units: [] });
    byOrderMap.get(u.order).units.push({ sku: u.sku, title: u.title, price: u.price, cost: u.cost, notInCatalog: u.notInCatalog });
  }
  const soldValue = ledger.reduce((s, u) => s + u.price, 0);
  const cost = ledger.reduce((s, u) => s + (u.cost || 0), 0);
  return {
    count: missing.length,
    uncostedValue: r2(missing.reduce((s, u) => s + u.price, 0)),
    byOrder: [...byOrderMap.values()],
    totals: { soldValue: r2(soldValue), cost: r2(cost), units: ledger.length },
    ledger
  };
}
...
export async function financialDashboard(period = 'month') {
  ...
  const rev = num((await query(`SELECT COALESCE(SUM(total - COALESCE(hst,0)),0) AS r FROM orders WHERE ${SALE('orders')} AND ${inCur(LT('created_at'))}`)).rows[0]?.r);
  const cg = (await query(`
    SELECT COALESCE(SUM(oi.price),0) AS sold_value, COALESCE(SUM(COALESCE(oi.cost, p.cost)),0) AS cost
      FROM order_items oi JOIN orders o ON o.id = oi.order_id
      LEFT JOIN products p ON p.sku = oi.sku
     WHERE ${SALE('o')} AND ${inCur(LT('o.created_at'))}`)).rows[0] || {};
  const cogs = num(cg.cost), grossProfit = rev - cogs;

  // Gross-profit trend (item revenue − item cost) per bucket.
  const series = (await query(`
    SELECT to_char(g, '${w.fmt}') AS label,
           COALESCE(SUM(oi.price - COALESCE(oi.cost, p.cost, 0)),0) AS value
      FROM generate_series(${cs}, ${ce} - interval '1 ${w.unit}', interval '1 ${w.unit}') g
      LEFT JOIN orders o ON ${SALE('o')} AND date_trunc('${w.unit}', ${LT('o.created_at')}) = g
  ...
  try {
    const a = (await query(`
      SELECT
        COALESCE(SUM(i.total - pay.paid) FILTER (WHERE due_date IS NULL OR due_date >= now()::date),0) AS current,
        COALESCE(SUM(i.total - pay.paid) FILTER (WHERE due_date < now()::date AND due_date >= now()::date - 30),0) AS d30,
        COALESCE(SUM(i.total - pay.paid) FILTER (WHERE due_date < now()::date - 30 AND due_date >= now()::date - 60),0) AS d60,
        COALESCE(SUM(i.total - pay.paid) FILTER (WHERE due_date < now()::date - 60),0) AS d90,
        COALESCE(SUM(i.total - pay.paid),0) AS total
      FROM invoices i
      LEFT JOIN LATERAL (SELECT COALESCE(SUM(amount),0) AS paid FROM invoice_payments p WHERE p.invoice_id = i.id) pay ON true
      WHERE i.status IN ('open','partial')`)).rows[0] || {};
  ...
  try {
    collections = (await query(`
      SELECT method, SUM(amount) AS amount FROM (
        ...
        UNION ALL
        SELECT COALESCE(NULLIF(payment_method,''),'unspecified') AS method, total AS amount
          FROM orders WHERE ${SALE('orders')} AND order_number IS NOT NULL AND ${inCur(LT('created_at'))}
  ...
  // Labor in the period — the crew's piece-rate/delivery/hourly pay from the
  // payroll system, so net profit reflects what the work actually cost.
  let labor = 0;
  try { labor = (await laborCostBetween(cs, ce)).total; } catch { /* no labor table */ }

  const opex = expenses.total + adSpend + labor;
  ...
  return {
    ...
    kpis: {
      ...
      opex, adSpend, expenses: expenses.total, labor, netProfit,
...
export async function marketingDashboard(period = 'month') {
  ...
  revenue = num((await query(`SELECT COALESCE(SUM(total - COALESCE(hst,0)),0) AS r FROM orders WHERE ${SALE('orders')} AND ${inCur(LT('created_at'))}`)).rows[0]?.r);
...
// ---- HST remittance ------------------------------------------------------
//
// HST charged on sales, less the input tax credits recorded against it — what is
// actually owed. Answers "how much of this money is not mine".
//
// BASIS. HST is charged on the SALE, so every figure here is dated to the order
// (`orders.created_at`, which for an invoice-raised sale is the invoice date) —
// not to when the money landed. That is what an accrual filer reports, and it is
// the same basis as the Revenue KPI beside it, so the two agree. The
// collected/owing split is shown because the cash position is a different
// question from the liability: tax is owed on a deposit sale from the day it was
// written, even though most of the money arrives on delivery.
//
// REFUNDS need no line of their own. A refund shrinks its order's `hst` in
// place, in the month of the original sale (see the refund model), so every
// total here is already net of anything handed back.
//
// CREDITS come from three places, all dated to the document (accrual, the same
// basis as the sales side): the tax on a supplier stock invoice, on an operating
// expense, and on ad spend. A credit counts only where somebody actually
// recorded one — `expenses.tax` is NULL until then, and null is not zero.
//
// `coverage` reports how much recorded spending still has no tax figure against
// it. Without that, a remittance that silently treats unreviewed spending as
// tax-free overstates what's owed and nothing on the screen would say so.
export async function hstRemittance(period = 'month') {
  if (!hasDb()) return null;
  if (!DASH_PERIODS.some((p) => p.key === period)) period = 'month';
  const w = dashWindow(period);
  const [cs, ce] = w.cur;

  // The unpaid share of an order's HST: the outstanding balance on its live
  // invoice times the order's tax fraction. An order with no live invoice behind
  // it has been settled, and contributes nothing.
  //
  // ONE invoice, not a SUM over them: one live invoice per order is the rule, and
  // if history ever left two, the newest wins rather than the two being added
  // together — same guard as balancesForOrders in lib/jobs.js. Summing would
  // report tax as uncollected twice over. GREATEST pins an overpaid invoice at
  // zero instead of letting it credit tax back.
  const OWING_HST = `
    GREATEST(COALESCE((
      SELECT (i.total - COALESCE((
                SELECT SUM(p.amount) FROM invoice_payments p WHERE p.invoice_id = i.id
              ), 0)) * COALESCE(o.hst,0) / NULLIF(o.total,0)
        FROM invoices i
       WHERE i.order_id = o.id AND i.status IN ('open','partial')
       ORDER BY i.id DESC LIMIT 1
    ), 0), 0)`;

  const COLS = `
    COALESCE(SUM(COALESCE(o.hst,0)),0)           AS charged,
    COALESCE(SUM(${OWING_HST}),0)                AS owing,
    COALESCE(SUM(o.total - COALESCE(o.hst,0)),0) AS sales,
    COUNT(*)                                     AS orders`;

  const shape = (r) => {
    const charged = num(r?.charged);
    const owing = Math.min(charged, Math.max(0, num(r?.owing)));
    return {
      charged: round2(charged),
      owing: round2(owing),
      collected: round2(charged - owing),
      sales: round2(num(r?.sales)),
      orders: num(r?.orders)
    };
  };

  const YEAR_START = `date_trunc('year',${NOW})`;
  const YEAR_END = `${YEAR_START} + interval '1 year'`;

  // Two queries: the chosen window, and the calendar year broken into quarters.
  // The year-to-date figure is the quarters added up rather than a third query —
  // they cover exactly the same rows, so they cannot disagree.
  // Every input tax credit on record, whatever it was paid on. Unioned rather
  // than queried source by source, so a bucket can't gain one and lose another.
  // invoice_date / incurred_on / spent_on are DATE columns — unlike the sales
  // side there is no timezone to convert.
  const CREDITS = `
    SELECT invoice_date AS d, tax, 'stock' AS src FROM purchase_invoices
    UNION ALL SELECT incurred_on, tax, 'expenses' FROM expenses WHERE tax IS NOT NULL
    UNION ALL SELECT spent_on,    tax, 'ads'      FROM ad_spend WHERE tax IS NOT NULL`;
  const CREDIT_COLS = `
    COALESCE(SUM(tax) FILTER (WHERE src = 'stock'),0)    AS stock,
    COALESCE(SUM(tax) FILTER (WHERE src = 'expenses'),0) AS expenses,
    COALESCE(SUM(tax) FILTER (WHERE src = 'ads'),0)      AS ads`;

  // The finance tables self-provision on first write, so a shop that has never
  // recorded a credit has no tables to read. That has to look like "no credits
  // yet", not like a broken dashboard.
  const none = () => ({ rows: [] });
  const [cur, qs, curCredits, qCredits, cov] = await Promise.all([
    query(`SELECT ${COLS} FROM orders o
            WHERE ${SALE('o')} AND ${LT('o.created_at')} >= ${cs} AND ${LT('o.created_at')} < ${ce}`),
    query(`SELECT EXTRACT(QUARTER FROM ${LT('o.created_at')})::int AS q, ${COLS}
             FROM orders o
            WHERE ${SALE('o')} AND ${LT('o.created_at')} >= ${YEAR_START} AND ${LT('o.created_at')} < ${YEAR_END}
            GROUP BY 1 ORDER BY 1`),
    query(`SELECT ${CREDIT_COLS} FROM (${CREDITS}) c WHERE d >= ${cs} AND d < ${ce}`).catch(none),
    query(`SELECT EXTRACT(QUARTER FROM d)::int AS q, ${CREDIT_COLS}
             FROM (${CREDITS}) c WHERE d >= ${YEAR_START} AND d < ${YEAR_END}
            GROUP BY 1 ORDER BY 1`).catch(none),
    // How much of the year's recorded spending has had its tax looked at. Stock
    // invoices are always reviewed (the tax is confirmed at intake), so only the
    // two ledgers that can hold a NULL are counted.
    query(`SELECT
             COALESCE(SUM(amount),0)                             AS spend,
             COALESCE(SUM(amount) FILTER (WHERE tax IS NOT NULL),0) AS reviewed,
             COUNT(*) FILTER (WHERE tax IS NULL)                 AS unreviewed
           FROM (
             SELECT incurred_on AS d, amount, tax FROM expenses
             UNION ALL SELECT spent_on, amount, tax FROM ad_spend
           ) sp WHERE d >= ${YEAR_START} AND d < ${YEAR_END}`).catch(none)
  ]);

  const creditsOf = (r) => {
    const stock = round2(num(r?.stock)), expenses = round2(num(r?.expenses)), ads = round2(num(r?.ads));
    return { stock, expenses, ads, total: round2(stock + expenses + ads) };
  };
  const byQCredits = new Map(qCredits.rows.map((r) => [Number(r.q), creditsOf(r)]));

  const nowRow = (await query(`SELECT EXTRACT(QUARTER FROM ${NOW})::int AS q, EXTRACT(YEAR FROM ${NOW})::int AS y`)).rows[0] || {};
  const thisQuarter = Number(nowRow.q) || 1;
  const year = Number(nowRow.y) || new Date().getFullYear();
  const Q_LABELS = ['Jan–Mar', 'Apr–Jun', 'Jul–Sep', 'Oct–Dec'];
  const byQ = new Map(qs.rows.map((r) => [Number(r.q), shape(r)]));

  // Quarters up to the one we're in. A quarter with no sales still prints, so a
  // gap reads as a quiet quarter rather than as missing data; a quarter that
  // hasn't started doesn't, and the current one is flagged so a part-period
  // total is never mistaken for a finished one.
  const NO_CREDITS = { stock: 0, expenses: 0, ads: 0, total: 0 };
  const quarters = [];
  for (let q = 1; q <= thisQuarter; q++) {
    const b = byQ.get(q) || { charged: 0, owing: 0, collected: 0, sales: 0, orders: 0 };
    const c = byQCredits.get(q) || NO_CREDITS;
    quarters.push({
      quarter: `Q${q}`, label: Q_LABELS[q - 1], ...b,
      credits: c, net: round2(b.charged - c.total),
      inProgress: q === thisQuarter
    });
  }
  const yearToDate = quarters.reduce((a, q) => ({
    charged: round2(a.charged + q.charged),
    owing: round2(a.owing + q.owing),
    collected: round2(a.collected + q.collected),
    sales: round2(a.sales + q.sales),
    orders: a.orders + q.orders,
    credits: {
      stock: round2(a.credits.stock + q.credits.stock),
      expenses: round2(a.credits.expenses + q.credits.expenses),
      ads: round2(a.credits.ads + q.credits.ads),
      total: round2(a.credits.total + q.credits.total)
    },
    net: round2(a.net + q.net)
  }), { charged: 0, owing: 0, collected: 0, sales: 0, orders: 0, credits: { ...NO_CREDITS }, net: 0 });

  // Recorded spending with no tax figure against it. A high number here is the
  // difference between "we owe this" and "we owe at most this".
  const covRow = cov.rows[0] || {};
  const spend = round2(num(covRow.spend));
  const reviewed = round2(num(covRow.reviewed));
  const coverage = {
    spend, reviewed,
    unreviewedRows: num(covRow.unreviewed),
    pct: spend > 0 ? Math.round((reviewed / spend) * 100) : 100
  };

  return {
    period,
    periodLabel: (DASH_PERIODS.find((p) => p.key === period) || {}).label || '',
    year,
    current: (() => {
      const b = shape(cur.rows[0]);
      const c = creditsOf(curCredits.rows[0]);
      return { ...b, credits: c, net: round2(b.charged - c.total) };
    })(),
    yearToDate,
    quarters,
    coverage,
...
export async function reportsData() {
  ...
  const revenueByMonth = (await query(`
    SELECT to_char(date_trunc('month', created_at),'YYYY-MM') AS month, COALESCE(SUM(total),0) AS revenue
      FROM orders WHERE ${SALE('orders')} GROUP BY 1 ORDER BY 1 DESC LIMIT 12
...
export async function customerList() {
  ...
  const { rows } = await query(`
    SELECT u.id, u.name, u.email, u.phone, u.role, u.member_status, u.business_name, u.created_at,
           COUNT(o.id)       FILTER (WHERE ${SALE('o')}) AS orders,
           COALESCE(SUM(o.total) FILTER (WHERE ${SALE('o')}), 0) AS spent,
           MAX(o.created_at) FILTER (WHERE ${SALE('o')}) AS last_order
      FROM users u
...
//
// Who sent us the business: walk-ins, the website, referrals — and, where a
// person sent the lead, their name. Read from ORDERS, not invoices, for two
// reasons: revenue lives on orders (so this table agrees with the Revenue KPI
// on the same screen, to the cent), and a storefront sale has no manual invoice
// anybody typed an answer into — checkout stamps its own 'website'.
//
// It lives in this file so it shares the dashboard's `SALE` predicate and window
// arithmetic rather than copying them. `SALE` is deliberately duplicated across
// the four files listed in CLAUDE.md; a fifth copy for a panel that sits ON the
// sales dashboard would be a copy that could disagree with the figure directly
// above it.
//
// UNRECORDED IS A ROW, NOT A GAP. A sale nobody answered for is reported as
// "Not recorded" with its revenue, because the number that matters when this is
// switched on is how much of the month is still unexplained — a report that
// quietly showed only the marked sales would read as complete on its first day,
// when it covers almost nothing.
export async function leadReport(period = 'month') {
  if (!hasDb()) return null;
  if (!DASH_PERIODS.some((p) => p.key === period)) period = 'month';
  const w = dashWindow(period);
  const [cs, ce] = w.cur;
  ...
  // Pre-tax, exactly as the Revenue KPI is: HST was never the shop's money.
  const REV = 'COALESCE(SUM(o.total - COALESCE(o.hst,0)),0)';
  const WHERE = `WHERE ${SALE('o')} AND ${inCur(LT('o.created_at'))}`;

  let bySource = [], bySender = [];
  try {
    bySource = (await query(`
      SELECT o.lead_source AS src, COUNT(*) AS orders, ${REV} AS revenue
        FROM orders o ${WHERE}
       GROUP BY 1 ORDER BY revenue DESC`)).rows.map((r) => ({
      key: r.src || null,
      // An answer recorded before a source was renamed (or typed by an importer)
      // still prints as itself rather than vanishing into "Not recorded" — the
      // row exists, we just have no label for it.
      label: leadSourceLabel(r.src) || (r.src ? String(r.src) : 'Not recorded'),
      known: !!r.src,
      orders: num(r.orders),
      revenue: num(r.revenue)
    }));
  } catch (e) {
    // Almost always the column not being provisioned yet (a deploy without the
    // migration), which is what the panel offers to fix. Logged rather than
    // silently swallowed so a real query fault is findable.
    console.error('leadReport failed', e.message);
    return null;
  }

  try {
    // Grouped on the FOLDED name, displayed as the spelling used most often.
    // The field is free text with a datalist that suggests but cannot constrain,
    // so "Dave Mitchell" on Monday and "dave mitchell" on Tuesday are one person
    // — grouping on the raw string splits their revenue in half and shows the
    // owner two referrers who each sent half the work, which is the one number
    // this panel exists to get right. mode() picks the winning spelling; the
    // count tiebreak is arbitrary but stable, and any spelling of a name is a
    // better label than a lowercased one.
    bySender = (await query(`
      SELECT mode() WITHIN GROUP (ORDER BY o.lead_by) AS name,
             COUNT(*) AS orders, ${REV} AS revenue,
             ARRAY_AGG(DISTINCT o.lead_source) FILTER (WHERE o.lead_source IS NOT NULL) AS sources
        FROM orders o ${WHERE} AND COALESCE(o.lead_by,'') <> ''
       GROUP BY lower(btrim(o.lead_by)) ORDER BY revenue DESC`)).rows.map((r) => ({
      name: r.name,
      orders: num(r.orders),
      revenue: num(r.revenue),
      sources: (r.sources || []).map((k) => leadSourceLabel(k) || k)
    }));
  } catch { bySender = []; }

  const orders = bySource.reduce((a, r) => a + r.orders, 0);
  const revenue = round2(bySource.reduce((a, r) => a + r.revenue, 0));
  const unrecorded = bySource.find((r) => !r.known) || { orders: 0, revenue: 0 };
  return {
    period,
    bySource,
    bySender,
    totals: {
      orders,
      revenue,
      unrecordedOrders: unrecorded.orders,
      unrecordedRevenue: round2(unrecorded.revenue),
      // What share of the period somebody actually answered for. This is the
      // number that says whether the rest of the table can be trusted yet.
      coverage: orders ? ((orders - unrecorded.orders) / orders) * 100 : 0
    },
    // Offered to the view so a source with no sales in the period still reads as
    // a zero rather than being absent — the owner asked "how many walk-ins",
    // and nothing is a real answer to that.
    allSources: Object.entries(LEAD_SOURCES).map(([key, label]) => ({ key, label }))
  };
}
