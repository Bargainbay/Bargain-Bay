...
export async function financialDashboard(period = 'month') {
  ...
  const rev = num((await query(`SELECT COALESCE(SUM(total),0) AS r FROM orders WHERE status IN ${SALE} AND ${inCur(LT('created_at'))}`)).rows[0]?.r);
...
export async function marketingDashboard(period = 'month') {
  ...
  revenue = num((await query(`SELECT COALESCE(SUM(total),0) AS r FROM orders WHERE status IN ${SALE} AND ${inCur(LT('created_at'))}`)).rows[0]?.r);
  ...
  try {
    revByChannel = (await query(`
      SELECT COALESCE(NULLIF(source,''),'Direct') AS channel, COALESCE(SUM(total),0) AS revenue, COUNT(*) AS orders
        FROM orders WHERE status IN ${SALE} AND ${inCur(LT('created_at'))}
