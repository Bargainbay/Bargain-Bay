...
//
// What the shop has to hand to the CRA off its own sales, and what it has
// actually taken in yet. Answers "how much of this is not mine".
//
...
//
// WHAT THIS IS NOT: the remittance figure. That is this minus input tax credits
// — the HST paid out on stock and expenses — and none of that is captured
// anywhere in this system. The panel says so rather than implying a number the
// owner could file.
export async function hstOwed(period = 'month') {
  if (!hasDb()) return null;
  if (!DASH_PERIODS.some((p) => p.key === period)) period = 'month';
  const w = dashWindow(period);
  const [cs, ce] = w.cur;
  ...
  for (let q = 1; q <= thisQuarter; q++) {
    ...
    quarters.push({ quarter: `Q${q}`, label: Q_LABELS[q - 1], ...b, inProgress: q === thisQuarter });
  }
  ...
  }), { charged: 0, owing: 0, collected: 0, sales: 0, orders: 0 });

  return {
    period,
    periodLabel: (DASH_PERIODS.find((p) => p.key === period) || {}).label || '',
    year,
    current: shape(cur.rows[0]),
    yearToDate,
    quarters,
    rate: HST_RATE
  };
