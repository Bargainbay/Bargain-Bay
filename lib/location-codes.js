...
//   part label    →  https://bargainbay.ca/w/p/<part id>
...
//
// **A part label carries the part's id, not its part number.** A part number is
// what somebody reads off the old part and it is allowed to be blank, to be
// typed two ways, or to be corrected later; the id is the row. It also keeps the
// payload short — `/w/p/42` encodes in far fewer modules than a SKU URL, which
// is what makes a part label legible at 1 x 1 in where a unit sticker is not.

export const UNIT_PATH = '/w/u/';
export const SPOT_PATH = '/w/l/';
export const PART_PATH = '/w/p/';
...
// A part id is a `parts.id`. Anything else off a label is a malformed sticker,
// and 0 is not a row — so it reads as null rather than as part number zero.
export const normPartId = (s) => {
...
export const partScanUrl = (base, id) => `${trimBase(base)}${PART_PATH}${encodeURIComponent(String(id))}`;

// What did the scanner just read? → { kind: 'unit' | 'location' | 'part' | 'text', value }
// or null. A caller that predates parts must still be given something it can
// handle, so an unreadable part label falls through to 'text' rather than
// arriving as a part with no id.
export function parseScan(raw) {
  ...
  // bargainbay.ca.
  const m = text.match(/\/w\/([ulp])\/([^/?#\s]+)/i);
  if (m) {
    let v = m[2];
    try { v = decodeURIComponent(v); } catch { /* a malformed escape is still the best text we have */ }
    const what = m[1].toLowerCase();
    if (what === 'l') return { kind: 'location', value: normCode(v) };
    if (what === 'p') {
      const id = normPartId(v);
      return id ? { kind: 'part', value: id } : { kind: 'text', value: text };
    }
    return { kind: 'unit', value: normSku(v) };
  }
