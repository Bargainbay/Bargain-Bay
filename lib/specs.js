...
export const CONDITION_COPY = {
  ...
  'New Scratch & Dent':
    'This unit is brand new and never used, with a cosmetic blemish from transit or handling — typically a scratch or dent, often on a side or back panel that hides against a wall or cabinet. Our technicians have bench-tested it and confirmed performance is not affected, and it’s backed by our one-year warranty.',
  // Kept for listings that carry the older wording.
  'Scratch & Dent':
...
export function seoDescription(unit) {
  ...
  return `${unit.make} ${unit.model} ${unit.category} — ${unit.condition}, ${price} CAD. Tested & working, one-year warranty. Pickering & Scarborough / GTA delivery.`;
}
