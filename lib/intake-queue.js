...
import { matchInvoiceLines, requestFills } from './stock-reconcile';
...
export async function commitIntake(id, { vendor, invoice, items } = {}) {
  ...
  // waiting for it. Each match is filed for an admin to approve on Stock gaps
  // (requestFills) rather than written here, and comes off its line's quantity;
  // what's left is added as new. A lookup that fails adds every line as new.
  let pending = [];
  try {
    const matches = await matchInvoiceLines(list, { invoice: inv || '' });
    if (matches.length) {
      const f = await requestFills(
        matches.map((m) => ({ line: list[m.line], skus: m.units.map((u) => u.sku) })),
        { vendor: vend, invoice: inv, lot: inv ? lotForInvoice(inv).lot : undefined, by: 'intake queue' }
      );
      pending = f.requested;
      const per = new Map(matches.map((m) => [m.line, m.units.filter((u) => pending.includes(u.sku)).length]));
      list = list
  ...
  await query("UPDATE intake_queue SET status = 'committed', added_skus = $2, reviewed_at = now() WHERE id = $1", [id, JSON.stringify(r.created)]);
  return { addedSkus: r.created, pendingSkus: pending, count: r.count, failed: [] };
}
