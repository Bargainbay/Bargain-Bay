...
import { round2, MAX_SHIFT_HOURS, torontoTime } from './constants';
...
export async function listVehicles({ includeInactive = false } = {}) {
  ...
  await ensureShiftSchema();
  const { rows } = await query(
    `SELECT id, name, plate, active, fuel_paid_by, carrier_name, day_rate, tracker_device_id
       FROM vehicles
...
export async function upsertVehicle(payload = {}) {
  ...
  // ONLY WHAT WAS PASSED IS WRITTEN — updateJob's rule, and it matters here for
  // one field. The fuel editor, the retire button and the day-rate box all post
  // a partial van and know nothing about a tracker; if this column were written
  // unconditionally, retiring a van for the winter would quietly unpair its
  // tracker and the truck would vanish off the Live tab with nothing said.
  const setsTracker = 'trackerDeviceId' in payload;
  const tracker = setsTracker ? (clean(payload.trackerDeviceId, 64) || null) : null;
  const cols = 'id, name, plate, active, fuel_paid_by, carrier_name, day_rate, tracker_device_id';
  try {
    if (id) {
      const { rows } = await query(
        `UPDATE vehicles SET name = $2, plate = $3, active = $4, fuel_paid_by = $5, carrier_name = $6,
                day_rate = $7${setsTracker ? ', tracker_device_id = $8' : ''}
          WHERE id = $1 RETURNING ${cols}`,
        [Number(id), nm, clean(plate, 20), active !== false, paid, carrier, rate,
          ...(setsTracker ? [tracker] : [])]
      );
      if (!rows.length) throw new Error('No such van.');
      return shapeVehicle(rows[0]);
    }
    const { rows } = await query(
      `INSERT INTO vehicles (name, plate, active, fuel_paid_by, carrier_name, day_rate, tracker_device_id)
       VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING ${cols}`,
      [nm, clean(plate, 20), active !== false, paid, carrier, rate, tracker]
    );
    return shapeVehicle(rows[0]);
  } catch (e) {
    // The unique index is the one error worth translating: "duplicate key" tells
    // nobody which van already has the device, and pairing the same tracker to
    // two trucks is the mistake somebody makes when they retype a device id.
    if (e?.code === '23505' && String(e.message || '').includes('idx_vehicles_tracker')) {
      throw new Error('Another van is already paired with that tracker — unpair it there first.');
    }
    throw e;
  }
}
...
export async function setShiftTimes(shiftId, { startTime, endTime, startKm, endKm, vehicleId, clearKm, note, clear } = {}, by) {
  ...
  if (ended && started) {
    ...
    if (hours > MAX_SHIFT_HOURS && hours > wasHours) {
      throw new Error(
...
// Two shifts that were one day.
//
// A driver who clocks on against the wrong truck notices, clocks off, and clocks
// straight back on against the right one — Kowsi on 9 September, 08:50–11:12
// then 11:13–18:13. Once the office has moved the first onto the right truck the
// day is two rows of the same thing. It costs the same either way; it READS as a
// man who went home for a minute.
//
// Only for shifts that genuinely are one day, and it says why when they aren't:
// same person, both closed, same seat (the same van, or riding with the same
// person), and the second starting within MERGE_GAP_MINUTES of the first ending.
// Merging across a real break would pay somebody for time off the clock.
//
// The later row is DELETED — after its fuel fills are moved onto the one kept,
// and its times and readings are written into the kept row's note, so a merged
// shift can still say what it was made from.
const MERGE_GAP_MINUTES = 60;
export async function mergeShifts(firstId, secondId, by) {
  if (!hasDb()) throw new Error('Database not configured.');
  await ensureShiftSchema();
  const ids = [Number(firstId), Number(secondId)];
  if (!ids[0] || !ids[1] || ids[0] === ids[1]) throw new Error('Pick two different shifts.');

  return withTransaction(async (c) => {
    const { rows } = await c.query(
      `SELECT * FROM driver_shifts WHERE id = ANY($1::int[]) ORDER BY started_at FOR UPDATE`, [ids]
    );
    if (rows.length !== 2) throw new Error('One of those shifts no longer exists.');
    const [a, b] = rows;
    if (a.user_id !== b.user_id) throw new Error('Those shifts belong to two different people.');
    if (!a.ended_at || !b.ended_at) throw new Error('Close both shifts before merging them.');
    if (a.driving !== b.driving) {
      throw new Error('One of those shifts is driving and the other is riding along. Fix that first.');
    }
    if (a.driving !== false && (a.vehicle_id || null) !== (b.vehicle_id || null)) {
      throw new Error('Those shifts are on two different trucks. Put them on the same truck first.');
    }
    if (a.driving === false && (a.riding_with || null) !== (b.riding_with || null)) {
      throw new Error('Those shifts are riding with two different people.');
    }
    const gap = (new Date(b.started_at) - new Date(a.ended_at)) / 60000;
    if (gap > MERGE_GAP_MINUTES) {
      throw new Error(`There are ${Math.round(gap)} minutes between those shifts. That is a break, not one shift.`);
    }

    // `b` can sit entirely inside `a`; the merged shift ends at whichever ends last.
    const later = new Date(b.ended_at) >= new Date(a.ended_at) ? b : a;
    const hours = (new Date(later.ended_at) - new Date(a.started_at)) / 3600000;
    if (hours > MAX_SHIFT_HOURS) {
      throw new Error(`Together that is ${hours.toFixed(1)} hours, over the ${MAX_SHIFT_HOURS}-hour limit.`);
    }
    const startKm = a.start_km ?? b.start_km;
    const endKm = later.end_km ?? (later === a ? b : a).end_km;
    if (startKm != null && endKm != null && endKm < startKm) {
      throw new Error(`${endKm.toLocaleString('en-CA')} km is lower than the ${startKm.toLocaleString('en-CA')} km start.`);
    }

    const km = b.start_km != null || b.end_km != null
      ? `, ${b.start_km ?? '?'}–${b.end_km ?? '?'} km` : '';
    const note = [
      a.note,
      `merged with shift #${b.id} (${torontoTime(b.started_at)}–${torontoTime(b.ended_at)}${km})`,
      b.note
    ].filter(Boolean).join(' · ');

    await c.query('UPDATE dispatch_expenses SET shift_id = $1 WHERE shift_id = $2', [a.id, b.id]);
    await c.query('DELETE FROM driver_shifts WHERE id = $1', [b.id]);
    const { rows: kept } = await c.query(
      `UPDATE driver_shifts
          SET ended_at = $2, start_km = $3, end_km = $4,
              end_lat = $5, end_lng = $6, note = $7,
              edited_at = now(), edited_by = $8
        WHERE id = $1
        RETURNING *, NULL AS vehicle_name, NULL AS riding_with_name`,
      [a.id, later.ended_at, startKm, endKm, later.end_lat, later.end_lng, note, clean(by, 120)]
    );
    return shapeShift(kept[0]);
  });
}
...
export async function createShift({
  ...
  if (ended) {
    const hours = (new Date(ended) - new Date(started)) / 3600000;
    if (hours > MAX_SHIFT_HOURS) throw new Error(`That is ${hours.toFixed(1)} hours — check the times.`);
  }
...
export async function shiftReport({ from, to, driverId } = {}) {
  ...
  await Promise.all(out.map(async (r) => {
    ...
    r.needsFixing = !r.endedAt || hours == null || hours <= 0 || hours > MAX_SHIFT_HOURS;
