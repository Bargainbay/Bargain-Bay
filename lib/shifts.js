...
export async function listVehicles({ includeInactive = false } = {}) {
  ...
  const { rows } = await query(
    `SELECT id, name, plate, active FROM vehicles
      ${includeInactive ? '' : 'WHERE active = true'} ORDER BY name`
  );
  return rows;
}

export async function upsertVehicle({ id, name, plate, active = true }) {
  ...
  if (id) {
    const { rows } = await query(
      'UPDATE vehicles SET name = $2, plate = $3, active = $4 WHERE id = $1 RETURNING id, name, plate, active',
      [Number(id), nm, clean(plate, 20), active !== false]
    );
    if (!rows.length) throw new Error('No such van.');
    return rows[0];
  }
  const { rows } = await query(
    'INSERT INTO vehicles (name, plate, active) VALUES ($1,$2,$3) RETURNING id, name, plate, active',
    [nm, clean(plate, 20), active !== false]
  );
  return rows[0];
}
...
export async function startShift(userId, { vehicleId, startKm, at, lat, lng, note, ref } = {}) {
  if (!hasDb()) throw new Error('Database not configured.');
  await ensureShiftSchema();
  ...
  if (ref) {
    const { rows: seen } = await query(
      'SELECT s.*, NULL AS vehicle_name FROM driver_shifts s WHERE s.user_id = $1 AND s.ref = $2 LIMIT 1', [uid, ref]
    );
  ...
  const { rows } = await query(
    `INSERT INTO driver_shifts (user_id, vehicle_id, started_at, start_km, start_lat, start_lng, note, ref)
     VALUES ($1,$2,$3,$4,$5,$6,$7,$8) RETURNING *, NULL AS vehicle_name`,
    [uid, vehicleId ? Number(vehicleId) : null, when, int(startKm),
...
export async function endShift(userId, { endKm, at, lat, lng, note } = {}) {
  ...
  const { rows } = await query(
    `UPDATE driver_shifts SET ended_at = GREATEST($2, started_at), end_km = $3,
      ...
      WHERE id = $1 RETURNING *, NULL AS vehicle_name`,
    [open.id, when, km,
     Number.isFinite(Number(lat)) ? Number(lat) : null,
     Number.isFinite(Number(lng)) ? Number(lng) : null,
     clean(note)]
...
export async function mileageReport({ from, to } = {}) {
  ...
  const [dist, fuel] = await Promise.all([
    ...
    query(
      `SELECT e.vehicle_id, COALESCE(SUM(e.amount), 0) AS spend,
