...
export async function setShiftTimes(shiftId, { startTime, endTime, startKm, endKm, note, clear } = {}, by) {
  if (!hasDb()) throw new Error('Database not configured.');
  await ensureShiftSchema();
  ...
  // The odometer gets the same guard as everywhere else: a corrected shift is
  // exactly where somebody retypes the wrong truck's reading from memory.
  const kmIn = startKm === undefined || startKm === '' ? shift.start_km : int(startKm);
  const kmOut = endKm === undefined || endKm === '' ? shift.end_km : int(endKm);
  if (shift.driving !== false && shift.vehicle_id) {
    for (const km of [kmIn, kmOut]) {
      if (km != null && km !== shift.start_km && km !== shift.end_km) {
        await assertOdometer(shift.vehicle_id, km, { vehicleName: shift.vehicle_name });
      }
    }
  }
  ...
  const { rows } = await query(
    `UPDATE driver_shifts
        SET started_at = $2, ended_at = $3, start_km = $4, end_km = $5,
            note = COALESCE(NULLIF($6,''), note),
            edited_at = now(), edited_by = $7
      WHERE id = $1
      RETURNING *, NULL AS vehicle_name, NULL AS riding_with_name`,
    [id, started, ended, kmIn, kmOut, clean(note), clean(by, 120)]
  );
  return shapeShift(rows[0]);
}
