...
export async function upsertClearance({ sku, price, warrantyMonths, note, active }) {
  ...
  // Don't write a clearance row for a SKU that isn't a real product — a markdown on
  // a typo'd/non-existent SKU joins to nothing and silently does nothing.
  const { rows: p } = await query('SELECT sku, active FROM products WHERE sku = $1', [s]);
  if (!p.length) return { sku: s, found: false };
  ...
  const { rows } = await query(
    `INSERT INTO clearance (sku, price, warranty_months, note, active, updated_at)
     ...
     RETURNING sku`,
    [s, price, wm, note || null, active !== false]
  );
  return { sku: rows[0].sku, found: true, productActive: p[0].active };
}
