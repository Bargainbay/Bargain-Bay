...
// SELLABLE_STATUSES), same condition normalization and money parsing.

// What the storefront sells. A unit RS Ops has TESTED WORKING is on sale while
// it is still being cleaned or waiting for QA (owner, 2026-09-17): it sells on
// its stock photo, and RS Ops's own photos join the product page once QA
// approves them (lib/rsops.js reads RS Ops's feed, which only lists a unit after
// that approval). Everything else — untested, not working, waiting for parts,
// sold, salvage — stays off. Still gated on a price like every other row: a
// unit nobody has given a Condition has none, so an ungraded unit in cleaning
// does not appear.
//
// These strings are RS Ops's (TRACKER_STATUS in rs-ops lib/tracker.js). Matching
// is case- and whitespace-tolerant but otherwise exact: "Tested Working - Needs
// Diagnosis" is not a thing, and a near-miss must not publish a machine.
export const LIVE_STATUS = 'tested working';
export const SELLABLE_STATUSES = [LIVE_STATUS, 'tested working - needs cleaning', 'tested working - needs qa'];
...
// opts.status: which Status(es) to keep — a string or a list (default
// SELLABLE_STATUSES); opts.requirePrice: drop rows without a Suggested Sale
// Price (default true — set false for salvage).
export function parseTrackerCsv(text, opts = {}) {
  const wanted = new Set((Array.isArray(opts.status) ? opts.status : [opts.status || SELLABLE_STATUSES].flat())
  ...
  const isWantedStatus = (v) => wanted.has(norm(v));

  const units = [];
  // Diagnostics so the importer can explain exactly what it kept and skipped.
  // skippedNoPrice counts only rows marked plain "Tested Working" — a unit the
  // shop has finished and believes is for sale. A unit still in cleaning or QA
  // with no Condition is simply not graded yet, which is normal; counting it
  // there would turn the one warning that matters into noise (skippedUngraded).
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    const finishing = norm(at(r, col.status)) !== LIVE_STATUS && SELLABLE_STATUSES.includes(norm(at(r, col.status)));
