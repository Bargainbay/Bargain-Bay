...
export function parseTrackerCsv(text) {
  ...
  // silently dropping sellable units over cosmetic spelling in the tracker.
  const norm = (v) => String(v || '').toLowerCase().replace(/\s+/g, ' ').trim();
  const isTestedWorking = (v) => norm(v) === 'tested working';
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    if (!isTestedWorking(at(r, col.status))) { report.skippedNotTested++; continue; }
    const id = at(r, col.sku);
    if (!id) { report.skippedNoId++; continue; }
    const price = money(at(r, col.price));
    if (price <= 0) { report.skippedNoPrice++; continue; }
    units.push({
      id,
      make: at(r, col.make),
