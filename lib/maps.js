...
export function loadGoogleMaps() {
  ...
  _loading = new Promise((resolve) => {
    ...
    s.async = true;
    // With loading=async the Places library loads on demand — await it so
    // google.maps.places (and Autocomplete) actually exists before we resolve.
    s.onload = async () => {
      try {
        if (window.google?.maps?.importLibrary) await window.google.maps.importLibrary('places');
      } catch { /* places may already be present, or load failed — fall through */ }
