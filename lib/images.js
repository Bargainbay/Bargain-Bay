...
export function imageFor(unit) {
  // A photo of THIS unit that we took ourselves beats every other source: it is
  // the only image in the chain guaranteed to be the actual machine. Attached by
  // lib/inventory from `unit_photos` — a vendor drop-off the sales floor
  // photographed on arrival, where there is no manufacturer stock shot to find
  // and the alternative is a category placeholder.
  //
  // RS Ops photos are deliberately NOT consulted here: that gallery stays
  // additive (see lib/rsops.js) and the stock image stays primary for units the
  // refurb floor processed.
  if (unit.photos?.length) return unit.photos[0].url;
