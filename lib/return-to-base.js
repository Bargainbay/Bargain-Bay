...
//
// TWO ways to be out, because there are two ways this business records a day.
// Having stops is the obvious one. The other is being CLOCKED ON: Dhinesh
// worked today riding with Nick and appears on none of Nick's stops, so by
// stops alone he was not out — while his shift has been running since Monday.
// A driver who is on the clock is working, whatever the board says, and the
// return-to-base stop is exactly what such a day is missing: an end time.
export async function driversOut(date) {
  if (!hasDb()) return [];
  const day = /^\d{4}-\d{2}-\d{2}$/.test(String(date || '')) ? date : torontoToday();
  const { rows } = await query(
    `SELECT d.id, COALESCE(d.name, d.email) AS name,
            bool_or(d.src = 'stops') AS has_stops,
            bool_or(d.src = 'shift') AS on_shift,
       ...
       FROM (
         SELECT u.id, u.name, u.email, 'stops' AS src
           FROM jobs j JOIN users u ON u.id IN (j.driver_id, j.driver2_id)
          WHERE j.job_date = $1::date AND j.status <> 'cancelled'
            AND j.type <> 'return_to_base'
         UNION ALL
         -- On the clock on that day: either the shift started on it, or it is
         -- still open from an earlier one and has therefore run through it.
         SELECT u.id, u.name, u.email, 'shift'
           FROM driver_shifts s JOIN users u ON u.id = s.user_id
          WHERE (s.started_at AT TIME ZONE 'America/Toronto')::date = $1::date
             OR (s.ended_at IS NULL
                 AND (s.started_at AT TIME ZONE 'America/Toronto')::date <= $1::date)
       ) d
  ...
  return rows.map((r) => ({
    id: r.id, name: r.name, hasOne: !!r.has_one,
    hasStops: !!r.has_stops, onShift: !!r.on_shift
  }));
}
...
export async function addReturnToBase({ date, assignments = [], createdBy } = {}) {
  ...
  if (!bases.length) throw new Error('Set a base address first — People & access, under Vans.');
  const day = /^\d{4}-\d{2}-\d{2}$/.test(String(date || '')) ? date : torontoToday();
