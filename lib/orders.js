...
export async function updateOrderItems(idOrNumber, items) {
  ...
  // Totals: keep the existing delivery fee (total − subtotal − hst) and the
  // order's HST treatment, recompute around the new lines.
  const fee = Math.max(0, round2(Number(order.total) - Number(order.subtotal) - Number(order.hst)));
  const hasHst = Number(order.hst) > 0;
  const subtotal = round2(lines.reduce((a, l) => a + l.price, 0));
  const hst = hasHst ? round2((subtotal + fee) * HST_RATE) : 0;
  const total = round2(subtotal + fee + hst);
