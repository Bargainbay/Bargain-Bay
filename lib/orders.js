...
// count now comes from countOrders() — the real total for the window being looked
...
// How many orders there actually are in that window — the number the board's
// heading prints. Counts the same rows getAllOrders pages through, so
// "showing 200 of 431" is always true of one query, not two different ideas.
export async function countOrders(opts = {}) {
  const { from, to } = orderOpts(opts);
  const { clause, params } = ordersWindow(from, to);
  const { rows } = await query(`SELECT count(*)::int AS n FROM orders ${clause}`, params);
  return rows[0]?.n || 0;
}
