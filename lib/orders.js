...
import { round2, HST_RATE, isUnitLine, isCreditLine, normalizeLineKind, torontoToday } from './constants';
...
export async function createOrderFromInvoice(inv, { status = 'confirmed', createdAt = null, holdSkus = false } = {}) {
  ...
  const order = await withTransaction(async (client) => {
    ...
    await client.query('ALTER TABLE order_items ADD COLUMN IF NOT EXISTS kind text').catch(() => {});
    for (const it of (inv.items || [])) {
      await client.query(
        ...
        [orderId, it.sku || null, it.title || '(item)', Number(it.price) || 0,
         it.cost != null ? Number(it.cost) : null, normalizeLineKind(it.kind)]
      );
    }
    return { id: orderId, orderNumber: numbered[0].order_number };
  });

  const contested = holdSkus
    ? await holdInvoiceSkus(order.id, (inv.items || []).filter((it) => isUnitLine(it.kind)).map((it) => it.sku))
    : [];
...
async function attachItems(orders) {
  ...
  const { rows: items } = await query(
    'SELECT id, order_id, sku, title, price, kind FROM order_items WHERE order_id = ANY($1) ORDER BY id',
    [ids]
  );
...
// The admin board's window onto the orders table.
//
// It is a WINDOW and it says so, because for months it did not: the board loaded
// the newest 200 rows and printed that array's length as the order count, so the
// day the shop passed 200 orders the heading pinned at "Orders (200)" forever and
// every order past that fell off the bottom with nothing on screen to say so. The
// count now comes from orderCounts() — the real total for the window being looked
// at — and the caller can page and date-filter to reach the rest.
//
// opts: { limit, offset, from, to }. A bare number is still accepted as the limit
// so an older caller can't silently lose its cap and pull the whole table.
export const ORDERS_PAGE = 200;

// from/to are 'YYYY-MM-DD' as a person types them, and they are read in TORONTO
// time, never UTC. Vercel runs as UTC: read there, "from 2026-09-23" would start
// at 8pm on the 22nd and quietly hand back the previous evening's orders (and
// drop the last five hours of the 'to' day) — the same class of bug as the POD
// timestamps. `to` is INCLUSIVE, because a person picking a range means the day
// they named, hence the +1 and the strict <.
//
// THE `::timestamp` IS LOAD-BEARING AND IT LOOKS LIKE NOISE. `date AT TIME ZONE`
// does NOT mean "midnight in Toronto": a bare date resolves to the timestamptz
// overload, so Postgres reads it as midnight in the SESSION zone and converts it
// TO Toronto wall-clock, handing back a naive `timestamp without time zone` that
// is then re-read in the session zone when compared against created_at. The
// window silently slides by the offset between the two zones — four hours on
// Vercel, which is a whole evening of orders in and out of the wrong day, with
// nothing to see. Casting to `timestamp` first picks the overload that means
// what we want and yields a real timestamptz. Verified against Postgres, both
// the result type and the boundary rows.
const DATE_RE = /^\d{4}-\d{2}-\d{2}$/;
function ordersWindow(from, to) {
  const where = [];
  const params = [];
  if (DATE_RE.test(String(from || ''))) {
    params.push(from);
    where.push(`created_at >= ($${params.length}::date)::timestamp AT TIME ZONE 'America/Toronto'`);
  }
  if (DATE_RE.test(String(to || ''))) {
    params.push(to);
    where.push(`created_at <  ($${params.length}::date + 1)::timestamp AT TIME ZONE 'America/Toronto'`);
  }
...
function orderOpts(opts) {
  ...
  const limit = Math.min(Math.max(Number(o.limit) || ORDERS_PAGE, 1), 1000);
  const offset = Math.max(Number(o.offset) || 0, 0);
  return { limit, offset, from: o.from || null, to: o.to || null };
}

export async function getAllOrders(opts = {}) {
  const { limit, offset, from, to } = orderOpts(opts);
  const { clause, params } = ordersWindow(from, to);
  const { rows } = await query(
...
// How many orders there are in that window, and what state they are in — the
// numbers the board's heading prints. Counts the same rows getAllOrders pages
// through, so "showing 200 of 431" is always true of one query, not two
// different ideas.
//
// The breakdown exists because the bare total reads as a sales figure and is
// not one: it counts cancelled orders, refunded ones, checkouts nobody paid for
// that the sweep closed, and every invoice-raised sale. The first question asked
// of the new count was "is that true?", which is the total answering a question
// it was never asked.
//
// IT REPORTS STATUSES AND NOTHING ELSE. It is tempting to also hand back "and N
// of those are sales", and that would be a FIFTH definition of SALE in a
// codebase already carrying four deliberate copies (analytics, pnl, books,
// finance-report) with "change one, change all" written over them. A board that
// quietly disagreed with the Revenue KPI on the next tab would be worse than a
// board that makes you go and look at it. Cancelled and refunded are visible
// here so nobody reads the total as revenue; what a sale IS stays with the
// dashboards.
export async function orderCounts(opts = {}) {
  const { from, to } = orderOpts(opts);
  const { clause, params } = ordersWindow(from, to);
  const { rows } = await query(
  ...
  // Summed from the same GROUP BY rather than counted again, so the total and
  // the parts can never disagree on screen — they are one read of one query.
  const byStatus = {};
  let total = 0;
...
// not in the original constraint). Mirrors db/migrations/0001_baseline.sql. Cached per process.
...
// order_items.kind on its own, for the storefront checkout — it writes lines
// before anything on the invoice side has run, and naming a column that doesn't
// exist yet would fail EVERY checkout rather than only the unusual ones.
let _lineKindSchema = null;
export function ensureOrderLineKind() {
  if (!hasDb()) return Promise.resolve();
  if (!_lineKindSchema) {
    _lineKindSchema = query('ALTER TABLE order_items ADD COLUMN IF NOT EXISTS kind text')
      .catch((e) => { _lineKindSchema = null; throw e; });
  }
  return _lineKindSchema;
}
...
async function loadOrder(idOrNumber) {
  ...
  const order = rows[0];
  const { rows: items } = await query(
    'SELECT id, sku, title, price, cost, kind FROM order_items WHERE order_id = $1 ORDER BY id', [order.id]
  );
  return { order, items };
}
...
export async function updateOrderItems(idOrNumber, items) {
  ...
  const lines = (items || [])
    .map((it) => {
      ...
      return {
        // Only an appliance line carries a SKU or a cost; a discount or trade-in
        // is money coming off, and is stored negative however it arrives.
        sku: unit ? (String(it.sku || '').trim() || null) : null,
        title: String(it.title || '').trim().slice(0, 500),
        price: isCreditLine(kind) ? -Math.abs(price) : price,
        cost: unit && Number.isFinite(Number(it.cost)) && Number(it.cost) >= 0 ? round2(Number(it.cost)) : null,
        kind
      };
    })
    .filter((li) => li.title && Number.isFinite(li.price) && (li.price > 0 || isCreditLine(li.kind)));
  if (!lines.some((li) => li.price > 0)) throw new Error('An order needs at least one line item with a positive price.');
  ...
  if (!(subtotal > 0)) throw new Error('The credit lines cancel out the order — its total has to be more than zero.');
  ...
  await withTransaction(async (client) => {
    ...
    for (const l of lines) {
      await client.query(
        'INSERT INTO order_items (order_id, sku, title, price, cost, kind) VALUES ($1,$2,$3,$4,$5,$6)',
        [order.id, l.sku, l.title, l.price, l.cost, l.kind]
      );
    }
    await client.query(
