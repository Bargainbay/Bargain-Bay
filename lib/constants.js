...
// Today, on Toronto time. The business day is a Toronto day everywhere in this
// app — dashboard bucketing, dispatch, backdated invoices — and Vercel runs as
// UTC, so `new Date()` is the wrong answer for eight hours of every evening.
//
// ONE DEFINITION. There were three identical ones: lib/jobs (exported and
// imported by seven modules), plus private copies in lib/orders and
// lib/invoices. Same consolidation as phoneKey, and for the same reason — three
// copies of a rule is three places for it to drift.
export const torontoToday = () => new Date().toLocaleDateString('en-CA', { timeZone: TZ });
