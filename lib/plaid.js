...
import { upsertExpense, deleteExpenseByExtId } from './finance';
...
export function mapTransaction(t, institution) {
  ...
  return {
    incurredOn: t.date,
    category: PFC_CATEGORY[pfc] || 'Other',
    vendor: String(name).slice(0, 120),
    // The GROSS charge off the statement. Whatever tax was inside it is unknown
    // until a person says — see the note at the top of this file.
    ...
    try {
      for (;;) {
        ...
        for (const t of (r.added || [])) {
          const m = mapTransaction(t, item.institution);
