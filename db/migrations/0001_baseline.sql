-- 0001_baseline — the schema as it stood when migrations were introduced.
--
-- This is db/schema.sql, moved verbatim. It is entirely IF NOT EXISTS /
-- ADD COLUMN IF NOT EXISTS, so applying it to the live database does nothing
-- except record that it ran — which is the point. The ledger starts here.
--
-- DO NOT EDIT THIS FILE. It has been applied, so it is history: the runner
-- checksums it and refuses to continue if it changes. A new change goes in a
-- new numbered file.
--
-- It also does NOT describe the whole database. Twenty-seven ensureXSchema()
-- functions in lib/ still create tables at runtime, and at least one CHECK
-- constraint (jobs_type_check) exists in production and in no file here. Folding
